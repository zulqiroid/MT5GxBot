...
from tests.test_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_application import (
    FakeAllowedNearTerminalDecision,
    bullish_continued_successive_iterative_recurrent_bounded_application_decision,
)
...
def bullish_continued_successive_iterative_recurrent_bounded_progressed_state_decision():
    return create_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_progressed_state(
        bullish_continued_successive_iterative_recurrent_bounded_application_decision()
    )
...
def test_bullish_continued_successive_iterative_recurrent_progressed_state_is_created() -> None:
    decision = bullish_continued_successive_iterative_recurrent_bounded_progressed_state_decision()
...
def test_progressed_state_preserves_lineage_identity() -> None:
    ...
    state = create_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_progressed_state(
        application_decision
    ).state_required
...
def test_default_progressed_state_counters_are_exact() -> None:
    state = bullish_continued_successive_iterative_recurrent_bounded_progressed_state_decision().state_required

    assert state.state_version == 13
    assert state.cursor_index == 230
    assert state.consumed_count == 230
    assert state.remaining_count == 570
    assert state.last_consumed_sequence_index == 229
    assert state.next_event_sequence_index == 230
...
def test_progressed_state_id_is_deterministic() -> None:
    first = bullish_continued_successive_iterative_recurrent_bounded_progressed_state_decision().state_required
    second = bullish_continued_successive_iterative_recurrent_bounded_progressed_state_decision().state_required
...
def test_blocked_application_blocks_progressed_state() -> None:
    decision = create_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_progressed_state(
...
def test_terminal_progressed_state_is_exact() -> None:
    ...
    assert state.state_version == 600
...
def test_prior_state_is_not_mutated() -> None:
    ...
    _ = create_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_progressed_state(
        application_decision
    ).state_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = StrategyPhase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
    function_decision = create_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_progressed_state(
        application_decision
    )
