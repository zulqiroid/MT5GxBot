...
from tests.test_phase8_offline_replay_iterative_continuation_transition_application import (
    CAPTURED_AT,
    ...
    bullish_iterative_continuation_application_decision,
)


@lru_cache(maxsize=1)
def state_bearish_continuation_application_decision():
...
@lru_cache(maxsize=1)
def state_existing_continuation_application_decision():
    return StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationFactory().generate(
        application_existing_continuation_contract_decision()
    )
...
@lru_cache(maxsize=1)
def state_blocked_continuation_application_decision():
    return StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationFactory().generate(
        application_blocked_continuation_contract_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_iterative_continuation_progressed_state_decision():
...
def test_invalid_application_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayIterativeContinuationProgressedSessionStateError,
        match=("INVALID_ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_DECISION"),
    ) as captured:
        (
            StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
                "invalid"
            )
        )
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy(**{field_name: 1})


def test_bullish_iterative_continuation_progressed_state_is_created() -> None:
    decision = bullish_iterative_continuation_progressed_state_decision()
...
def test_bearish_iterative_continuation_progressed_state_is_created() -> None:
    decision = (
        StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
            state_bearish_continuation_application_decision()
        )
    )
...
def test_existing_iterative_continuation_progressed_state_is_created() -> None:
    decision = (
        StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
            state_existing_continuation_application_decision()
        )
    )
...
def test_blocked_application_blocks_state() -> None:
    decision = (
        StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
            state_blocked_continuation_application_decision()
        )
    )
...
def test_state_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 iterative continuation progressed",
    ):
        _ = decision.state_required


def test_state_preserves_application_identity() -> None:
    state = bullish_iterative_continuation_progressed_state_decision().state_required

    assert state.iterative_continuation_transition_application_decision is (
        bullish_iterative_continuation_application_decision()
    )
    assert state.application_receipt is (
        bullish_iterative_continuation_application_decision().receipt_required
    )
...
def test_state_preserves_primary_lineage() -> None:
    ...
    assert state.iterative_continuation_transition_contract is (receipt.transition_contract)
...
def test_state_preserves_identifiers() -> None:
    ...
    assert state.iterative_continuation_transition_contract_id == (
        state.iterative_continuation_transition_contract.stable_id
    )
...
def test_state_preserves_digests() -> None:
    ...
    assert state.iterative_continuation_transition_contract_digest == (
        state.iterative_continuation_transition_contract.transition_digest
    )
...
def test_iterative_continuation_progressed_counters_are_exact() -> None:
    state = bullish_iterative_continuation_progressed_state_decision().state_required

    assert state.state_version == 6
    assert state.cursor_index == 6
    assert state.consumed_count == 6
    assert state.remaining_count == 794
    assert state.total_event_count == 800
    assert state.last_consumed_sequence_index == 5
    assert state.next_event_sequence_index == 6
...
def test_next_event_matches_receipt() -> None:
    ...
    assert expected.sequence_index == 6
...
def test_prior_source_state_remains_unchanged() -> None:
    ...
    assert prior.state_version == 5
    assert prior.cursor_index == 5
    assert prior.consumed_count == 5
    assert prior.remaining_count == 795
    assert prior.last_consumed_sequence_index == 4
    assert prior.next_event_sequence_index == 5
...
def test_state_is_reusable_state_only() -> None:
    ...
    assert state.is_iterative_continuation_progressed_state is True
    ...
    assert state.can_continue_to_future_iterative_continuation_transition_contract is True
...
def test_iterative_continuation_progressed_state_id_is_deterministic() -> None:
    state = bullish_iterative_continuation_progressed_state_decision().state_required

    assert state.iterative_continuation_progressed_state_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_iterative_continuation_application_decision()
    decision = bullish_iterative_continuation_progressed_state_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = state_blocked_continuation_application_decision()
    decision = (
        StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
            source
        )
...
def test_direct_state_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="IterativeContinuationProgressedSessionStateMode",
    ):
        replace(
            state,
            state_mode=("IMMUTABLE_ITERATIVE_CONTINUATION_PROGRESSED_STATE"),
        )
...
def test_direct_state_rejects_raw_lifecycle() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="IterativeContinuationProgressedSessionLifecycle",
    ):
        replace(
            state,
            lifecycle="ACTIVE",
...
def test_manual_decision_rejects_wrong_status() -> None:
    ...
    with pytest.raises(ValueError, match="does not match"):
        replace(
            decision,
            status=(Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus.BLOCKED),
        )
...
def test_state_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory()
    source = bullish_iterative_continuation_application_decision()
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_iterative_continuation_progressed_session_state(
        bullish_iterative_continuation_application_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplayIterativeContinuationProgressedSessionState
        is StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionState
    )
    assert (
        Phase8OfflineReplayIterativeContinuationProgressedSessionStateFactory
        is StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory
    )
