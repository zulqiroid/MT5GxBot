...
from tests.test_phase8_offline_replay_iterative_progressed_session_state import (
    CAPTURED_AT,
    bullish_iterative_progressed_state_decision,
...
@lru_cache(maxsize=1)
def continuation_bearish_source_decision():
...
@lru_cache(maxsize=1)
def continuation_existing_source_decision():
    return StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        state_existing_iterative_application_decision()
    )
...
@lru_cache(maxsize=1)
def continuation_blocked_source_decision():
    return StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        state_blocked_iterative_application_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_iterative_continuation_contract_decision():
...
def test_invalid_source_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayIterativeContinuationTransitionContractError,
        match="INVALID_ITERATIVE_PROGRESSED_STATE_DECISION",
    ) as captured:
        (
            StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory().generate(
                "invalid"
            )
        )
...
def test_default_policy_is_strict() -> None:
    ...
    assert policy.source_state_active is True
    assert policy.current_event_bound is True
    assert policy.sequence_continuity_verified is True
    assert policy.consume_exactly_one_future_event is True
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayIterativeContinuationTransitionContractPolicy(**{field_name: 1})


def test_bullish_continuation_contract_is_created() -> None:
    decision = bullish_iterative_continuation_contract_decision()
...
def test_bearish_continuation_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory().generate(
...
def test_existing_continuation_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory().generate(
...
def test_blocked_state_blocks_continuation_contract() -> None:
    decision = StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory().generate(
...
def test_contract_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 iterative continuation",
    ):
        _ = decision.transition_contract_required


def test_contract_preserves_source_identity() -> None:
    contract = bullish_iterative_continuation_contract_decision().transition_contract_required

    assert contract.iterative_progressed_state_decision is (
        bullish_iterative_progressed_state_decision()
    )
    assert contract.source_state is (bullish_iterative_progressed_state_decision().state_required)
...
def test_contract_preserves_lineage() -> None:
    ...
    assert contract.prior_transition_contract is (state.iterative_transition_contract)
...
def test_current_transition_state_is_exact() -> None:
    ...
    assert contract.transition_index == 5
    assert contract.current_cursor_index == 5
    assert contract.current_consumed_count == 5
    assert contract.current_remaining_count == 795
    assert contract.prior_last_consumed_sequence_index == 4
    assert contract.current_event_sequence_index == 5
...
def test_resulting_transition_state_is_exact() -> None:
    ...
    assert contract.resulting_cursor_index == 6
    assert contract.resulting_consumed_count == 6
    assert contract.resulting_remaining_count == 794
    assert contract.last_consumed_sequence_index == 5
    assert contract.completion_after_transition is False
    assert contract.next_event_sequence_index == 6
...
def test_next_event_matches_resulting_cursor() -> None:
    contract = bullish_iterative_continuation_contract_decision().transition_contract_required
    expected = contract.event_batch.events[6]
...
def test_contract_is_definition_only() -> None:
    ...
    assert contract.authorizes_transition is True
    assert contract.source_state_preserved is True
    ...
    assert contract.can_continue_to_iterative_continuation_application is True
...
def test_decision_performs_no_transition() -> None:
    ...
    assert decision.authorizes_transition is True
...
def test_contract_id_is_deterministic() -> None:
    ...
    assert contract.continuation_transition_contract_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_iterative_progressed_state_decision()
    decision = bullish_iterative_continuation_contract_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = continuation_blocked_source_decision()
    decision = StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory().generate(
        source
    )
    ...
    assert decision.stable_id == (
        ...
        "CONTINUATION_CONTRACT_GENERATION:"
        "BLOCKED:"
        "ITERATIVE_PROGRESSED_STATE_BLOCKED:"
        "ITERATIVE_PROGRESSED_STATE_BLOCKED"
    )
...
def test_direct_contract_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            contract,
            policy=(
                Phase8OfflineReplayIterativeContinuationTransitionContractPolicy(no_lookahead=False)
            ),
        )
...
def test_direct_contract_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="ContinuationTransitionContractMode",
    ):
        replace(
            contract,
            contract_mode=("IMMUTABLE_SINGLE_EVENT_CONTINUATION"),
        )
...
def test_direct_contract_rejects_raw_action() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="ContinuationTransitionAction",
    ):
        replace(
            contract,
...
def test_direct_contract_rejects_wrong_consumed_count() -> None:
    ...
    with pytest.raises(ValueError, match="transition_index"):
        replace(
            contract,
            current_consumed_count=6,
            current_remaining_count=794,
        )
...
def test_direct_contract_rejects_wrong_current_sequence() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            contract,
            current_event_sequence_index=6,
...
def test_contract_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        contract.resulting_cursor_index = 7
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayIterativeContinuationTransitionContractStatus.BLOCKED


def test_contract_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory()
    source = bullish_iterative_progressed_state_decision()
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_iterative_continuation_transition_contract(
        bullish_iterative_progressed_state_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplayIterativeContinuationTransitionContract
        is StrategyPhase8OfflineReplayIterativeContinuationTransitionContract
    )
    assert (
        Phase8OfflineReplayIterativeContinuationTransitionContractFactory
        is StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory
    )
