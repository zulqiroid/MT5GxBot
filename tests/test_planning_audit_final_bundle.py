...
def test_invalid_completion_is_fail_safe() -> None:
    with pytest.raises(
        PlanningAuditFinalBundleError,
        match="INVALID_PERSISTENCE_COMPLETION_DECISION",
    ) as captured:
        (StrategyPlanningAuditFinalBundleFactory().generate("invalid"))
...
def test_bullish_final_bundle_is_created() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(bullish_persistence_completion())

    assert decision.status == (PlanningAuditFinalBundleStatus.CREATED)
    assert decision.reason == (PlanningAuditFinalBundleReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_bundle is True


def test_existing_final_bundle_is_created() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(existing_persistence_completion())
    bundle = decision.bundle_required

    assert decision.is_created is True
    assert bundle.outcome_kind == (PlanningAuditPersistenceOutcomeKind.EXISTING)
    assert bundle.certificate.indicates_existing is True


def test_bearish_final_bundle_is_created() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(bearish_persistence_completion())

    assert decision.is_created is True
    assert decision.direction == (DirectionalPermissionDirection.BEARISH)
    assert decision.bundle_required.side == StrategyOrderSide.SELL


def test_blocked_completion_produces_no_bundle() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(blocked_persistence_completion())

    assert decision.is_blocked is True
    assert decision.bundle is None
    assert decision.has_bundle is False
    assert decision.reason == (PlanningAuditFinalBundleReason.PERSISTENCE_COMPLETION_BLOCKED)
    assert decision.blockers == (PlanningAuditFinalBundleBlocker.PERSISTENCE_COMPLETION_BLOCKED,)


def test_bundle_required_rejects_blocked_result() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(blocked_persistence_completion())

    with pytest.raises(
        ValueError,
        match="No final planning audit bundle",
    ):
        _ = decision.bundle_required


def test_bundle_preserves_completion_decision() -> None:
    completion = bullish_persistence_completion()
    bundle = StrategyPlanningAuditFinalBundleFactory().generate(completion).bundle_required

    assert bundle.persistence_completion is completion
    assert bundle.certificate is (completion.certificate_required)


def test_bundle_preserves_complete_object_lineage() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    )

    assert bundle.receipt is bundle.certificate.receipt
    assert bundle.evidence is bundle.certificate.evidence
    assert bundle.outcome_contract is bundle.certificate.contract
    assert bundle.request_verification_receipt is bundle.outcome_contract.verification_receipt
    assert bundle.request is (bundle.outcome_contract.request)
    assert bundle.binding_verification_receipt is bundle.request.receipt


def test_bundle_preserves_metadata() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.broker_symbol == "XAUUSDm"
    assert bundle.observed_at == OBSERVED_AT
    assert bundle.direction == (DirectionalPermissionDirection.BULLISH)
    assert bundle.side == StrategyOrderSide.BUY
    assert bundle.schema_version == (PLANNING_AUDIT_FINAL_BUNDLE_SCHEMA_VERSION)


def test_required_components_are_exact() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.component_count == 7


def test_component_ids_preserve_exact_lineage() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.component_ids == (
        bundle.request.binding_verification.stable_id,
        bundle.request.stable_id,
        (bundle.outcome_contract.request_verification.stable_id),
        bundle.outcome_contract.stable_id,
        bundle.receipt.outcome_evidence.stable_id,
        bundle.certificate.outcome_receipt.stable_id,
        bundle.certificate.stable_id,
    )


def test_component_digests_preserve_exact_lineage() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.component_digests == (
        bundle.certificate.binding_receipt_digest,
        bundle.certificate.request_digest,
        (bundle.request_verification_receipt.verification_digest),
        bundle.certificate.outcome_contract_digest,
        bundle.certificate.evidence_digest,
        bundle.certificate.outcome_receipt_digest,
        bundle.certificate.completion_digest,
    )


def test_component_identities_are_unique() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert len(set(bundle.component_ids)) == 7
...
def test_component_digests_are_lowercase_sha256(
    digest,
) -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
    ...
    value = bundle.component_digests[digest]
...
def test_external_persistence_metadata_is_sealed() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.target == bundle.certificate.target
    assert bundle.outcome_kind == (bundle.certificate.outcome_kind)
    assert bundle.source_name == (bundle.certificate.source_name)
    assert bundle.storage_record_reference == (bundle.certificate.storage_record_reference)


def test_request_identity_is_sealed() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.request_id == (bundle.certificate.request_id)
    assert bundle.request_id == (bundle.request.request_id)
...
def test_payload_lineage_is_sealed() -> None:
    ...
    assert bundle.content_length_bytes == (bundle.certificate.content_length_bytes)
    assert bundle.content_length_bytes == (bundle.request.content_length_bytes)
    assert bundle.content_digest == (bundle.certificate.content_digest)
    assert bundle.content_digest == (bundle.request.content_digest)
    assert bundle.manifest_digest == (bundle.certificate.manifest_digest)
    assert bundle.manifest_digest == (bundle.request.manifest_digest)
    assert bundle.idempotency_key == (bundle.certificate.idempotency_key)
    assert bundle.idempotency_key == (bundle.request.idempotency_key)


def test_binding_identity_lineage_is_sealed() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.binding_id == (bundle.certificate.binding_id)
    assert bundle.binding_id == (bundle.request.binding_id)
    assert bundle.snapshot_id == (bundle.certificate.snapshot_id)
    assert bundle.snapshot_id == (bundle.request.snapshot_id)
    assert bundle.adapter_contract_id == (bundle.certificate.contract_id)
    assert bundle.adapter_contract_id == (bundle.request.contract_id)
...
def test_bundle_digests_are_lowercase_sha256(
    field_name,
) -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
...
def test_bundle_digest_matches_canonical_payload() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    )
    expected = hashlib.sha256(bundle.canonical_payload.encode("utf-8")).hexdigest()

    assert bundle.bundle_digest == expected
    assert bundle.digest_algorithm == "SHA-256"


def test_bundle_marks_phase_7_complete() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.is_tamper_evident is True
    assert bundle.phase_7_complete is True
    assert decision.phase_7_complete is True
    assert bundle.can_continue_to_phase_8_design is True
    assert decision.can_continue_to_phase_8_design is True


def test_bundle_records_but_does_not_perform_persistence() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.records_external_persistence is True
    assert bundle.performed_persistence is False


def test_bundle_performs_no_write_or_execution() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    assert bundle.has_adapter_instance is False
    assert bundle.request_submission_authorized is False
    assert bundle.adapter_invocation_authorized is False
    assert bundle.storage_write_authorized is False
    assert bundle.can_write_storage is False
    assert bundle.can_write_network is False
    assert bundle.execution_authorized is False
    assert bundle.has_broker_request is False
    assert bundle.can_submit_order is False
    assert bundle.is_executable is False
...
def test_bundle_has_no_implementation_surface(
    attribute_name,
) -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
    ...
    assert not hasattr(bundle, attribute_name)
...
def test_bundle_id_is_deterministic() -> None:
    ...
    assert bundle.bundle_id == (
        "XAUUSDm:BUY:"
        "FINAL_PLANNING_AUDIT_BUNDLE:"
        "CREATED:"
        "RECORD[AUDIT-RECORD-0001]:"
        f"BUNDLE_SHA256[{bundle.bundle_digest}]"
    )


def test_bundle_stable_id_is_deterministic() -> None:
    completion = bullish_persistence_completion()
    bundle = StrategyPlanningAuditFinalBundleFactory().generate(completion).bundle_required

    assert bundle.stable_id == (
        f"{completion.stable_id}:FINAL_PLANNING_AUDIT_BUNDLE:{bundle.bundle_id}"
    )


def test_created_decision_stable_id_is_deterministic() -> None:
    completion = bullish_persistence_completion()
    decision = StrategyPlanningAuditFinalBundleFactory().generate(completion)

    assert decision.stable_id == (
        f"{completion.stable_id}:FINAL_PLANNING_AUDIT_BUNDLE_GENERATION:CREATED:CREATED:NONE"
    )


def test_blocked_decision_stable_id_is_deterministic() -> None:
    completion = blocked_persistence_completion()
    decision = StrategyPlanningAuditFinalBundleFactory().generate(completion)

    assert decision.stable_id == (
        f"{completion.stable_id}:"
        "FINAL_PLANNING_AUDIT_BUNDLE_GENERATION:"
        "BLOCKED:PERSISTENCE_COMPLETION_BLOCKED:"
        "PERSISTENCE_COMPLETION_BLOCKED"
    )


def test_direct_bundle_rejects_blocked_completion() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ValueError,
        match="completed persistence decision",
    ):
        replace(
            bundle,
            persistence_completion=(blocked_persistence_completion()),
        )


def test_direct_bundle_rejects_wrong_schema() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
...
def test_direct_bundle_requires_tuple_components() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
...
def test_direct_bundle_rejects_raw_components() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ValueError,
        match="component members",
    ):
        replace(
            bundle,
            components=tuple(component.value for component in bundle.components),
        )


def test_direct_bundle_rejects_missing_component() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ValueError,
        match="all seven required components",
    ):
        replace(
            bundle,
            components=bundle.components[:-1],
        )


def test_direct_bundle_rejects_reordered_components() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ...
        replace(
            bundle,
            components=tuple(reversed(bundle.components)),
        )


def test_direct_bundle_requires_tuple_component_ids() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ValueError,
        match="component_ids must be a tuple",
    ):
        replace(
            bundle,
...
def test_direct_bundle_requires_tuple_component_digests() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    with pytest.raises(
        ValueError,
        match="component_digests must be a tuple",
    ):
        replace(
            bundle,
            component_digests=list(bundle.component_digests),
        )


def test_direct_bundle_rejects_foreign_component_id() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    component_ids = list(bundle.component_ids)
    component_ids[0] = "foreign-component"
...
def test_direct_bundle_rejects_foreign_component_digest() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
    ...
    component_digests = list(bundle.component_digests)
    component_digests[0] = "0" * 64

    with pytest.raises(
        ValueError,
        match="component_digests",
    ):
        replace(
            bundle,
            component_digests=tuple(component_digests),
        )
...
def test_direct_bundle_rejects_raw_enums(
    field_name,
    value,
    message,
) -> None:
...
def test_direct_bundle_rejects_foreign_values(
    field_name,
    value,
) -> None:
    bundle = (
...
def test_direct_bundle_rejects_uppercase_digest() -> None:
    bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
...
def test_manual_decision_rejects_missing_bundle() -> None:
    decision = StrategyPlanningAuditFinalBundleFactory().generate(bullish_persistence_completion())
...
def test_manual_decision_rejects_unexpected_bundle() -> None:
    blocked = StrategyPlanningAuditFinalBundleFactory().generate(blocked_persistence_completion())
    created_bundle = (
        StrategyPlanningAuditFinalBundleFactory()
        .generate(bullish_persistence_completion())
        .bundle_required
...
def test_manual_decision_rejects_duplicate_blockers() -> None:
    ...
    with pytest.raises(ValueError, match="duplicates"):
        replace(
            decision,
            blockers=(
                PlanningAuditFinalBundleBlocker.PERSISTENCE_COMPLETION_BLOCKED,
                PlanningAuditFinalBundleBlocker.PERSISTENCE_COMPLETION_BLOCKED,
            ),
        )
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = PlanningAuditFinalBundleStatus.BLOCKED


def test_generation_is_deterministic() -> None:
    factory = StrategyPlanningAuditFinalBundleFactory()
    completion = bullish_persistence_completion()

    assert factory.generate(completion) == factory.generate(completion)


def test_function_api_delegates() -> None:
    decision = generate_planning_audit_final_bundle(bullish_persistence_completion())
...
def test_factory_alias_methods_delegate() -> None:
    ...
    assert factory.build(completion) == factory.generate(completion)
    assert factory.evaluate(completion) == factory.generate(completion)


def test_public_aliases_are_preserved() -> None:
    assert AuditFinalBundle is StrategyPlanningAuditFinalBundle
    assert AuditFinalBundleComponent is PlanningAuditFinalBundleComponent
    assert AuditFinalBundleDecision is PlanningAuditFinalBundleDecision
    assert AuditFinalBundleFactory is StrategyPlanningAuditFinalBundleFactory
    assert PlanningAuditFinalBundle is StrategyPlanningAuditFinalBundle
    assert PlanningAuditFinalBundleFactory is StrategyPlanningAuditFinalBundleFactory
    assert StrategyAuditFinalBundle is StrategyPlanningAuditFinalBundle
    assert StrategyAuditFinalBundleFactory is StrategyPlanningAuditFinalBundleFactory
