...
from tests.test_phase13_deterministic_fake_runtime_boundary_validation import (
    bullish_phase13_fake_boundary_validation_decision,
)
...
@lru_cache(maxsize=1)
def bullish_phase13_runtime_safety_audit_decision():
    return audit_phase13_controlled_read_only_runtime_safety(
        bullish_phase13_fake_boundary_validation_decision()
    )


def test_static_runtime_safety_audit_contract_is_stable() -> None:
    assert PHASE_13_RUNTIME_SAFETY_AUDIT_SCHEMA_VERSION == "1.0"
    assert PHASE_13_RUNTIME_SAFETY_AUDIT_STATUS == "PASSED"
    assert PHASE_13_RUNTIME_SAFETY_AUDIT_HANDOFF_STATUS == "READY_FOR_FINAL_HANDOFF"
    assert PHASE_13_RUNTIME_SAFETY_AUDIT_SOURCE == "DETERMINISTIC_FAKE_BOUNDARY_EVIDENCE_ONLY"


def test_runtime_safety_audit_report_is_created() -> None:
...
def test_complete_lineage_is_preserved() -> None:
    validation_decision = bullish_phase13_fake_boundary_validation_decision()
    validation = validation_decision.report_required
    report = audit_phase13_controlled_read_only_runtime_safety(validation_decision).report_required

    assert report.validation_decision is validation_decision
    assert report.validation_report is validation
    assert report.boundary_decision is validation.boundary_decision
    assert report.runtime_boundary_contract is validation.runtime_boundary_contract
    ...
    assert report.phase12_handoff_bundle is validation.phase12_handoff_bundle


def test_audit_status_and_handoff_are_exact() -> None:
    report = bullish_phase13_runtime_safety_audit_decision().report_required
    assert report.audit_status == "PASSED"
    assert report.handoff_status == "READY_FOR_FINAL_HANDOFF"
    assert report.audit_source == "DETERMINISTIC_FAKE_BOUNDARY_EVIDENCE_ONLY"
    assert report.runtime_safety_audit_passed is True
    assert report.ready_for_final_handoff is True


def test_operation_error_and_snapshot_counts_are_exact() -> None:
    report = bullish_phase13_runtime_safety_audit_decision().report_required
    ...
    assert report.validation_event_count == 15


def test_operation_and_error_contracts_are_valid() -> None:
    report = bullish_phase13_runtime_safety_audit_decision().report_required
    assert report.runtime_operation_order_valid is True
    assert report.all_runtime_operations_fake_only is True
...
def test_snapshot_mappings_and_payloads_are_valid() -> None:
    report = bullish_phase13_runtime_safety_audit_decision().report_required
...
def test_runtime_safety_findings_are_complete() -> None:
    report = bullish_phase13_runtime_safety_audit_decision().report_required
...
def test_audit_id_is_deterministic() -> None:
    first = bullish_phase13_runtime_safety_audit_decision().report_required
    second = bullish_phase13_runtime_safety_audit_decision().report_required
    assert first.audit_digest == second.audit_digest
    assert first.audit_id == second.audit_id


def test_missing_validation_blocks_audit() -> None:
    decision = audit_phase13_controlled_read_only_runtime_safety(None)
    assert decision.is_allowed is False
    assert decision.report is None
    assert decision.blockers == ("fake_boundary_validation_decision_missing",)


def test_blocked_validation_blocks_audit() -> None:
    decision = audit_phase13_controlled_read_only_runtime_safety(FakeBlockedValidationDecision())
    assert decision.is_allowed is False
    assert decision.report is None
    assert decision.blockers == ("fake_boundary_validation_decision_blocked",)
...
def test_factory_and_function_api_match() -> None:
    ...
    factory = (
        StrategyPhase13ControlledReadOnlyRuntimeSafetyAuditor()
        .audit(validation_decision)
        .report_required
    )
    function = audit_phase13_controlled_read_only_runtime_safety(
        validation_decision
    ).report_required
