...
from tests.test_phase8_offline_replay_successive_iterative_recurrent_bounded_plan import (
    bullish_successive_iterative_recurrent_bounded_plan_decision,
)
...
def bullish_successive_iterative_recurrent_bounded_application_decision():
    return apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        bullish_successive_iterative_recurrent_bounded_plan_decision()
    )
...
def test_bullish_successive_iterative_recurrent_application_is_created() -> None:
    decision = bullish_successive_iterative_recurrent_bounded_application_decision()
...
def test_application_preserves_lineage_identity() -> None:
    plan_decision = bullish_successive_iterative_recurrent_bounded_plan_decision()
    plan = plan_decision.plan_required

    receipt = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        plan_decision
    ).receipt_required
...
def test_default_application_counters_are_exact() -> None:
    receipt = bullish_successive_iterative_recurrent_bounded_application_decision().receipt_required

    assert receipt.source_state_version == 11
    assert receipt.start_cursor_index == 166
    assert receipt.resulting_cursor_index == 198
    assert receipt.initial_consumed_count == 166
    assert receipt.resulting_consumed_count == 198
    assert receipt.initial_remaining_count == 634
    assert receipt.resulting_remaining_count == 602
    assert receipt.consumed_event_count == 32
    assert receipt.next_event_sequence_index == 198
...
def test_default_consumed_sequence_range_is_exact() -> None:
    ...
    assert receipt.consumed_event_sequence_indices == tuple(range(166, 198))
...
def test_application_id_is_deterministic() -> None:
    first = bullish_successive_iterative_recurrent_bounded_application_decision().receipt_required
    second = bullish_successive_iterative_recurrent_bounded_application_decision().receipt_required
...
def test_blocked_plan_blocks_application() -> None:
    decision = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
...
def test_missing_plan_blocks_application() -> None:
    decision = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(None)
...
def test_terminal_application_is_exact() -> None:
    plan_decision = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
    ...
    assert receipt.source_state_version == 499
...
def test_source_state_is_not_mutated() -> None:
    ...
    _ = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        plan_decision
    ).receipt_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = (
        StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplication().apply(
            plan_decision
        )
    )
    function_decision = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        plan_decision
    )
