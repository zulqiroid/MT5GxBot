...
def test_invalid_materialization_plan_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayEventMaterializationError,
        match="INVALID_MATERIALIZATION_PLAN_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplayEventMaterializationFactory().generate("invalid"))
...
def test_bullish_event_batch_is_created() -> None:
    decision = replay_events_bullish_decision()

    assert decision.status == (Phase8OfflineReplayEventMaterializationStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayEventMaterializationReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_batch is True


def test_bearish_event_batch_is_created() -> None:
    decision = StrategyPhase8OfflineReplayEventMaterializationFactory().generate(
        replay_events_bearish_plan_decision()
    )

    assert decision.is_created is True
    assert decision.direction == (DirectionalPermissionDirection.BEARISH)
    assert decision.batch_required.side == StrategyOrderSide.SELL


def test_existing_event_batch_is_created() -> None:
    decision = StrategyPhase8OfflineReplayEventMaterializationFactory().generate(
...
def test_blocked_plan_blocks_event_batch() -> None:
    decision = StrategyPhase8OfflineReplayEventMaterializationFactory().generate(
...
def test_batch_required_rejects_blocked_result() -> None:
    decision = StrategyPhase8OfflineReplayEventMaterializationFactory().generate(
        replay_events_blocked_plan_decision()
    )
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 offline replay-event batch",
    ):
        _ = decision.batch_required


def test_batch_preserves_plan_identity() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.materialization_plan_decision is (materialization_bullish_decision())
    assert batch.materialization_plan is (materialization_bullish_decision().plan_required)


def test_batch_preserves_complete_lineage() -> None:
    batch = replay_events_bullish_decision().batch_required
    plan = batch.materialization_plan

    assert batch.event_contract is plan.event_contract
    assert batch.replay_plan is plan.replay_plan
    assert batch.specification is plan.specification
    assert batch.input_package is plan.input_package
    assert batch.verification_receipt is (plan.verification_receipt)
    assert batch.snapshot is plan.snapshot
    assert batch.contract is plan.contract
    assert batch.dry_run_package is (plan.dry_run_package)


def test_batch_preserves_identifiers() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.materialization_plan_id == (batch.materialization_plan.stable_id)
    assert batch.event_contract_id == (batch.event_contract.stable_id)
    assert batch.replay_plan_id == (batch.replay_plan.stable_id)
    assert batch.specification_id == (batch.specification.stable_id)
    assert batch.input_package_id == (batch.input_package.stable_id)
    assert batch.snapshot_id == batch.snapshot.stable_id


def test_batch_preserves_digests() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.materialization_plan_digest == (batch.materialization_plan.materialization_digest)
    assert batch.event_contract_digest == (batch.event_contract.contract_digest)
    assert batch.replay_plan_digest == (batch.replay_plan.plan_digest)
    assert batch.specification_digest == (batch.specification.specification_digest)
    assert batch.input_digest == (batch.input_package.input_digest)
    assert batch.snapshot_digest == (batch.snapshot.snapshot_digest)


def test_batch_preserves_metadata() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.broker_symbol == "XAUUSDm"
    assert batch.direction == (DirectionalPermissionDirection.BULLISH)
    assert batch.side == StrategyOrderSide.BUY
    assert batch.source_name == "EXTERNAL_TEST_FIXTURE"
    assert batch.captured_at == CAPTURED_AT
    assert batch.schema_version == (PHASE_8_OFFLINE_REPLAY_EVENT_MATERIALIZATION_SCHEMA_VERSION)


def test_exact_800_events_are_materialized() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.total_event_count == 800
    assert len(batch.events) == 800
    assert batch.sequence_start == 0
    assert batch.sequence_end == 799


def test_global_sequence_is_exact_and_zero_based() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert tuple(event.sequence_index for event in batch.events) == tuple(range(800))


def test_events_are_chronologically_stable() -> None:
    batch = replay_events_bullish_decision().batch_required
    ordering_keys = tuple(event.ordering_key for event in batch.events)

    assert ordering_keys == tuple(sorted(ordering_keys))


def test_every_event_matches_snapshot_lineage() -> None:
    batch = replay_events_bullish_decision().batch_required

    for event in batch.events:
        series = batch.snapshot.series[event.series_index]
        candle = series.candles[event.candle_index]

        assert event.timeframe == series.timeframe
        assert event.event_time == candle.close_time
        assert event.open_time == candle.open_time
        assert event.close_time == candle.close_time
        assert event.open_price == source_price(
            candle,
            "open_price",
            "open",
        )
        assert event.high_price == source_price(
            candle,
            "high_price",
            "high",
        )
        assert event.low_price == source_price(
            candle,
            "low_price",
            "low",
        )
        assert event.close_price == source_price(
            candle,
            "close_price",
            "close",
        )
        assert event.candle_digest == (candle.candle_digest)
        assert event.series_digest == (series.series_digest)


def test_events_have_exact_timeframe_priorities() -> None:
    batch = replay_events_bullish_decision().batch_required
    expected = {
    ...
    assert all(event.timeframe_priority == expected[event.timeframe] for event in batch.events)


def test_all_events_represent_closed_candles() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.last_event_time <= batch.captured_at
    assert all(
        event.event_time == event.close_time
        and event.close_time > event.open_time
        and event.close_time <= batch.captured_at
        for event in batch.events
    )


def test_every_event_digest_is_deterministic() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert all(
        event.event_digest == hashlib.sha256(event.canonical_payload.encode("utf-8")).hexdigest()
        for event in batch.events
    )


def test_event_digests_are_unique() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert len({event.event_digest for event in batch.events}) == 800


def test_batch_digest_is_deterministic() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.batch_digest == hashlib.sha256(batch.canonical_payload.encode("utf-8")).hexdigest()
    assert batch.digest_algorithm == "SHA-256"


def test_batch_controls_match_materialization_plan() -> None:
    batch = replay_events_bullish_decision().batch_required
    plan = batch.materialization_plan

    assert batch.event_kind == plan.event_kind
    assert batch.timestamp_source == (plan.timestamp_source)
    assert batch.replay_clock == plan.replay_clock
    assert batch.merge_mode == plan.merge_mode
    assert batch.tie_break == plan.tie_break
    assert batch.materialization_mode == (plan.materialization_mode)
    assert batch.sequence_assignment == (plan.sequence_assignment)
    assert batch.ordering_key == plan.ordering_key
    assert batch.event_fields == plan.event_fields


def test_batch_is_in_memory_materialization_only() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.is_ready is True
    assert batch.in_memory_only is True
    assert batch.creates_events is True
    assert batch.materializes_events is True
    assert batch.executes_replay is False
    assert batch.evaluates_strategy is False
    assert batch.executes_simulation is False
    assert batch.emits_orders is False
    assert batch.can_continue_to_offline_replay_session_plan is True


def test_batch_performs_no_external_io_or_execution() -> None:
    batch = replay_events_bullish_decision().batch_required

    assert batch.fetches_data is False
    assert batch.initializes_mt5 is False
    assert batch.has_adapter_instance is False
    assert batch.request_submission_authorized is False
    assert batch.adapter_invocation_authorized is False
    assert batch.storage_write_authorized is False
    assert batch.can_write_storage is False
    assert batch.can_write_network is False
    assert batch.execution_authorized is False
    assert batch.has_broker_request is False
    assert batch.can_submit_order is False
    assert batch.is_executable is False


def test_decision_performs_no_external_execution() -> None:
    decision = replay_events_bullish_decision()

    assert decision.creates_events is True
    assert decision.materializes_events is True
...
def test_batch_has_no_execution_or_io_surface(
    attribute_name,
) -> None:
    batch = replay_events_bullish_decision().batch_required

    assert not hasattr(batch, attribute_name)
...
def test_batch_id_is_deterministic() -> None:
    ...
    assert batch.batch_id == (
        f"XAUUSDm:BUY:PHASE_8_OFFLINE_REPLAY_EVENT_BATCH:BATCH_SHA256[{batch.batch_digest}]"
    )


def test_direct_event_rejects_wrong_sequence() -> None:
    event = replay_events_bullish_decision().batch_required.events[0]
...
def test_direct_event_rejects_wrong_event_time() -> None:
    event = replay_events_bullish_decision().batch_required.events[0]
...
def test_direct_event_rejects_wrong_digest() -> None:
    event = replay_events_bullish_decision().batch_required.events[0]
...
def test_direct_batch_rejects_wrong_schema() -> None:
    batch = replay_events_bullish_decision().batch_required
...
def test_direct_batch_rejects_reordered_events() -> None:
    batch = replay_events_bullish_decision().batch_required
    events = (
        batch.events[1],
        batch.events[0],
...
def test_direct_batch_rejects_missing_event() -> None:
    batch = replay_events_bullish_decision().batch_required
...
def test_direct_batch_rejects_wrong_total() -> None:
    batch = replay_events_bullish_decision().batch_required
...
def test_direct_batch_rejects_wrong_digest() -> None:
    batch = replay_events_bullish_decision().batch_required

    with pytest.raises(ValueError, match="batch_digest"):
        replace(
            batch,
            batch_digest="0" * 64,
        )
...
def test_manual_decision_rejects_missing_batch() -> None:
    decision = replay_events_bullish_decision()
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayEventMaterializationStatus.BLOCKED


def test_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayEventMaterializationFactory()
    source = materialization_bullish_decision()

    first = factory.generate(source).batch_required
    second = factory.generate(source).batch_required
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_events(materialization_bullish_decision())
...
def test_factory_alias_methods_delegate() -> None:
    ...
    assert (
        factory.build(source).batch_required.batch_digest == generated.batch_required.batch_digest
    )
    assert (
        factory.evaluate(source).batch_required.batch_digest
        == generated.batch_required.batch_digest
    )


def test_public_aliases_are_preserved() -> None:
    assert Phase8OfflineReplayEventBatch is StrategyPhase8OfflineReplayEventBatch
    assert (
        Phase8OfflineReplayEventMaterializationFactory
        is StrategyPhase8OfflineReplayEventMaterializationFactory
    )
