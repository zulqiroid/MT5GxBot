...
def test_invalid_session_state_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayTransitionContractError,
        match="INVALID_SESSION_STATE_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplayTransitionContractFactory().generate("invalid"))
...
def test_default_transition_policy_is_strict() -> None:
...
def test_transition_policy_rejects_non_boolean(
    field_name,
) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayTransitionContractPolicy(**{field_name: 1})


def test_bullish_transition_contract_is_created() -> None:
    decision = bullish_transition_contract_decision()

    assert decision.status == (Phase8OfflineReplayTransitionContractStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayTransitionContractReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_transition_contract is True


def test_bearish_transition_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayTransitionContractFactory().generate(
...
def test_existing_transition_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayTransitionContractFactory().generate(
...
def test_blocked_state_blocks_transition_contract() -> None:
    decision = StrategyPhase8OfflineReplayTransitionContractFactory().generate(
    ...
    assert decision.reason == (Phase8OfflineReplayTransitionContractReason.SESSION_STATE_BLOCKED)
    assert decision.blockers == (
        Phase8OfflineReplayTransitionContractBlocker.SESSION_STATE_BLOCKED,
    )
...
def test_transition_preserves_state_identity() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.session_state_decision is (bullish_session_state_decision())
    assert contract.session_state is (bullish_session_state_decision().state_required)


def test_transition_preserves_complete_lineage() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_preserves_identifiers() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.session_state_id == (contract.session_state.stable_id)
...
def test_transition_preserves_digests() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.session_state_digest == (contract.session_state.state_digest)
...
def test_transition_preserves_metadata() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
    ...
    assert contract.schema_version == (PHASE_8_OFFLINE_REPLAY_TRANSITION_CONTRACT_SCHEMA_VERSION)


def test_transition_timeframes_are_exact() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_controls_are_exact() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.contract_mode == (
        Phase8OfflineReplayTransitionContractMode.IMMUTABLE_SINGLE_EVENT
    )
    assert contract.action == (Phase8OfflineReplayTransitionAction.CONSUME_CURRENT_EVENT)
    assert contract.cursor_rule == (Phase8OfflineReplayTransitionCursorRule.INCREMENT_BY_ONE)
...
def test_current_transition_state_is_exact() -> None:
    ...
    assert contract.transition_index == 0
    assert contract.current_cursor_index == 0
    assert contract.current_consumed_count == 0
    assert contract.current_remaining_count == 800
...
def test_resulting_transition_state_is_exact() -> None:
    ...
    assert contract.resulting_cursor_index == 1
    assert contract.resulting_consumed_count == 1
    assert contract.resulting_remaining_count == 799
    assert contract.last_consumed_sequence_index == 0
...
def test_current_event_matches_session_cursor() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
    expected = contract.session_state.next_event

    assert contract.current_event is expected
    assert contract.current_event_sequence_index == 0
    assert contract.current_event_sequence_index == (contract.current_cursor_index)
...
def test_current_event_is_closed() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_is_contract_only() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.is_ready is True
    assert contract.contract_only is True
    assert contract.source_state_immutable is True
    assert contract.current_event_bound is True
    ...
    assert contract.can_continue_to_transition_application is True


def test_transition_performs_no_external_io() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_decision_performs_no_execution() -> None:
    decision = bullish_transition_contract_decision()
...
def test_transition_has_no_execution_or_io_surface(
    attribute_name,
) -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_contract_id_is_deterministic() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    assert contract.transition_contract_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_session_state_decision()
    decision = bullish_transition_contract_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = transition_blocked_state_decision()
    decision = StrategyPhase8OfflineReplayTransitionContractFactory().generate(source)

    assert decision.stable_id == (
        f"{source.stable_id}:"
        "PHASE_8_OFFLINE_REPLAY_TRANSITION_"
        "CONTRACT_GENERATION:"
        "BLOCKED:SESSION_STATE_BLOCKED:"
...
def test_direct_contract_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            contract,
            policy=(Phase8OfflineReplayTransitionContractPolicy(no_lookahead=False)),
        )
...
@pytest.mark.parametrize(
    ("field_name", "value"),
    [
        ("session_state_id", "foreign-state"),
        (
            "session_contract_id",
...
def test_direct_contract_rejects_wrong_current_cursor() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="current_cursor_index",
    ):
        replace(
            contract,
            current_cursor_index=1,
            current_event_sequence_index=1,
            resulting_cursor_index=2,
        )


def test_direct_contract_rejects_consumed_source_state() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    with pytest.raises(
        ValueError,
        match="current_consumed_count",
    ):
        replace(
            contract,
            current_consumed_count=1,
            current_remaining_count=799,
            resulting_consumed_count=2,
            resulting_remaining_count=798,
        )


def test_direct_contract_rejects_wrong_current_remaining() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_direct_contract_rejects_wrong_event_sequence() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_direct_contract_rejects_wrong_event_id() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_direct_contract_rejects_wrong_event_digest() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_direct_contract_rejects_wrong_event_time() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    with pytest.raises(
        ValueError,
        match="current_event_time",
    ):
        replace(
            contract,
            current_event_time=(contract.captured_at),
        )


def test_direct_contract_rejects_wrong_event_timeframe() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    replacement_timeframe = (
        Phase8Timeframe.M5
        if contract.current_event_timeframe != Phase8Timeframe.M5
        else Phase8Timeframe.H4
    )

    with pytest.raises(
        ValueError,
        match="current_event_timeframe",
    ):
        replace(
            contract,
            current_event_timeframe=(replacement_timeframe),
        )
...
def test_direct_contract_rejects_false_completion_mismatch() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required
...
def test_transition_policy_is_immutable() -> None:
    policy = Phase8OfflineReplayTransitionContractPolicy()
...
def test_transition_contract_is_immutable() -> None:
    contract = bullish_transition_contract_decision().transition_contract_required

    with pytest.raises(FrozenInstanceError):
        contract.resulting_cursor_index = 2
...
def test_transition_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayTransitionContractStatus.BLOCKED


def test_transition_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayTransitionContractFactory()
    source = bullish_session_state_decision()
...
def test_transition_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_transition_contract(bullish_session_state_decision())

    assert decision.is_created is True


def test_transition_factory_aliases_delegate() -> None:
    factory = StrategyPhase8OfflineReplayTransitionContractFactory()
    source = bullish_session_state_decision()
...
def test_transition_public_aliases_are_preserved() -> None:
    assert Phase8OfflineReplayTransitionContract is StrategyPhase8OfflineReplayTransitionContract
    assert (
        Phase8OfflineReplayTransitionContractFactory
        is StrategyPhase8OfflineReplayTransitionContractFactory
    )
