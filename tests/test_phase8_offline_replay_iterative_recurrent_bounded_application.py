...
from tests.test_phase8_offline_replay_iterative_recurrent_bounded_plan import (
    bullish_iterative_recurrent_bounded_plan_decision,
)
...
class FakeNearTerminalState:
    ...
    state_id: str = "ITERATIVE_RECURRENT_NEAR_TERMINAL_STATE"
...
def bullish_iterative_recurrent_bounded_application_decision():
    return apply_phase8_offline_replay_iterative_recurrent_bounded_plan(
        bullish_iterative_recurrent_bounded_plan_decision()
    )


def test_schema_version_is_stable() -> None:
    assert PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_APPLICATION_SCHEMA_VERSION == "1.0"


def test_bullish_iterative_recurrent_application_is_created() -> None:
    decision = bullish_iterative_recurrent_bounded_application_decision()
...
def test_application_preserves_lineage_identity() -> None:
    plan_decision = bullish_iterative_recurrent_bounded_plan_decision()
    plan = plan_decision.plan_required

    receipt = apply_phase8_offline_replay_iterative_recurrent_bounded_plan(
        plan_decision
    ).receipt_required
...
def test_default_application_counters_are_exact() -> None:
    receipt = bullish_iterative_recurrent_bounded_application_decision().receipt_required

    assert receipt.source_state_version == 9
    assert receipt.start_cursor_index == 102
    assert receipt.resulting_cursor_index == 134
    assert receipt.initial_consumed_count == 102
    assert receipt.resulting_consumed_count == 134
    assert receipt.initial_remaining_count == 698
    assert receipt.resulting_remaining_count == 666
    assert receipt.consumed_event_count == 32
    assert receipt.next_event_sequence_index == 134
...
def test_default_consumed_sequence_range_is_exact() -> None:
    ...
    assert receipt.consumed_event_sequence_indices == tuple(range(102, 134))
...
def test_application_id_is_deterministic() -> None:
    first = bullish_iterative_recurrent_bounded_application_decision().receipt_required
    second = bullish_iterative_recurrent_bounded_application_decision().receipt_required
...
def test_blocked_plan_blocks_application() -> None:
    decision = apply_phase8_offline_replay_iterative_recurrent_bounded_plan(
...
def test_missing_plan_blocks_application() -> None:
    decision = apply_phase8_offline_replay_iterative_recurrent_bounded_plan(None)
...
def test_terminal_application_is_exact() -> None:
    ...
    assert receipt.source_state_version == 299
...
def test_source_state_is_not_mutated() -> None:
    ...
    _ = apply_phase8_offline_replay_iterative_recurrent_bounded_plan(plan_decision).receipt_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = StrategyPhase8OfflineReplayIterativeRecurrentBoundedApplication().apply(
        plan_decision
    )
    function_decision = apply_phase8_offline_replay_iterative_recurrent_bounded_plan(plan_decision)
