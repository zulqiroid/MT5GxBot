...
from tests.test_phase8_offline_replay_subsequent_iterative_recurrent_bounded_progressed_state import (
    bullish_subsequent_iterative_recurrent_bounded_progressed_state_decision,
)
...
class FakeTerminalState:
    ...
    state_id: str = "SUCCESSIVE_ITERATIVE_RECURRENT_TERMINAL_SOURCE"
...
def bullish_successive_iterative_recurrent_bounded_plan_decision():
    return generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        bullish_subsequent_iterative_recurrent_bounded_progressed_state_decision()
    )
...
def test_schema_and_default_limit_are_stable() -> None:
    assert (
        PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_PLAN_SCHEMA_VERSION == "1.0"
    )
    assert PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT == 32


def test_bullish_successive_iterative_recurrent_plan_is_created() -> None:
    decision = bullish_successive_iterative_recurrent_bounded_plan_decision()
...
def test_plan_preserves_source_identity() -> None:
    source_decision = bullish_subsequent_iterative_recurrent_bounded_progressed_state_decision()
    source_state = source_decision.state_required

    plan = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        source_decision
    ).plan_required

    assert plan.source_decision is source_decision
    assert plan.source_state is source_state
    assert plan.source_state_version == 11


def test_default_plan_counters_are_exact() -> None:
    plan = bullish_successive_iterative_recurrent_bounded_plan_decision().plan_required

    assert plan.start_cursor_index == 166
    assert plan.resulting_cursor_index == 198
    assert plan.initial_consumed_count == 166
    assert plan.resulting_consumed_count == 198
    assert plan.initial_remaining_count == 634
    assert plan.resulting_remaining_count == 602
    assert plan.planned_transition_count == 32
    assert plan.next_event_sequence_index == 198
...
def test_default_sequence_range_is_exact() -> None:
    ...
    assert plan.planned_event_sequence_indices == tuple(range(166, 198))
...
def test_plan_id_is_deterministic() -> None:
    first = bullish_successive_iterative_recurrent_bounded_plan_decision().plan_required
    second = bullish_successive_iterative_recurrent_bounded_plan_decision().plan_required
...
def test_custom_limit_is_respected() -> None:
    ...
    assert plan.start_cursor_index == 166
    assert plan.resulting_cursor_index == 171
    assert plan.resulting_consumed_count == 171
    assert plan.resulting_remaining_count == 629
    assert plan.planned_event_sequence_indices == tuple(range(166, 171))
    assert plan.next_event_sequence_index == 171


def test_blocked_source_blocks_plan() -> None:
    decision = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
...
def test_missing_source_blocks_plan() -> None:
    decision = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(None)
...
def test_source_state_is_not_mutated() -> None:
    ...
    _ = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        source_decision
    ).plan_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = (
        StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanner().generate(
            source_decision
        )
    )
    function_decision = generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        source_decision
    )
