...
def test_invalid_plan_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayEventContractError,
        match="INVALID_REPLAY_PLAN_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplayEventContractFactory().generate("invalid"))
...
def test_default_event_contract_policy_is_strict() -> None:
    policy = Phase8OfflineReplayEventContractPolicy()

    assert policy.is_strict is True
    assert policy.strict_sequence is True
    assert policy.zero_based_sequence is True
    assert policy.event_time_from_candle_close is True
    assert policy.include_full_ohlc is True
    assert policy.preserve_candle_digest is True
    assert policy.preserve_series_digest is True
...
def test_event_contract_policy_rejects_non_boolean(
    field_name,
) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayEventContractPolicy(**{field_name: 1})


def test_bullish_event_contract_is_created() -> None:
    decision = event_contract_bullish_decision()

    assert decision.status == (Phase8OfflineReplayEventContractStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayEventContractReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_event_contract is True


def test_bearish_event_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayEventContractFactory().generate(
        event_contract_bearish_plan_decision()
    )

    assert decision.is_created is True
    assert decision.direction == (DirectionalPermissionDirection.BEARISH)
    assert decision.event_contract_required.side == StrategyOrderSide.SELL


def test_existing_event_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayEventContractFactory().generate(
...
def test_blocked_plan_blocks_event_contract() -> None:
    decision = StrategyPhase8OfflineReplayEventContractFactory().generate(
        event_contract_blocked_plan_decision()
    )

    assert decision.is_blocked is True
    assert decision.event_contract is None
    assert decision.reason == (Phase8OfflineReplayEventContractReason.REPLAY_PLAN_BLOCKED)
    assert decision.blockers == (Phase8OfflineReplayEventContractBlocker.REPLAY_PLAN_BLOCKED,)


def test_event_contract_required_rejects_blocked() -> None:
    decision = StrategyPhase8OfflineReplayEventContractFactory().generate(
        event_contract_blocked_plan_decision()
    )
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 offline replay-event contract",
    ):
        _ = decision.event_contract_required


def test_event_contract_preserves_plan_identity() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.plan_decision is (bullish_replay_plan_decision())
    assert event_contract.plan is (bullish_replay_plan_decision().plan_required)


def test_event_contract_preserves_complete_lineage() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required
    plan = event_contract.plan

    assert event_contract.specification is (plan.specification)
    assert event_contract.input_package is (plan.input_package)
    assert event_contract.verification_receipt is (plan.verification_receipt)
    assert event_contract.snapshot is plan.snapshot
    assert event_contract.contract is plan.contract
    assert event_contract.dry_run_package is (plan.dry_run_package)


def test_event_contract_preserves_identifiers() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.plan_id == (event_contract.plan.stable_id)
    assert event_contract.specification_id == (event_contract.specification.stable_id)
    assert event_contract.input_package_id == (event_contract.input_package.stable_id)
    assert event_contract.snapshot_id == (event_contract.snapshot.stable_id)


def test_event_contract_preserves_digests() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.plan_digest == (event_contract.plan.plan_digest)
    assert event_contract.specification_digest == (
        event_contract.specification.specification_digest
    )
    assert event_contract.input_digest == (event_contract.input_package.input_digest)
    assert event_contract.snapshot_digest == (event_contract.snapshot.snapshot_digest)


def test_event_contract_preserves_metadata() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.broker_symbol == "XAUUSDm"
    assert event_contract.direction == (DirectionalPermissionDirection.BULLISH)
    assert event_contract.side == StrategyOrderSide.BUY
    assert event_contract.source_name == ("EXTERNAL_TEST_FIXTURE")
    assert event_contract.captured_at == CAPTURED_AT
    assert event_contract.schema_version == (PHASE_8_OFFLINE_REPLAY_EVENT_CONTRACT_SCHEMA_VERSION)


def test_event_contract_has_exact_timeframes() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.timeframes == (
        Phase8Timeframe.H4,
        Phase8Timeframe.H1,
        Phase8Timeframe.M15,
        Phase8Timeframe.M5,
    )
    assert event_contract.series_count == 4


def test_event_contract_priorities_are_exact() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.timeframe_priorities == (
...
def test_event_contract_counts_are_exact() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.series_counts == (
        (Phase8Timeframe.H4, 200),
        (Phase8Timeframe.H1, 200),
        (Phase8Timeframe.M15, 200),
        (Phase8Timeframe.M5, 200),
    )
    assert event_contract.total_event_count == 800
    assert event_contract.planned_event_count == 800


def test_event_contract_kind_and_timestamp_are_exact() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.event_kind == (Phase8OfflineReplayEventKind.CANDLE_CLOSED)
    assert event_contract.timestamp_source == (
        Phase8OfflineReplayEventTimestampSource.CANDLE_CLOSE_TIME
    )


def test_event_contract_replay_controls_match_plan() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.replay_clock == (Phase8OfflineReplayClock.CANDLE_CLOSE)
    assert event_contract.merge_mode == (Phase8OfflineReplayMergeMode.CHRONOLOGICAL_STABLE)
    assert event_contract.tie_break == (Phase8OfflineReplayTieBreak.TIMEFRAME_PRIORITY)


def test_event_fields_are_exact_and_ordered() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.event_fields == (
    ...
    assert event_contract.event_field_count == 14


def test_event_fields_are_unique() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert len(set(event_contract.event_fields)) == (event_contract.event_field_count)


def test_event_contract_digest_is_deterministic() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert (
        event_contract.contract_digest
        == hashlib.sha256(event_contract.canonical_payload.encode("utf-8")).hexdigest()
    )
    assert event_contract.digest_algorithm == "SHA-256"


def test_event_contract_is_schema_only() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.is_ready is True
    assert event_contract.candle_close_events_only is True
    assert event_contract.zero_based_sequence is True
    assert event_contract.no_lookahead is True
    assert event_contract.creates_events is False
    assert event_contract.materializes_events is False
    assert event_contract.executes_replay is False
    assert event_contract.executes_simulation is False
    assert event_contract.emits_orders is False
    assert event_contract.can_continue_to_event_materialization_plan is True


def test_event_contract_performs_no_io_or_execution() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.fetches_data is False
    assert event_contract.initializes_mt5 is False
    assert event_contract.has_adapter_instance is False
    assert event_contract.request_submission_authorized is False
    assert event_contract.adapter_invocation_authorized is False
    assert event_contract.storage_write_authorized is False
    assert event_contract.can_write_storage is False
    assert event_contract.can_write_network is False
    assert event_contract.execution_authorized is False
    assert event_contract.has_broker_request is False
    assert event_contract.can_submit_order is False
    assert event_contract.is_executable is False


def test_event_contract_decision_performs_no_io() -> None:
    decision = event_contract_bullish_decision()
...
def test_event_contract_has_no_implementation_surface(
    attribute_name,
) -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert not hasattr(event_contract, attribute_name)


def test_event_contract_id_is_deterministic() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    assert event_contract.event_contract_id == (
        "XAUUSDm:BUY:"
        "PHASE_8_OFFLINE_REPLAY_EVENT_CONTRACT:"
        "CONTRACT_SHA256["
        f"{event_contract.contract_digest}]"
    )


def test_created_decision_stable_id_is_deterministic() -> None:
    plan_decision = bullish_replay_plan_decision()
    decision = event_contract_bullish_decision()

    assert decision.stable_id == (
        f"{plan_decision.stable_id}:"
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    plan_decision = event_contract_blocked_plan_decision()
    decision = StrategyPhase8OfflineReplayEventContractFactory().generate(plan_decision)

    assert decision.stable_id == (
        f"{plan_decision.stable_id}:"
        "PHASE_8_OFFLINE_REPLAY_EVENT_CONTRACT_GENERATION:"
        "BLOCKED:REPLAY_PLAN_BLOCKED:"
        "REPLAY_PLAN_BLOCKED"
    )
...
def test_direct_contract_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            event_contract,
            policy=Phase8OfflineReplayEventContractPolicy(no_lookahead=False),
        )
...
def test_direct_contract_rejects_reordered_timeframes() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            event_contract,
            timeframes=tuple(reversed(event_contract.timeframes)),
        )


def test_direct_contract_rejects_wrong_priorities() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required
    priorities = (
        (Phase8Timeframe.H4, 9),
        *event_contract.timeframe_priorities[1:],
    )

    with pytest.raises(ValueError, match="priorities"):
        replace(
            event_contract,
            timeframe_priorities=priorities,
        )


def test_direct_contract_rejects_wrong_counts() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required
    counts = (
        (Phase8Timeframe.H4, 199),
        *event_contract.series_counts[1:],
    )
...
def test_direct_contract_rejects_reordered_fields() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    with pytest.raises(ValueError, match="schema"):
        replace(
            event_contract,
            event_fields=tuple(reversed(event_contract.event_fields)),
        )


def test_direct_contract_rejects_duplicate_fields() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required
    fields = (
        event_contract.event_fields[0],
        event_contract.event_fields[0],
        *event_contract.event_fields[2:],
    )
...
def test_manual_decision_rejects_duplicate_blockers() -> None:
    ...
    with pytest.raises(ValueError, match="duplicates"):
        replace(
            decision,
            blockers=(
                Phase8OfflineReplayEventContractBlocker.REPLAY_PLAN_BLOCKED,
                Phase8OfflineReplayEventContractBlocker.REPLAY_PLAN_BLOCKED,
            ),
        )


def test_event_contract_policy_is_immutable() -> None:
    policy = Phase8OfflineReplayEventContractPolicy()
...
def test_event_contract_is_immutable() -> None:
    event_contract = event_contract_bullish_decision().event_contract_required

    with pytest.raises(FrozenInstanceError):
        event_contract.contract_digest = "0" * 64


def test_event_contract_decision_is_immutable() -> None:
    decision = event_contract_bullish_decision()

    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayEventContractStatus.BLOCKED


def test_event_contract_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayEventContractFactory()
    source = bullish_replay_plan_decision()

    first = factory.generate(source).event_contract_required
    second = factory.generate(source).event_contract_required

    assert first.contract_digest == second.contract_digest
    assert first.canonical_payload == second.canonical_payload
    assert first.event_fields == second.event_fields


def test_event_contract_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_event_contract(bullish_replay_plan_decision())

    assert decision.is_created is True


def test_event_contract_factory_aliases_delegate() -> None:
    factory = StrategyPhase8OfflineReplayEventContractFactory()
    source = bullish_replay_plan_decision()
    generated = factory.generate(source)

    assert factory.build(source).event_contract_required.contract_digest == (
        generated.event_contract_required.contract_digest
    )
    assert factory.evaluate(source).event_contract_required.contract_digest == (
        generated.event_contract_required.contract_digest
    )


def test_event_contract_public_aliases_are_preserved() -> None:
    assert Phase8OfflineReplayEventContract is StrategyPhase8OfflineReplayEventContract
    assert (
        Phase8OfflineReplayEventContractFactory is StrategyPhase8OfflineReplayEventContractFactory
    )
