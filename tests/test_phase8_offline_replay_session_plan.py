...
def test_invalid_event_materialization_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplaySessionPlanError,
        match="INVALID_EVENT_MATERIALIZATION_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplaySessionPlanFactory().generate("invalid"))
...
def test_default_session_policy_is_strict() -> None:
...
def test_session_policy_rejects_non_boolean(
    field_name,
) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplaySessionPolicy(**{field_name: 1})


def test_bullish_session_plan_is_created() -> None:
    decision = bullish_session_plan_decision()

    assert decision.status == (Phase8OfflineReplaySessionPlanStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplaySessionPlanReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_plan is True


def test_bearish_session_plan_is_created() -> None:
    decision = StrategyPhase8OfflineReplaySessionPlanFactory().generate(
...
def test_existing_session_plan_is_created() -> None:
    decision = StrategyPhase8OfflineReplaySessionPlanFactory().generate(
...
def test_blocked_batch_blocks_session_plan() -> None:
    decision = StrategyPhase8OfflineReplaySessionPlanFactory().generate(
    ...
    assert decision.reason == (Phase8OfflineReplaySessionPlanReason.EVENT_BATCH_BLOCKED)
    assert decision.blockers == (Phase8OfflineReplaySessionPlanBlocker.EVENT_BATCH_BLOCKED,)
...
def test_plan_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 offline replay-session plan",
    ):
        _ = decision.plan_required


def test_session_plan_preserves_batch_identity() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.event_materialization_decision is (replay_events_bullish_decision())
    assert plan.event_batch is (replay_events_bullish_decision().batch_required)


def test_session_plan_preserves_complete_lineage() -> None:
    plan = bullish_session_plan_decision().plan_required
    batch = plan.event_batch

    assert plan.materialization_plan is (batch.materialization_plan)
    assert plan.event_contract is batch.event_contract
    assert plan.replay_plan is batch.replay_plan
    assert plan.specification is batch.specification
    assert plan.input_package is batch.input_package
    assert plan.verification_receipt is (batch.verification_receipt)
    assert plan.snapshot is batch.snapshot
    assert plan.contract is batch.contract
    assert plan.dry_run_package is (batch.dry_run_package)


def test_session_plan_preserves_identifiers() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.event_batch_id == plan.event_batch.stable_id
    assert plan.materialization_plan_id == (plan.materialization_plan.stable_id)
...
def test_session_plan_preserves_digests() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.event_batch_digest == (plan.event_batch.batch_digest)
    assert plan.materialization_plan_digest == (plan.materialization_plan.materialization_digest)
...
def test_session_plan_preserves_metadata() -> None:
    plan = bullish_session_plan_decision().plan_required
    ...
    assert plan.schema_version == (PHASE_8_OFFLINE_REPLAY_SESSION_PLAN_SCHEMA_VERSION)


def test_session_plan_timeframes_are_exact() -> None:
    plan = bullish_session_plan_decision().plan_required
...
def test_session_window_is_exact() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.sequence_start == 0
    assert plan.sequence_end == 799
    assert plan.initial_cursor_index == 0
    assert plan.total_event_count == 800
    assert plan.starts_at_first_event is True


def test_session_time_bounds_match_batch() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.first_event_time == (plan.event_batch.first_event_time)
    assert plan.last_event_time == (plan.event_batch.last_event_time)
    assert plan.last_event_time <= plan.captured_at


def test_session_controls_are_exact() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.session_mode == (Phase8OfflineReplaySessionMode.IMMUTABLE_IN_MEMORY_REPLAY)
    assert plan.cursor_mode == (Phase8OfflineReplayCursorMode.ZERO_BASED_FORWARD_ONLY)
    assert plan.transition_mode == (Phase8OfflineReplayTransitionMode.ONE_EVENT_PER_TRANSITION)
    assert plan.initial_state_mode == (Phase8OfflineReplayInitialStateMode.FRESH_STRATEGY_STATE)


def test_session_digest_is_deterministic() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.session_digest == hashlib.sha256(plan.canonical_payload.encode("utf-8")).hexdigest()
    assert plan.digest_algorithm == "SHA-256"


def test_session_plan_is_plan_only() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.is_ready is True
    assert plan.plans_replay_session is True
    assert plan.in_memory_only is True
    assert plan.forward_only is True
    assert plan.no_lookahead is True
    assert plan.starts_replay is False
    assert plan.consumes_events is False
    assert plan.executes_replay is False
    assert plan.evaluates_strategy is False
    assert plan.executes_simulation is False
    assert plan.emits_orders is False
    assert plan.can_continue_to_offline_replay_session_contract is True


def test_session_plan_performs_no_external_io() -> None:
    plan = bullish_session_plan_decision().plan_required
...
def test_session_decision_performs_no_execution() -> None:
    decision = bullish_session_plan_decision()

    assert decision.plans_replay_session is True
...
def test_session_plan_has_no_execution_surface(
    attribute_name,
) -> None:
    plan = bullish_session_plan_decision().plan_required
...
def test_session_plan_id_is_deterministic() -> None:
    plan = bullish_session_plan_decision().plan_required

    assert plan.session_plan_id == (
        f"XAUUSDm:BUY:PHASE_8_OFFLINE_REPLAY_SESSION_PLAN:SESSION_SHA256[{plan.session_digest}]"
    )


def test_created_decision_stable_id_is_deterministic() -> None:
    source = replay_events_bullish_decision()
    decision = bullish_session_plan_decision()

    assert decision.stable_id == (
        f"{source.stable_id}:PHASE_8_OFFLINE_REPLAY_SESSION_PLAN_GENERATION:CREATED:CREATED:NONE"
    )


def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = session_blocked_event_decision()
    decision = StrategyPhase8OfflineReplaySessionPlanFactory().generate(source)

    assert decision.stable_id == (
        f"{source.stable_id}:"
        "PHASE_8_OFFLINE_REPLAY_SESSION_PLAN_GENERATION:"
        "BLOCKED:EVENT_BATCH_BLOCKED:"
        "EVENT_BATCH_BLOCKED"
    )
...
def test_direct_plan_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            plan,
            policy=Phase8OfflineReplaySessionPolicy(no_lookahead=False),
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "session_mode",
            "IMMUTABLE_IN_MEMORY_REPLAY",
            "Phase8OfflineReplaySessionMode",
        ),
        (
            "cursor_mode",
            "ZERO_BASED_FORWARD_ONLY",
            "Phase8OfflineReplayCursorMode",
        ),
        (
            "transition_mode",
...
def test_direct_plan_rejects_reordered_timeframes() -> None:
    plan = bullish_session_plan_decision().plan_required
    ...
    with pytest.raises(
        ...
        replace(
            plan,
            timeframes=tuple(reversed(plan.timeframes)),
        )
...
def test_direct_plan_rejects_wrong_initial_cursor() -> None:
    plan = bullish_session_plan_decision().plan_required
...
def test_direct_plan_rejects_wrong_first_time() -> None:
    plan = bullish_session_plan_decision().plan_required
    ...
    with pytest.raises(
        ...
        replace(
            plan,
            first_event_time=plan.last_event_time,
        )


def test_direct_plan_rejects_wrong_last_time() -> None:
    plan = bullish_session_plan_decision().plan_required
    ...
    with pytest.raises(
        ...
        replace(
            plan,
            last_event_time=plan.first_event_time,
        )
...
def test_manual_decision_rejects_duplicate_blockers() -> None:
    ...
    with pytest.raises(ValueError, match="duplicates"):
        replace(
            decision,
            blockers=(
                Phase8OfflineReplaySessionPlanBlocker.EVENT_BATCH_BLOCKED,
                Phase8OfflineReplaySessionPlanBlocker.EVENT_BATCH_BLOCKED,
            ),
        )


def test_session_policy_is_immutable() -> None:
    policy = Phase8OfflineReplaySessionPolicy()
...
def test_session_plan_is_immutable() -> None:
    plan = bullish_session_plan_decision().plan_required

    with pytest.raises(FrozenInstanceError):
        plan.session_digest = "0" * 64


def test_session_decision_is_immutable() -> None:
    decision = bullish_session_plan_decision()

    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplaySessionPlanStatus.BLOCKED


def test_session_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplaySessionPlanFactory()
    source = replay_events_bullish_decision()
...
def test_session_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_session_plan(replay_events_bullish_decision())

    assert decision.is_created is True


def test_session_factory_aliases_delegate() -> None:
    factory = StrategyPhase8OfflineReplaySessionPlanFactory()
    source = replay_events_bullish_decision()
    generated = factory.generate(source)

    assert (
        factory.build(source).plan_required.session_digest == generated.plan_required.session_digest
    )
    assert (
        factory.evaluate(source).plan_required.session_digest
        == generated.plan_required.session_digest
    )


def test_session_public_aliases_are_preserved() -> None:
    assert Phase8OfflineReplaySessionPlan is StrategyPhase8OfflineReplaySessionPlan
    assert Phase8OfflineReplaySessionPlanFactory is StrategyPhase8OfflineReplaySessionPlanFactory
