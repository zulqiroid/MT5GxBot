...
def test_invalid_verification_is_fail_safe() -> None:
    with pytest.raises(
        ...
        (StrategyPhase8SimulationInputPackageFactory().generate("invalid"))
...
def test_bullish_input_package_is_created() -> None:
    decision = bullish_input_decision()

    assert decision.status == (Phase8SimulationInputPackageStatus.CREATED)
    assert decision.reason == (Phase8SimulationInputPackageReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_package is True


def test_bearish_input_package_is_created() -> None:
    decision = StrategyPhase8SimulationInputPackageFactory().generate(
        bearish_verification_for_input()
    )
...
def test_existing_input_package_is_created() -> None:
    decision = StrategyPhase8SimulationInputPackageFactory().generate(
        existing_verification_for_input()
    )

    assert decision.is_created is True


def test_blocked_verification_blocks_package() -> None:
    decision = StrategyPhase8SimulationInputPackageFactory().generate(
        blocked_verification_for_input()
    )

    assert decision.is_blocked is True
    assert decision.package is None
    assert decision.reason == (Phase8SimulationInputPackageReason.SNAPSHOT_VERIFICATION_BLOCKED)
    assert decision.blockers == (Phase8SimulationInputPackageBlocker.SNAPSHOT_VERIFICATION_BLOCKED,)
...
def test_package_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 simulation-input package",
    ):
        _ = decision.package_required


def test_package_preserves_verification_identity() -> None:
    package = bullish_input_decision().package_required

    assert package.verification_decision is (bullish_verification_decision())
    assert package.verification_receipt is (bullish_verification_decision().receipt_required)


def test_package_preserves_complete_lineage() -> None:
    package = bullish_input_decision().package_required
    receipt = package.verification_receipt
    snapshot = receipt.snapshot
    contract = snapshot.contract
    dry_run_package = contract.package

    assert package.snapshot is snapshot
    assert package.contract is contract
    assert package.dry_run_package is dry_run_package
    assert package.verification_receipt_id == (receipt.stable_id)
    assert package.snapshot_id == snapshot.stable_id
    assert package.contract_id == contract.stable_id
    assert package.dry_run_package_id == (dry_run_package.stable_id)


def test_package_preserves_all_digests() -> None:
    package = bullish_input_decision().package_required

    assert package.verification_receipt_digest == (package.verification_receipt.receipt_digest)
    assert package.snapshot_digest == (package.snapshot.snapshot_digest)
    assert package.contract_digest == (package.contract.contract_digest)
    assert package.dry_run_package_digest == (package.dry_run_package.package_digest)
...
def test_package_preserves_metadata() -> None:
    ...
    assert package.source_name == ("EXTERNAL_TEST_FIXTURE")
    assert package.captured_at == CAPTURED_AT
    assert package.schema_version == (PHASE_8_SIMULATION_INPUT_PACKAGE_SCHEMA_VERSION)


def test_package_timeframes_are_exact() -> None:
    package = bullish_input_decision().package_required

    assert package.timeframes == (
        Phase8Timeframe.H4,
        Phase8Timeframe.H1,
        Phase8Timeframe.M15,
        Phase8Timeframe.M5,
    )
    assert package.series_count == 4


def test_series_digest_lineage_is_preserved() -> None:
    package = bullish_input_decision().package_required

    assert package.series_digests == (package.verification_receipt.series_digests)


def test_series_count_lineage_is_preserved() -> None:
    package = bullish_input_decision().package_required

    assert package.series_counts == (package.verification_receipt.series_counts)
    assert package.total_candle_count == 800
...
def test_input_digest_is_deterministic() -> None:
    ...
    assert (
        package.input_digest
        == hashlib.sha256(package.canonical_payload.encode("utf-8")).hexdigest()
    )
    assert package.digest_algorithm == "SHA-256"


def test_package_is_gold_only() -> None:
    package = bullish_input_decision().package_required

    assert package.broker_symbol.upper().startswith("XAUUSD")


def test_package_is_verified_simulation_input() -> None:
    package = bullish_input_decision().package_required

    assert package.is_ready is True
    assert package.is_verified_input is True
    assert package.simulation_only is True
    assert package.uses_closed_candles_only is True
    assert package.can_continue_to_offline_simulation_design is True


def test_package_performs_no_io_or_execution() -> None:
    package = bullish_input_decision().package_required

    assert package.fetches_data is False
    assert package.initializes_mt5 is False
    assert package.has_adapter_instance is False
    assert package.request_submission_authorized is False
    assert package.adapter_invocation_authorized is False
    assert package.storage_write_authorized is False
    assert package.can_write_storage is False
    assert package.can_write_network is False
...
def test_input_package_id_is_deterministic() -> None:
    package = bullish_input_decision().package_required

    assert package.input_package_id == (
        f"XAUUSDm:BUY:PHASE_8_SIMULATION_INPUT_PACKAGE:INPUT_SHA256[{package.input_digest}]"
    )


def test_created_decision_stable_id_is_deterministic() -> None:
    verification = bullish_verification_decision()
    decision = bullish_input_decision()

    assert decision.stable_id == (
        f"{verification.stable_id}:PHASE_8_SIMULATION_INPUT_PACKAGE_GENERATION:CREATED:CREATED:NONE"
    )


def test_blocked_decision_stable_id_is_deterministic() -> None:
    verification = blocked_verification_for_input()
    decision = StrategyPhase8SimulationInputPackageFactory().generate(verification)

    assert decision.stable_id == (
        f"{verification.stable_id}:"
        "PHASE_8_SIMULATION_INPUT_PACKAGE_GENERATION:"
        "BLOCKED:SNAPSHOT_VERIFICATION_BLOCKED:"
        "SNAPSHOT_VERIFICATION_BLOCKED"
    )


def test_direct_package_rejects_wrong_schema() -> None:
    package = bullish_input_decision().package_required
...
def test_direct_package_rejects_foreign_digests(
    field_name,
) -> None:
    package = bullish_input_decision().package_required
...
def test_direct_package_rejects_reordered_timeframes() -> None:
    package = bullish_input_decision().package_required
    ...
    with pytest.raises(
        ...
        replace(
            package,
            timeframes=tuple(reversed(package.timeframes)),
        )


def test_direct_package_rejects_wrong_series_digests() -> None:
    package = bullish_input_decision().package_required
    series_digests = (
        (
            package.series_digests[0][0],
            "0" * 64,
        ),
        *package.series_digests[1:],
    )
...
def test_direct_package_rejects_wrong_series_counts() -> None:
    package = bullish_input_decision().package_required
    series_counts = (
        (
            package.series_counts[0][0],
            199,
        ),
        *package.series_counts[1:],
    )
...
def test_direct_package_rejects_wrong_total_count() -> None:
    package = bullish_input_decision().package_required
...
def test_direct_package_rejects_wrong_input_digest() -> None:
    package = bullish_input_decision().package_required
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8SimulationInputPackageStatus.BLOCKED


def test_generation_is_deterministic() -> None:
    factory = StrategyPhase8SimulationInputPackageFactory()
    source = bullish_verification_decision()

    first = factory.generate(source).package_required
    second = factory.generate(source).package_required
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_simulation_input_package(bullish_verification_decision())
...
def test_factory_alias_methods_delegate() -> None:
    ...
    assert (
        factory.build(source).package_required.input_digest
        == generated.package_required.input_digest
    )
    assert (
        factory.evaluate(source).package_required.input_digest
        == generated.package_required.input_digest
    )


def test_public_aliases_are_preserved() -> None:
    assert Phase8SimulationInputPackage is StrategyPhase8SimulationInputPackage
    assert Phase8SimulationInputPackageFactory is StrategyPhase8SimulationInputPackageFactory
