...
def test_invalid_audit_export_is_fail_safe() -> None:
    with pytest.raises(
        PlanningAuditVerificationError,
        match="INVALID_AUDIT_EXPORT_DECISION",
    ) as captured:
        (StrategyPlanningAuditVerificationFactory().verify("invalid"))
...
def test_bullish_export_is_verified() -> None:
    decision = StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export())

    assert decision.status == (PlanningAuditVerificationStatus.VERIFIED)
    assert decision.reason == (PlanningAuditVerificationReason.VERIFIED)
    assert decision.blockers == ()
    assert decision.is_verified is True
    assert decision.has_receipt is True


def test_bearish_export_is_verified() -> None:
    decision = StrategyPlanningAuditVerificationFactory().verify(bearish_audit_export())
...
def test_blocked_export_produces_no_receipt() -> None:
    decision = StrategyPlanningAuditVerificationFactory().verify(blocked_audit_export())

    assert decision.is_blocked is True
    assert decision.receipt is None
    assert decision.has_receipt is False
    assert decision.reason == (PlanningAuditVerificationReason.AUDIT_EXPORT_BLOCKED)
    assert decision.blockers == (PlanningAuditVerificationBlocker.AUDIT_EXPORT_BLOCKED,)
...
def test_receipt_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No planning-audit verification receipt",
    ):
        _ = decision.receipt_required


def test_receipt_preserves_export_decision() -> None:
    audit_export = bullish_audit_export()
    receipt = StrategyPlanningAuditVerificationFactory().verify(audit_export).receipt_required

    assert receipt.audit_export is audit_export
    assert receipt.envelope is audit_export.envelope_required


def test_receipt_preserves_audit_lineage() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    assert receipt.record is receipt.envelope.record
    assert receipt.manifest is receipt.envelope.manifest
    assert receipt.package is receipt.manifest.package
...
def test_receipt_preserves_metadata() -> None:
    ...
    assert receipt.schema_version == (PLANNING_AUDIT_VERIFICATION_SCHEMA_VERSION)
...
def test_all_required_checks_are_present() -> None:
    ...
    assert receipt.verification_count == 6


def test_content_matches_canonical_record() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    assert receipt.envelope.content == receipt.record.canonical_json


def test_verified_length_matches_independent_measurement() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    expected = len(receipt.envelope.content.encode("utf-8"))

    assert receipt.verified_content_length_bytes == expected
    assert receipt.verified_content_length_bytes == receipt.envelope.content_length_bytes


def test_verified_digest_matches_independent_hash() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    expected = hashlib.sha256(receipt.envelope.content.encode("utf-8")).hexdigest()

    assert receipt.verified_content_digest == expected
    assert receipt.digest_algorithm == "SHA-256"


def test_verified_digest_matches_export_and_record() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    assert receipt.verified_content_digest == receipt.envelope.content_digest
    assert receipt.verified_content_digest == receipt.record.record_digest


def test_verified_manifest_digest_matches_lineage() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    assert receipt.verified_manifest_digest == receipt.envelope.manifest_digest
    assert receipt.verified_manifest_digest == receipt.manifest.digest


def test_package_execution_boundary_remains_locked() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )

    assert receipt.package.is_locked is True
    assert receipt.package.execution_authorized is False
    assert receipt.package.has_broker_request is False
    assert receipt.package.can_build_broker_request is False
    assert receipt.package.can_submit_order is False
    assert receipt.package.is_executable is False
...
def test_receipt_is_verified_and_tamper_evident() -> None:
    ...
    assert receipt.can_continue_to_storage_admission is True
...
def test_decision_performs_no_write_or_execution() -> None:
    ...
    assert decision.can_continue_to_storage_admission is True
...
def test_receipt_contains_no_write_or_execution_surface(
    attribute_name: str,
) -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
...
def test_receipt_id_is_deterministic() -> None:
    ...
    checks = ",".join(check.value for check in receipt.checks)

    assert receipt.receipt_id == (
        "XAUUSDm:BUY:"
        "AUDIT_EXPORT_VERIFIED:"
        f"BYTES[{receipt.verified_content_length_bytes}]:"
        f"SHA256[{receipt.verified_content_digest}]:"
        f"CHECKS[{checks}]"
    )
...
def test_receipt_stable_id_is_deterministic() -> None:
    ...
    assert receipt.stable_id == (
        f"{audit_export.stable_id}:PLANNING_AUDIT_VERIFICATION_RECEIPT:{receipt.receipt_id}"
    )


def test_verified_decision_stable_id_is_deterministic() -> None:
    audit_export = bullish_audit_export()
    decision = StrategyPlanningAuditVerificationFactory().verify(audit_export)

    assert decision.stable_id == (
        f"{audit_export.stable_id}:PLANNING_AUDIT_VERIFICATION:VERIFIED:VERIFIED:NONE"
    )


def test_blocked_decision_stable_id_is_deterministic() -> None:
    audit_export = blocked_audit_export()
    decision = StrategyPlanningAuditVerificationFactory().verify(audit_export)

    assert decision.stable_id == (
        f"{audit_export.stable_id}:"
        "PLANNING_AUDIT_VERIFICATION:"
        "BLOCKED:AUDIT_EXPORT_BLOCKED:"
        "AUDIT_EXPORT_BLOCKED"
    )


def test_direct_receipt_rejects_blocked_export() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    ...
    with pytest.raises(
        ValueError,
        match="created audit-export envelope",
    ):
        replace(
            receipt,
            audit_export=blocked_audit_export(),
...
def test_direct_receipt_rejects_raw_checks() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="PlanningAuditVerificationCheck",
    ):
        replace(
            receipt,
...
def test_direct_receipt_rejects_invalid_length(
    verified_content_length_bytes: object,
) -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    ...
    with pytest.raises(ValueError):
        replace(
            receipt,
            verified_content_length_bytes=(verified_content_length_bytes),
        )


def test_direct_receipt_rejects_wrong_length() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    ...
    with pytest.raises(
        ValueError,
        match="independently measured",
    ):
        replace(
            receipt,
            verified_content_length_bytes=(receipt.verified_content_length_bytes + 1),
        )


def test_direct_receipt_rejects_wrong_content_digest() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    wrong_digest = "0" * 64 if receipt.verified_content_digest != "0" * 64 else "1" * 64

    with pytest.raises(
        ValueError,
        match="independently calculated",
    ):
        replace(
            receipt,
            verified_content_digest=wrong_digest,
        )


def test_direct_receipt_rejects_wrong_manifest_digest() -> None:
    receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
    wrong_digest = "0" * 64 if receipt.verified_manifest_digest != "0" * 64 else "1" * 64

    with pytest.raises(
        ValueError,
        match="export envelope",
    ):
        replace(
            receipt,
            verified_manifest_digest=wrong_digest,
        )
...
def test_manual_decision_rejects_unexpected_receipt() -> None:
    blocked = StrategyPlanningAuditVerificationFactory().verify(blocked_audit_export())
    created_receipt = (
        StrategyPlanningAuditVerificationFactory().verify(bullish_audit_export()).receipt_required
    )
...
def test_manual_decision_rejects_duplicate_blockers() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            decision,
            blockers=(
                PlanningAuditVerificationBlocker.AUDIT_EXPORT_BLOCKED,
                PlanningAuditVerificationBlocker.AUDIT_EXPORT_BLOCKED,
            ),
        )
...
def test_receipt_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        receipt.verified_content_digest = "0" * 64
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = PlanningAuditVerificationStatus.BLOCKED


def test_verification_is_deterministic() -> None:
    factory = StrategyPlanningAuditVerificationFactory()
    audit_export = bullish_audit_export()

    assert factory.verify(audit_export) == factory.verify(audit_export)


def test_function_api_delegates() -> None:
    decision = verify_planning_audit_export(bullish_audit_export())
...
def test_factory_alias_methods_delegate() -> None:
    ...
    assert factory.generate(audit_export) == factory.verify(audit_export)
    assert factory.evaluate(audit_export) == factory.verify(audit_export)


def test_public_aliases_are_preserved() -> None:
    assert AuditExportVerificationCheck is PlanningAuditVerificationCheck
    assert AuditExportVerificationDecision is PlanningAuditVerificationDecision
    assert AuditExportVerificationFactory is StrategyPlanningAuditVerificationFactory
    assert AuditExportVerificationReceipt is StrategyPlanningAuditVerificationReceipt
    assert PlanningAuditVerificationFactory is StrategyPlanningAuditVerificationFactory
    assert PlanningAuditVerificationReceipt is StrategyPlanningAuditVerificationReceipt
    assert StrategyAuditVerificationReceipt is StrategyPlanningAuditVerificationReceipt
