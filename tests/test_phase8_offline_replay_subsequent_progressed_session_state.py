...
from tests.test_phase8_offline_replay_subsequent_transition_application import (
    CAPTURED_AT,
    ...
    bullish_subsequent_transition_application_decision,
)


@lru_cache(maxsize=1)
def state_bearish_application_decision():
...
@lru_cache(maxsize=1)
def state_existing_application_decision():
    return StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        application_existing_subsequent_contract_decision()
    )
...
@lru_cache(maxsize=1)
def state_blocked_application_decision():
    return StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        application_blocked_subsequent_contract_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_subsequent_progressed_state_decision():
...
def test_invalid_application_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplaySubsequentProgressedSessionStateError,
        match=("INVALID_SUBSEQUENT_TRANSITION_APPLICATION_DECISION"),
    ) as captured:
        (StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate("invalid"))
...
def test_default_policy_is_strict() -> None:
    ...
    assert policy.prior_progressed_state_immutable is True
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplaySubsequentProgressedSessionStatePolicy(**{field_name: 1})


def test_bullish_subsequent_progressed_state_is_created() -> None:
    decision = bullish_subsequent_progressed_state_decision()

    assert decision.status == (Phase8OfflineReplaySubsequentProgressedSessionStateStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplaySubsequentProgressedSessionStateReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_state is True


def test_bearish_subsequent_progressed_state_is_created() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(
        state_bearish_application_decision()
    )
...
def test_existing_subsequent_progressed_state_is_created() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(
        state_existing_application_decision()
    )
...
def test_blocked_application_blocks_state() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(
        state_blocked_application_decision()
    )
...
def test_state_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 subsequent progressed",
    ):
        _ = decision.state_required


def test_state_preserves_application_identity() -> None:
    state = bullish_subsequent_progressed_state_decision().state_required

    assert (
        state.subsequent_transition_application_decision
        is bullish_subsequent_transition_application_decision()
    )
    assert state.application_receipt is (
        bullish_subsequent_transition_application_decision().receipt_required
    )
...
def test_state_preserves_lineage() -> None:
    ...
    assert state.subsequent_transition_contract is (receipt.transition_contract)
    assert state.prior_progressed_state is (receipt.progressed_state)
...
def test_state_preserves_identifiers() -> None:
    ...
    assert state.subsequent_transition_contract_id == (
        state.subsequent_transition_contract.stable_id
    )
    assert state.prior_progressed_state_id == (state.prior_progressed_state.stable_id)
...
def test_state_preserves_digests() -> None:
    ...
    assert state.subsequent_transition_contract_digest == (
        state.subsequent_transition_contract.transition_digest
    )
    assert state.prior_progressed_state_digest == (state.prior_progressed_state.state_digest)
...
def test_subsequent_progressed_counters_are_exact() -> None:
    state = bullish_subsequent_progressed_state_decision().state_required

    assert state.state_version == 3
    assert state.cursor_index == 3
    assert state.consumed_count == 3
    assert state.remaining_count == 797
    assert state.total_event_count == 800
    assert state.last_consumed_sequence_index == 2
    assert state.next_event_sequence_index == 3
...
def test_sequence_progresses_from_prior_state() -> None:
    state = bullish_subsequent_progressed_state_decision().state_required
    prior = state.prior_progressed_state
...
def test_prior_progressed_state_remains_unchanged() -> None:
    state = bullish_subsequent_progressed_state_decision().state_required
    prior = state.prior_progressed_state

    assert state.prior_state_preserved is True
    assert prior.cursor_index == 2
    assert prior.consumed_count == 2
    assert prior.remaining_count == 798
    assert prior.last_consumed_sequence_index == 1
    assert prior.next_event_sequence_index == 2
    assert prior.state_digest == (state.prior_progressed_state_digest)
...
def test_state_controls_are_exact() -> None:
    ...
    assert state.lifecycle == (Phase8OfflineReplaySubsequentProgressedSessionLifecycle.ACTIVE)
...
def test_state_is_reusable_state_only() -> None:
    ...
    assert state.is_subsequent_progressed_state is True
...
def test_subsequent_progressed_state_id_is_deterministic() -> None:
    state = bullish_subsequent_progressed_state_decision().state_required

    assert state.subsequent_progressed_state_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_subsequent_transition_application_decision()
    decision = bullish_subsequent_progressed_state_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = state_blocked_application_decision()
    decision = StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(source)
    ...
    assert decision.stable_id == (
        ...
        "PROGRESSED_SESSION_STATE_GENERATION:"
        "BLOCKED:"
        "SUBSEQUENT_TRANSITION_APPLICATION_BLOCKED:"
        "SUBSEQUENT_TRANSITION_APPLICATION_BLOCKED"
    )
...
def test_direct_state_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            state,
            policy=(Phase8OfflineReplaySubsequentProgressedSessionStatePolicy(no_lookahead=False)),
        )
...
def test_direct_state_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Phase8OfflineReplaySubsequentProgressedSessionStateMode"),
    ):
        replace(
            state,
            state_mode=("IMMUTABLE_SUBSEQUENT_PROGRESSED_STATE"),
        )
...
def test_direct_state_rejects_raw_lifecycle() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Phase8OfflineReplaySubsequentProgressedSessionLifecycle"),
    ):
        replace(
            state,
            lifecycle="ACTIVE",
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplaySubsequentProgressedSessionStateStatus.BLOCKED


def test_state_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory()
    source = bullish_subsequent_transition_application_decision()
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_subsequent_progressed_session_state(
        bullish_subsequent_transition_application_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplaySubsequentProgressedSessionState
        is StrategyPhase8OfflineReplaySubsequentProgressedSessionState
    )
    assert (
        Phase8OfflineReplaySubsequentProgressedSessionStateFactory
        is StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory
    )
