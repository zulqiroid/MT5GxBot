...
def test_invalid_specification_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayPlanError,
        match="INVALID_RUN_SPECIFICATION_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplayPlanFactory().generate("invalid"))
...
def test_default_replay_policy_is_strict() -> None:
    policy = Phase8OfflineReplayPlanPolicy()

    assert policy.is_strict is True
    assert policy.emit_on_candle_close is True
    assert policy.preserve_series_order is True
    assert policy.deterministic_tie_break is True
    assert policy.no_lookahead is True
    assert policy.include_warmup_candles is True
...
def test_replay_policy_rejects_non_boolean(
    field_name,
) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayPlanPolicy(**{field_name: 1})


def test_bullish_replay_plan_is_created() -> None:
    decision = bullish_replay_plan_decision()

    assert decision.status == (Phase8OfflineReplayPlanStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayPlanReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_plan is True


def test_bearish_replay_plan_is_created() -> None:
    decision = StrategyPhase8OfflineReplayPlanFactory().generate(
...
def test_existing_replay_plan_is_created() -> None:
    decision = StrategyPhase8OfflineReplayPlanFactory().generate(
...
def test_blocked_specification_blocks_plan() -> None:
    decision = StrategyPhase8OfflineReplayPlanFactory().generate(
    ...
    assert decision.reason == (Phase8OfflineReplayPlanReason.RUN_SPECIFICATION_BLOCKED)
    assert decision.blockers == (Phase8OfflineReplayPlanBlocker.RUN_SPECIFICATION_BLOCKED,)
...
def test_plan_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 offline replay plan",
    ):
        _ = decision.plan_required


def test_plan_preserves_specification_identity() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert plan.specification_decision is (offline_bullish_specification_decision())
    assert plan.specification is (offline_bullish_specification_decision().specification_required)


def test_plan_preserves_complete_lineage() -> None:
    plan = bullish_replay_plan_decision().plan_required
    specification = plan.specification

    assert plan.input_package is (specification.input_package)
    assert plan.verification_receipt is (specification.verification_receipt)
    assert plan.snapshot is specification.snapshot
    assert plan.contract is specification.contract
    assert plan.dry_run_package is (specification.dry_run_package)


def test_plan_preserves_identifiers() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_plan_preserves_digests() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_plan_preserves_metadata() -> None:
    ...
    assert plan.schema_version == (PHASE_8_OFFLINE_REPLAY_PLAN_SCHEMA_VERSION)


def test_plan_has_exact_timeframes() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_series_priorities_are_exact() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert tuple(item.priority for item in plan.series_plans) == (0, 1, 2, 3)


def test_series_counts_are_exact() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert tuple(item.candle_count for item in plan.series_plans) == (200, 200, 200, 200)
    assert plan.total_event_count == 800
    assert plan.replay_event_count == 800


def test_series_index_bounds_are_complete() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert all(
        item.start_index == 0
        and item.end_index == item.candle_count - 1
        and item.is_complete_series
        for item in plan.series_plans
    )


def test_series_timestamp_bounds_match_snapshot() -> None:
    plan = bullish_replay_plan_decision().plan_required

    for source, planned in zip(
        plan.snapshot.series,
        plan.series_plans,
        strict=True,
    ):
        assert planned.first_open_time == (source.first_open_time)
        assert planned.first_close_time == (source.candles[0].close_time)
        assert planned.latest_open_time == (source.latest_open_time)
        assert planned.latest_close_time == (source.latest_close_time)


def test_series_digests_match_snapshot() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert tuple(item.series_digest for item in plan.series_plans) == tuple(
        item.series_digest for item in plan.snapshot.series
    )


def test_all_planned_candles_are_closed() -> None:
    plan = bullish_replay_plan_decision().plan_required

    assert plan.latest_close_time <= plan.captured_at
    assert all(item.latest_close_time <= plan.captured_at for item in plan.series_plans)


def test_replay_controls_are_exact() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_plan_digest_is_deterministic() -> None:
    ...
    assert plan.plan_digest == hashlib.sha256(plan.canonical_payload.encode("utf-8")).hexdigest()
    assert plan.digest_algorithm == "SHA-256"


def test_plan_is_offline_and_no_lookahead() -> None:
    plan = bullish_replay_plan_decision().plan_required
    ...
    assert plan.can_continue_to_replay_event_contract is True


def test_plan_performs_no_io_or_execution() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_plan_has_no_implementation_surface(
    attribute_name,
) -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_plan_id_is_deterministic() -> None:
    ...
    assert plan.plan_id == (
        f"XAUUSDm:BUY:PHASE_8_OFFLINE_REPLAY_PLAN:PLAN_SHA256[{plan.plan_digest}]"
    )


def test_created_decision_stable_id_is_deterministic() -> None:
    specification = offline_bullish_specification_decision()
    decision = bullish_replay_plan_decision()

    assert decision.stable_id == (
        f"{specification.stable_id}:PHASE_8_OFFLINE_REPLAY_PLAN_GENERATION:CREATED:CREATED:NONE"
    )


def test_blocked_decision_stable_id_is_deterministic() -> None:
    specification = blocked_specification_for_replay_plan()
    decision = StrategyPhase8OfflineReplayPlanFactory().generate(specification)

    assert decision.stable_id == (
        f"{specification.stable_id}:"
        "PHASE_8_OFFLINE_REPLAY_PLAN_GENERATION:"
        "BLOCKED:RUN_SPECIFICATION_BLOCKED:"
        "RUN_SPECIFICATION_BLOCKED"
    )
...
def test_direct_plan_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            plan,
            policy=Phase8OfflineReplayPlanPolicy(no_lookahead=False),
        )
...
def test_direct_plan_rejects_reordered_series() -> None:
    plan = bullish_replay_plan_decision().plan_required
    ...
    with pytest.raises(
        ...
        replace(
            plan,
            series_plans=tuple(reversed(plan.series_plans)),
        )
...
def test_direct_plan_rejects_wrong_series_count() -> None:
    plan = bullish_replay_plan_decision().plan_required
    first = replace(
        plan.series_plans[0],
        candle_count=199,
...
def test_direct_plan_rejects_wrong_series_digest() -> None:
    plan = bullish_replay_plan_decision().plan_required
    first = replace(
        plan.series_plans[0],
        series_digest="0" * 64,
...
def test_direct_plan_rejects_wrong_total_events() -> None:
    plan = bullish_replay_plan_decision().plan_required
...
def test_series_plan_rejects_nonzero_start() -> None:
    series_plan = bullish_replay_plan_decision().plan_required.series_plans[0]
...
def test_series_plan_rejects_wrong_end() -> None:
    series_plan = bullish_replay_plan_decision().plan_required.series_plans[0]
...
def test_series_plan_rejects_naive_time() -> None:
    series_plan = bullish_replay_plan_decision().plan_required.series_plans[0]
    ...
    with pytest.raises(
        ...
        replace(
            series_plan,
            first_open_time=datetime(2025, 1, 1),
        )
...
def test_manual_decision_rejects_duplicate_blockers() -> None:
    ...
    with pytest.raises(ValueError, match="duplicates"):
        replace(
            decision,
            blockers=(
                Phase8OfflineReplayPlanBlocker.RUN_SPECIFICATION_BLOCKED,
                Phase8OfflineReplayPlanBlocker.RUN_SPECIFICATION_BLOCKED,
            ),
        )
...
def test_series_plan_is_immutable() -> None:
    series_plan = bullish_replay_plan_decision().plan_required.series_plans[0]

    with pytest.raises(FrozenInstanceError):
        series_plan.priority = 9
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayPlanStatus.BLOCKED


def test_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayPlanFactory()
    source = offline_bullish_specification_decision()
    ...
    assert first.series_plans == second.series_plans


def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_plan(offline_bullish_specification_decision())
...
def test_factory_alias_methods_delegate() -> None:
    ...
    assert factory.build(source).plan_required.plan_digest == generated.plan_required.plan_digest
    assert factory.evaluate(source).plan_required.plan_digest == generated.plan_required.plan_digest


def test_public_aliases_are_preserved() -> None:
    assert Phase8OfflineReplayPlan is StrategyPhase8OfflineReplayPlan
    assert Phase8OfflineReplayPlanFactory is StrategyPhase8OfflineReplayPlanFactory
