...
from tests.test_phase8_offline_replay_successive_iterative_recurrent_bounded_application import (
    FakeAllowedNearTerminalDecision,
    bullish_successive_iterative_recurrent_bounded_application_decision,
)
...
def bullish_successive_iterative_recurrent_bounded_progressed_state_decision():
    return create_phase8_offline_replay_successive_iterative_recurrent_bounded_progressed_state(
        bullish_successive_iterative_recurrent_bounded_application_decision()
    )
...
def test_bullish_successive_iterative_recurrent_progressed_state_is_created() -> None:
    decision = bullish_successive_iterative_recurrent_bounded_progressed_state_decision()
...
def test_progressed_state_preserves_lineage_identity() -> None:
    application_decision = bullish_successive_iterative_recurrent_bounded_application_decision()
    receipt = application_decision.receipt_required

    state = create_phase8_offline_replay_successive_iterative_recurrent_bounded_progressed_state(
...
def test_default_progressed_state_counters_are_exact() -> None:
    state = (
        bullish_successive_iterative_recurrent_bounded_progressed_state_decision().state_required
    )

    assert state.state_version == 12
    assert state.cursor_index == 198
    assert state.consumed_count == 198
    assert state.remaining_count == 602
    assert state.last_consumed_sequence_index == 197
    assert state.next_event_sequence_index == 198
...
def test_blocked_application_blocks_progressed_state() -> None:
    decision = create_phase8_offline_replay_successive_iterative_recurrent_bounded_progressed_state(
...
def test_terminal_progressed_state_is_exact() -> None:
    ...
    application_decision = apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
        plan_decision
    )
    ...
    assert state.state_version == 500
...
def test_prior_state_is_not_mutated() -> None:
    ...
    _ = create_phase8_offline_replay_successive_iterative_recurrent_bounded_progressed_state(
        application_decision
    ).state_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
    function_decision = (
        create_phase8_offline_replay_successive_iterative_recurrent_bounded_progressed_state(
