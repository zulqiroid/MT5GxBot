...
from tests.test_phase8_offline_replay_iterative_transition_contract import (
    CAPTURED_AT,
    bullish_iterative_transition_contract_decision,
...
@lru_cache(maxsize=1)
def application_bearish_iterative_contract_decision():
...
@lru_cache(maxsize=1)
def application_existing_iterative_contract_decision():
    return StrategyPhase8OfflineReplayIterativeTransitionContractFactory().generate(
        iterative_existing_source_decision()
    )
...
@lru_cache(maxsize=1)
def application_blocked_iterative_contract_decision():
    return StrategyPhase8OfflineReplayIterativeTransitionContractFactory().generate(
        iterative_blocked_source_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_iterative_transition_application_decision():
...
def test_invalid_contract_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayIterativeTransitionApplicationError,
        match="INVALID_ITERATIVE_TRANSITION_CONTRACT_DECISION",
    ) as captured:
        (StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate("invalid"))
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayIterativeTransitionApplicationPolicy(**{field_name: 1})


def test_bullish_iterative_transition_is_applied() -> None:
    decision = bullish_iterative_transition_application_decision()

    assert decision.status == (Phase8OfflineReplayIterativeTransitionApplicationStatus.APPLIED)
    assert decision.reason == (Phase8OfflineReplayIterativeTransitionApplicationReason.APPLIED)
    assert decision.blockers == ()
    assert decision.is_applied is True
    assert decision.has_receipt is True


def test_bearish_iterative_transition_is_applied() -> None:
    decision = StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(
...
def test_existing_iterative_transition_is_applied() -> None:
    decision = StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(
...
def test_receipt_preserves_contract_identity() -> None:
    receipt = bullish_iterative_transition_application_decision().receipt_required

    assert receipt.iterative_transition_contract_decision is (
        bullish_iterative_transition_contract_decision()
    )
    assert receipt.transition_contract is (
        bullish_iterative_transition_contract_decision().transition_contract_required
    )
...
def test_receipt_preserves_identifiers() -> None:
    ...
    assert receipt.iterative_transition_contract_id == (receipt.transition_contract.stable_id)
...
def test_receipt_preserves_digests() -> None:
    ...
    assert receipt.iterative_transition_contract_digest == (
...
def test_prior_transition_state_is_exact() -> None:
    ...
    assert receipt.transition_index == 4
    assert receipt.prior_cursor_index == 4
    assert receipt.prior_consumed_count == 4
    assert receipt.prior_remaining_count == 796
    assert receipt.prior_last_consumed_sequence_index == 3
...
def test_resulting_transition_state_is_exact() -> None:
    ...
    assert receipt.resulting_cursor_index == 5
    assert receipt.resulting_consumed_count == 5
    assert receipt.resulting_remaining_count == 795
    assert receipt.last_consumed_sequence_index == 4
    assert receipt.completion_reached is False
    assert receipt.next_event_sequence_index == 5
...
def test_consumed_event_matches_contract() -> None:
    ...
    assert receipt.consumed_event_sequence_index == 4
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_iterative_transition_contract_decision()
    decision = bullish_iterative_transition_application_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = application_blocked_iterative_contract_decision()
    decision = StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(source)
    ...
    assert decision.stable_id == (
        ...
        "TRANSITION_APPLICATION:"
        "BLOCKED:"
        "ITERATIVE_TRANSITION_CONTRACT_BLOCKED:"
        "ITERATIVE_TRANSITION_CONTRACT_BLOCKED"
    )
...
def test_direct_receipt_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            receipt,
            policy=(Phase8OfflineReplayIterativeTransitionApplicationPolicy(no_lookahead=False)),
        )
...
def test_direct_receipt_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="IterativeTransitionApplicationMode",
    ):
        replace(
            receipt,
...
def test_direct_receipt_rejects_wrong_consumed_count() -> None:
    ...
    with pytest.raises(ValueError, match="transition_index"):
        replace(
            receipt,
            prior_consumed_count=5,
            prior_remaining_count=795,
        )
...
def test_direct_receipt_rejects_wrong_consumed_sequence() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            receipt,
            consumed_event_sequence_index=5,
...
def test_receipt_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        receipt.resulting_cursor_index = 6
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayIterativeTransitionApplicationStatus.BLOCKED


def test_application_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory()
    source = bullish_iterative_transition_contract_decision()
...
def test_function_api_delegates() -> None:
    decision = apply_phase8_offline_replay_iterative_transition(
        bullish_iterative_transition_contract_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplayIterativeTransitionApplicationReceipt
        is StrategyPhase8OfflineReplayIterativeTransitionApplicationReceipt
    )
    assert (
        Phase8OfflineReplayIterativeTransitionApplicationFactory
        is StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory
    )
