...
from tests.test_phase8_offline_replay_subsequent_progressed_session_state import (
    CAPTURED_AT,
    bullish_subsequent_progressed_state_decision,
...
@lru_cache(maxsize=1)
def recurrent_bearish_source_decision():
...
@lru_cache(maxsize=1)
def recurrent_existing_source_decision():
    return StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(
        state_existing_application_decision()
    )
...
@lru_cache(maxsize=1)
def recurrent_blocked_source_decision():
    return StrategyPhase8OfflineReplaySubsequentProgressedSessionStateFactory().generate(
        state_blocked_application_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_recurrent_transition_contract_decision():
...
def test_invalid_source_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayRecurrentTransitionContractError,
        match=("INVALID_SUBSEQUENT_PROGRESSED_STATE_DECISION"),
    ) as captured:
        (StrategyPhase8OfflineReplayRecurrentTransitionContractFactory().generate("invalid"))
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayRecurrentTransitionContractPolicy(**{field_name: 1})


def test_bullish_recurrent_contract_is_created() -> None:
    decision = bullish_recurrent_transition_contract_decision()

    assert decision.status == (Phase8OfflineReplayRecurrentTransitionContractStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayRecurrentTransitionContractReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_transition_contract is True


def test_bearish_recurrent_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayRecurrentTransitionContractFactory().generate(
        recurrent_bearish_source_decision()
    )
...
def test_existing_recurrent_contract_is_created() -> None:
    decision = StrategyPhase8OfflineReplayRecurrentTransitionContractFactory().generate(
        recurrent_existing_source_decision()
    )

    assert decision.is_created is True


def test_blocked_state_blocks_recurrent_contract() -> None:
    decision = StrategyPhase8OfflineReplayRecurrentTransitionContractFactory().generate(
        recurrent_blocked_source_decision()
    )
...
def test_contract_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 recurrent offline",
    ):
        _ = decision.transition_contract_required


def test_contract_preserves_source_identity() -> None:
    contract = bullish_recurrent_transition_contract_decision().transition_contract_required

    assert contract.subsequent_progressed_state_decision is (
        bullish_subsequent_progressed_state_decision()
    )
    assert contract.source_state is (bullish_subsequent_progressed_state_decision().state_required)
...
def test_contract_preserves_lineage() -> None:
    ...
    assert contract.prior_transition_contract is (state.subsequent_transition_contract)
...
def test_contract_controls_are_exact() -> None:
    ...
    assert contract.action == (Phase8OfflineReplayRecurrentTransitionAction.CONSUME_CURRENT_EVENT)


def test_current_transition_state_is_exact() -> None:
    contract = bullish_recurrent_transition_contract_decision().transition_contract_required

    assert contract.transition_index == 3
    assert contract.current_cursor_index == 3
    assert contract.current_consumed_count == 3
    assert contract.current_remaining_count == 797
    assert contract.prior_last_consumed_sequence_index == 2
    assert contract.current_event_sequence_index == 3
...
def test_resulting_transition_state_is_exact() -> None:
    ...
    assert contract.resulting_cursor_index == 4
    assert contract.resulting_consumed_count == 4
    assert contract.resulting_remaining_count == 796
    assert contract.last_consumed_sequence_index == 3
    assert contract.completion_after_transition is False
    assert contract.next_event_sequence_index == 4
...
def test_next_event_matches_resulting_cursor() -> None:
    contract = bullish_recurrent_transition_contract_decision().transition_contract_required
    expected = contract.event_batch.events[4]
...
def test_source_state_remains_unchanged() -> None:
    ...
    assert source.cursor_index == 3
    assert source.consumed_count == 3
    assert source.remaining_count == 797
    assert source.last_consumed_sequence_index == 2
    assert source.next_event_sequence_index == 3
...
def test_contract_is_definition_only() -> None:
    ...
    assert contract.can_continue_to_recurrent_transition_application is True
...
def test_recurrent_contract_id_is_deterministic() -> None:
    contract = bullish_recurrent_transition_contract_decision().transition_contract_required

    assert contract.recurrent_transition_contract_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_subsequent_progressed_state_decision()
    decision = bullish_recurrent_transition_contract_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = recurrent_blocked_source_decision()
    decision = StrategyPhase8OfflineReplayRecurrentTransitionContractFactory().generate(source)
    ...
    assert decision.stable_id == (
        ...
        "TRANSITION_CONTRACT_GENERATION:"
        "BLOCKED:"
        "SUBSEQUENT_PROGRESSED_STATE_BLOCKED:"
        "SUBSEQUENT_PROGRESSED_STATE_BLOCKED"
    )
...
def test_direct_contract_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            contract,
            policy=(Phase8OfflineReplayRecurrentTransitionContractPolicy(no_lookahead=False)),
        )
...
def test_direct_contract_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Phase8OfflineReplayRecurrentTransitionContractMode"),
    ):
        replace(
            contract,
...
def test_direct_contract_rejects_raw_action() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Phase8OfflineReplayRecurrentTransitionAction"),
    ):
        replace(
            contract,
...
def test_direct_contract_rejects_wrong_consumed_count() -> None:
    ...
    with pytest.raises(ValueError, match="transition_index"):
        replace(
            contract,
            current_consumed_count=4,
            current_remaining_count=796,
        )
...
def test_direct_contract_rejects_wrong_prior_sequence() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            contract,
            prior_last_consumed_sequence_index=3,
        )
...
def test_direct_contract_rejects_wrong_current_sequence() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            contract,
            current_event_sequence_index=4,
...
def test_direct_contract_rejects_wrong_result_consumed() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            contract,
            resulting_consumed_count=5,
            resulting_remaining_count=795,
        )
...
def test_contract_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        contract.resulting_cursor_index = 5
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayRecurrentTransitionContractStatus.BLOCKED


def test_contract_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayRecurrentTransitionContractFactory()
    source = bullish_subsequent_progressed_state_decision()
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_recurrent_transition_contract(
        bullish_subsequent_progressed_state_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplayRecurrentTransitionContract
        is StrategyPhase8OfflineReplayRecurrentTransitionContract
    )
    assert (
        Phase8OfflineReplayRecurrentTransitionContractFactory
        is StrategyPhase8OfflineReplayRecurrentTransitionContractFactory
    )
