...
from tests.test_phase8_offline_replay_subsequent_transition_contract import (
    CAPTURED_AT,
    bullish_subsequent_transition_contract_decision,
...
@lru_cache(maxsize=1)
def application_bearish_subsequent_contract_decision():
...
@lru_cache(maxsize=1)
def application_existing_subsequent_contract_decision():
    return StrategyPhase8OfflineReplaySubsequentTransitionContractFactory().generate(
        subsequent_existing_progressed_state_decision()
    )
...
@lru_cache(maxsize=1)
def application_blocked_subsequent_contract_decision():
    return StrategyPhase8OfflineReplaySubsequentTransitionContractFactory().generate(
        subsequent_blocked_progressed_state_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_subsequent_transition_application_decision():
...
def test_invalid_contract_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplaySubsequentTransitionApplicationError,
        match=("INVALID_SUBSEQUENT_TRANSITION_CONTRACT_DECISION"),
    ) as captured:
        (StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate("invalid"))
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplaySubsequentTransitionApplicationPolicy(**{field_name: 1})


def test_bullish_subsequent_transition_is_applied() -> None:
    decision = bullish_subsequent_transition_application_decision()

    assert decision.status == (Phase8OfflineReplaySubsequentTransitionApplicationStatus.APPLIED)
    assert decision.reason == (Phase8OfflineReplaySubsequentTransitionApplicationReason.APPLIED)
    assert decision.blockers == ()
    assert decision.is_applied is True
    assert decision.has_receipt is True


def test_bearish_subsequent_transition_is_applied() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        application_bearish_subsequent_contract_decision()
    )
...
def test_existing_subsequent_transition_is_applied() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        application_existing_subsequent_contract_decision()
    )
...
def test_blocked_contract_blocks_application() -> None:
    decision = StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        application_blocked_subsequent_contract_decision()
    )
...
def test_receipt_preserves_contract_identity() -> None:
    receipt = bullish_subsequent_transition_application_decision().receipt_required

    assert receipt.subsequent_transition_contract_decision is (
        bullish_subsequent_transition_contract_decision()
    )
    assert receipt.transition_contract is (
        bullish_subsequent_transition_contract_decision().transition_contract_required
    )
...
def test_receipt_preserves_lineage() -> None:
    ...
    assert receipt.progressed_state is (contract.progressed_state)
...
def test_receipt_preserves_identifiers() -> None:
    ...
    assert receipt.subsequent_transition_contract_id == (receipt.transition_contract.stable_id)
    assert receipt.progressed_state_id == (receipt.progressed_state.stable_id)
...
def test_receipt_preserves_digests() -> None:
    ...
    assert receipt.subsequent_transition_contract_digest == (
        receipt.transition_contract.transition_digest
    )
    assert receipt.progressed_state_digest == (receipt.progressed_state.state_digest)
...
def test_prior_state_is_exact() -> None:
    ...
    assert receipt.transition_index == 2
    assert receipt.prior_cursor_index == 2
    assert receipt.prior_consumed_count == 2
    assert receipt.prior_remaining_count == 798
    assert receipt.prior_last_consumed_sequence_index == 1
...
def test_resulting_state_is_exact() -> None:
    ...
    assert receipt.resulting_cursor_index == 3
    assert receipt.resulting_consumed_count == 3
    assert receipt.resulting_remaining_count == 797
    assert receipt.last_consumed_sequence_index == 2
    assert receipt.completion_reached is False
    assert receipt.next_event_sequence_index == 3
...
def test_consumed_event_matches_contract() -> None:
    ...
    assert receipt.consumed_event_sequence_index == 2
...
def test_progressed_source_state_remains_unchanged() -> None:
    receipt = bullish_subsequent_transition_application_decision().receipt_required
    source = receipt.progressed_state

    assert receipt.progressed_source_state_preserved is True
    assert source.cursor_index == 2
    assert source.consumed_count == 2
    assert source.remaining_count == 798
    assert source.last_consumed_sequence_index == 1
    assert source.next_event_sequence_index == 2
    assert source.state_digest == (receipt.progressed_state_digest)
...
def test_receipt_applies_one_transition_only() -> None:
    ...
    assert receipt.can_continue_to_subsequent_progressed_state is True
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_subsequent_transition_contract_decision()
    decision = bullish_subsequent_transition_application_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = application_blocked_subsequent_contract_decision()
    decision = StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(source)
    ...
    assert decision.stable_id == (
        ...
        "TRANSITION_APPLICATION:"
        "BLOCKED:"
        "SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED:"
        "SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED"
    )
...
def test_direct_receipt_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            receipt,
            policy=(Phase8OfflineReplaySubsequentTransitionApplicationPolicy(no_lookahead=False)),
        )
...
def test_direct_receipt_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Phase8OfflineReplaySubsequentTransitionApplicationMode"),
    ):
        replace(
            receipt,
...
def test_direct_receipt_rejects_wrong_consumed_count() -> None:
    ...
    with pytest.raises(ValueError, match="transition_index"):
        replace(
            receipt,
            prior_consumed_count=3,
            prior_remaining_count=797,
        )
...
def test_receipt_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        receipt.resulting_cursor_index = 4
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplaySubsequentTransitionApplicationStatus.BLOCKED


def test_application_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory()
    source = bullish_subsequent_transition_contract_decision()
...
def test_function_api_delegates() -> None:
    decision = apply_phase8_offline_replay_subsequent_transition(
        bullish_subsequent_transition_contract_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplaySubsequentTransitionApplicationReceipt
        is StrategyPhase8OfflineReplaySubsequentTransitionApplicationReceipt
    )
    assert (
        Phase8OfflineReplaySubsequentTransitionApplicationFactory
        is StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory
    )
