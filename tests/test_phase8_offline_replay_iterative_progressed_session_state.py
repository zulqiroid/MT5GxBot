...
from tests.test_phase8_offline_replay_iterative_transition_application import (
    CAPTURED_AT,
    application_bearish_iterative_contract_decision,
    application_blocked_iterative_contract_decision,
    application_existing_iterative_contract_decision,
    bullish_iterative_transition_application_decision,
)


@lru_cache(maxsize=1)
def state_bearish_iterative_application_decision():
...
@lru_cache(maxsize=1)
def state_existing_iterative_application_decision():
    return StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(
        application_existing_iterative_contract_decision()
    )
...
@lru_cache(maxsize=1)
def state_blocked_iterative_application_decision():
    return StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(
        application_blocked_iterative_contract_decision()
    )
...
@lru_cache(maxsize=1)
def bullish_iterative_progressed_state_decision():
...
def test_invalid_application_decision_is_fail_safe() -> None:
    with pytest.raises(
        Phase8OfflineReplayIterativeProgressedSessionStateError,
        match=("INVALID_ITERATIVE_TRANSITION_APPLICATION_DECISION"),
    ) as captured:
        (StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate("invalid"))
...
def test_policy_rejects_non_boolean(field_name) -> None:
    with pytest.raises(ValueError, match="boolean"):
        Phase8OfflineReplayIterativeProgressedSessionStatePolicy(**{field_name: 1})


def test_bullish_iterative_progressed_state_is_created() -> None:
    decision = bullish_iterative_progressed_state_decision()

    assert decision.status == (Phase8OfflineReplayIterativeProgressedSessionStateStatus.CREATED)
    assert decision.reason == (Phase8OfflineReplayIterativeProgressedSessionStateReason.CREATED)
    assert decision.blockers == ()
    assert decision.is_created is True
    assert decision.has_state is True


def test_bearish_iterative_progressed_state_is_created() -> None:
    decision = StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        state_bearish_iterative_application_decision()
    )
...
def test_existing_iterative_progressed_state_is_created() -> None:
    decision = StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        state_existing_iterative_application_decision()
    )
...
def test_blocked_application_blocks_state() -> None:
    decision = StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        state_blocked_iterative_application_decision()
    )
...
def test_state_required_rejects_blocked_result() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="No Phase 8 iterative progressed",
    ):
        _ = decision.state_required


def test_state_preserves_application_identity() -> None:
    state = bullish_iterative_progressed_state_decision().state_required

    assert state.iterative_transition_application_decision is (
        bullish_iterative_transition_application_decision()
    )
    assert state.application_receipt is (
        bullish_iterative_transition_application_decision().receipt_required
    )


def test_state_preserves_primary_lineage() -> None:
    state = bullish_iterative_progressed_state_decision().state_required
    receipt = state.application_receipt

    assert state.iterative_transition_contract is (receipt.transition_contract)
    assert state.prior_source_state is receipt.source_state
    assert state.event_batch is receipt.event_batch


def test_state_preserves_prior_recurrent_lineage() -> None:
    state = bullish_iterative_progressed_state_decision().state_required
    receipt = state.application_receipt
...
def test_state_preserves_identifiers() -> None:
    ...
    assert state.iterative_transition_contract_id == (state.iterative_transition_contract.stable_id)
...
def test_state_preserves_digests() -> None:
    ...
    assert state.iterative_transition_contract_digest == (
        state.iterative_transition_contract.transition_digest
    )
...
def test_iterative_progressed_counters_are_exact() -> None:
    state = bullish_iterative_progressed_state_decision().state_required

    assert state.state_version == 5
    assert state.cursor_index == 5
    assert state.consumed_count == 5
    assert state.remaining_count == 795
    assert state.total_event_count == 800
    assert state.last_consumed_sequence_index == 4
    assert state.next_event_sequence_index == 5
...
def test_last_consumed_event_matches_receipt() -> None:
    ...
    assert expected.sequence_index == 4
...
def test_next_event_matches_receipt() -> None:
    ...
    assert expected.sequence_index == 5
...
def test_prior_source_state_remains_unchanged() -> None:
    ...
    assert prior.state_version == 4
    assert prior.cursor_index == 4
    assert prior.consumed_count == 4
    assert prior.remaining_count == 796
    assert prior.last_consumed_sequence_index == 3
    assert prior.next_event_sequence_index == 4
...
def test_state_controls_are_exact() -> None:
    ...
    assert state.lifecycle == (Phase8OfflineReplayIterativeProgressedSessionLifecycle.ACTIVE)
...
def test_state_is_reusable_state_only() -> None:
    ...
    assert state.is_iterative_progressed_state is True
    ...
    assert state.can_continue_to_future_iterative_transition_contract is True
...
def test_iterative_progressed_state_id_is_deterministic() -> None:
    state = bullish_iterative_progressed_state_decision().state_required

    assert state.iterative_progressed_state_id == (
        "XAUUSDm:BUY:"
...
def test_created_decision_stable_id_is_deterministic() -> None:
    source = bullish_iterative_transition_application_decision()
    decision = bullish_iterative_progressed_state_decision()
...
def test_blocked_decision_stable_id_is_deterministic() -> None:
    source = state_blocked_iterative_application_decision()
    decision = StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(source)
    ...
    assert decision.stable_id == (
        ...
        "PROGRESSED_SESSION_STATE_GENERATION:"
        "BLOCKED:"
        "ITERATIVE_TRANSITION_APPLICATION_BLOCKED:"
        "ITERATIVE_TRANSITION_APPLICATION_BLOCKED"
    )
...
def test_direct_state_rejects_unsafe_policy() -> None:
    ...
    with pytest.raises(ValueError, match="strict"):
        replace(
            state,
            policy=(Phase8OfflineReplayIterativeProgressedSessionStatePolicy(no_lookahead=False)),
        )
...
def test_direct_state_rejects_raw_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="IterativeProgressedSessionStateMode",
    ):
        replace(
            state,
            state_mode=("IMMUTABLE_ITERATIVE_PROGRESSED_STATE"),
        )
...
def test_direct_state_rejects_raw_lifecycle() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="IterativeProgressedSessionLifecycle",
    ):
        replace(
            state,
            lifecycle="ACTIVE",
...
def test_decision_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        decision.status = Phase8OfflineReplayIterativeProgressedSessionStateStatus.BLOCKED


def test_state_generation_is_deterministic() -> None:
    factory = StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory()
    source = bullish_iterative_transition_application_decision()
...
def test_function_api_delegates() -> None:
    decision = generate_phase8_offline_replay_iterative_progressed_session_state(
        bullish_iterative_transition_application_decision()
    )
...
def test_public_aliases_are_preserved() -> None:
    assert (
        Phase8OfflineReplayIterativeProgressedSessionState
        is StrategyPhase8OfflineReplayIterativeProgressedSessionState
    )
    assert (
        Phase8OfflineReplayIterativeProgressedSessionStateFactory
        is StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory
    )
