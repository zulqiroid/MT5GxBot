...
from tests.test_phase8_offline_replay_recurrent_bounded_application import (
    FakeAllowedNearTerminalDecision,
    bullish_recurrent_bounded_application_decision,
)
...
def bullish_recurrent_bounded_progressed_state_decision():
    return create_phase8_offline_replay_recurrent_bounded_progressed_state(
        bullish_recurrent_bounded_application_decision()
    )


def test_schema_version_is_stable() -> None:
    assert PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_PROGRESSED_STATE_SCHEMA_VERSION == "1.0"


def test_bullish_recurrent_progressed_state_is_created() -> None:
    decision = bullish_recurrent_bounded_progressed_state_decision()
...
def test_progressed_state_preserves_lineage_identity() -> None:
    application_decision = bullish_recurrent_bounded_application_decision()
    receipt = application_decision.receipt_required

    state = create_phase8_offline_replay_recurrent_bounded_progressed_state(
...
def test_default_progressed_state_counters_are_exact() -> None:
    state = bullish_recurrent_bounded_progressed_state_decision().state_required

    assert state.state_version == 9
    assert state.cursor_index == 102
    assert state.consumed_count == 102
    assert state.remaining_count == 698
    assert state.last_consumed_sequence_index == 101
    assert state.next_event_sequence_index == 102
    assert state.total_event_count == 800
    assert state.lifecycle == PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_ACTIVE
...
def test_progressed_state_id_is_deterministic() -> None:
    first = bullish_recurrent_bounded_progressed_state_decision().state_required
    second = bullish_recurrent_bounded_progressed_state_decision().state_required
...
def test_blocked_application_blocks_progressed_state() -> None:
    decision = create_phase8_offline_replay_recurrent_bounded_progressed_state(
...
def test_missing_application_blocks_progressed_state() -> None:
    decision = create_phase8_offline_replay_recurrent_bounded_progressed_state(None)
...
def test_terminal_progressed_state_is_exact() -> None:
    ...
    application_decision = apply_phase8_offline_replay_recurrent_bounded_plan(plan_decision)

    state = create_phase8_offline_replay_recurrent_bounded_progressed_state(
        application_decision
    ).state_required

    assert state.state_version == 200
    ...
    assert state.total_event_count == 800
    assert state.lifecycle == PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_COMPLETED
...
def test_prior_state_is_not_mutated() -> None:
    ...
    _ = create_phase8_offline_replay_recurrent_bounded_progressed_state(
        application_decision
    ).state_required
...
def test_factory_and_function_api_match() -> None:
    ...
    factory_decision = StrategyPhase8OfflineReplayRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
    function_decision = create_phase8_offline_replay_recurrent_bounded_progressed_state(
        application_decision
    )
