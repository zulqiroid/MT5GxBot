...
PHASE_8_OFFLINE_REPLAY_RECURRENT_PROGRESSED_SESSION_STATE_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayRecurrentProgressedSessionStateMode(
    str,
    Enum,
):
    IMMUTABLE_RECURRENT_PROGRESSED_STATE = "IMMUTABLE_RECURRENT_PROGRESSED_STATE"


class Phase8OfflineReplayRecurrentProgressedSessionLifecycle(
    str,
    Enum,
):
    ACTIVE = "ACTIVE"
...
class Phase8OfflineReplayRecurrentProgressedSessionStateStatus(
    str,
    Enum,
):
    CREATED = "CREATED"
...
class Phase8OfflineReplayRecurrentProgressedSessionStateReason(
    str,
    Enum,
):
    CREATED = "CREATED"
    RECURRENT_TRANSITION_APPLICATION_BLOCKED = "RECURRENT_TRANSITION_APPLICATION_BLOCKED"


class Phase8OfflineReplayRecurrentProgressedSessionStateBlocker(
    str,
    Enum,
):
    RECURRENT_TRANSITION_APPLICATION_BLOCKED = "RECURRENT_TRANSITION_APPLICATION_BLOCKED"
...
class Phase8OfflineReplayRecurrentProgressedSessionStateErrorReason(
    str,
    Enum,
):
    INVALID_RECURRENT_TRANSITION_APPLICATION_DECISION = (
        "INVALID_RECURRENT_TRANSITION_APPLICATION_DECISION"
    )


class Phase8OfflineReplayRecurrentProgressedSessionStateError(
    RuntimeError,
):
    """Structured recurrent progressed-state failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayRecurrentProgressedSessionStateErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayRecurrentProgressedSessionStateErrorReason(reason)
        self.message = str(message)

        super().__init__(
            "Phase 8 recurrent progressed offline "
...
class Phase8OfflineReplayRecurrentProgressedSessionStatePolicy:
    """Strict reusable state requirements after four events."""
...
def _canonical_state_payload(
    *,
    ...
    state_mode: (Phase8OfflineReplayRecurrentProgressedSessionStateMode),
    lifecycle: (Phase8OfflineReplayRecurrentProgressedSessionLifecycle),
    state_version: int,
    cursor_index: int,
    consumed_count: int,
    remaining_count: int,
    ...
    return "\n".join(
        (
            ...
            "RECURRENT_PROGRESSED_STATE_CREATED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayRecurrentProgressedSessionState:
    """
    Immutable reusable session state after four events.

    It represents cursor four, four consumed events and
    796 remaining events without applying another transition.
    """

    recurrent_transition_application_decision: Phase8OfflineReplayRecurrentTransitionApplicationDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayRecurrentProgressedSessionStatePolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.recurrent_transition_application_decision,
            Phase8OfflineReplayRecurrentTransitionApplicationDecision,
        ):
            raise ValueError(
                "recurrent_transition_application_decision "
                "must be a "
                "Phase8OfflineReplayRecurrentTransitionApplicationDecision."
            )

        if not (self.recurrent_transition_application_decision.is_applied):
            raise ValueError(
                "A recurrent progressed session state "
                "requires an applied recurrent-transition "
                "receipt."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayRecurrentProgressedSessionStatePolicy."
            )

        if not self.policy.is_strict:
            raise ValueError(
                "Recurrent progressed-state policy must remain strict and no-lookahead."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current recurrent progressed-state schema."
            )
        ...
        if self.state_mode != (
            ...
            raise ValueError("state_mode must remain IMMUTABLE_RECURRENT_PROGRESSED_STATE.")
        ...
        if not isinstance(
            ...
            raise ValueError(
                "lifecycle must be a Phase8OfflineReplayRecurrentProgressedSessionLifecycle member."
            )

        if self.lifecycle != (Phase8OfflineReplayRecurrentProgressedSessionLifecycle.ACTIVE):
        ...
        if state_version != 4:
            raise ValueError("state_version must remain four after the fourth replay transition.")
        ...
        if completion_reached:
            raise ValueError("The state after four events cannot be complete.")

        application_receipt = self.recurrent_transition_application_decision.receipt_required
        ...
        for field_name, supplied, expected in comparisons:
            if supplied != expected:
                raise ValueError(
                    ...
                    "recurrent-transition application "
                    "lineage."
                )
        ...
        if not _is_gold_symbol(broker_symbol):
            raise ValueError("Recurrent progressed states support Gold/XAUUSD only.")
        ...
        for field_name, supplied, expected in numeric_comparisons:
            if supplied != expected:
                raise ValueError(
                    f"{field_name} must match the recurrent transition-application receipt."
                )
        ...
        if normalized_strings["last_consumed_event_id"] != last_consumed_event.stable_id:
            raise ValueError("last_consumed_event_id must match the fourth consumed event.")

        if normalized_strings["last_consumed_event_digest"] != last_consumed_event.event_digest:
            raise ValueError("last_consumed_event_digest must match the fourth consumed event.")

        if last_consumed_event_time != (last_consumed_event.event_time):
            raise ValueError("last_consumed_event_time must match the fourth consumed event.")

        if self.last_consumed_event_timeframe != (last_consumed_event.timeframe):
            raise ValueError("last_consumed_event_timeframe must match the fourth consumed event.")

        if normalized_strings["next_event_id"] != (next_event.stable_id):
            raise ValueError("next_event_id must match the event at cursor four.")

        if normalized_strings["next_event_digest"] != (next_event.event_digest):
            raise ValueError("next_event_digest must match the event at cursor four.")

        if next_event_time != next_event.event_time:
            raise ValueError("next_event_time must match the event at cursor four.")

        if self.next_event_timeframe != next_event.timeframe:
            raise ValueError("next_event_timeframe must match the event at cursor four.")
        ...
        if last_consumed_sequence_index != (prior_source_state.last_consumed_sequence_index + 1):
            raise ValueError("Recurrent progressed-state sequence continuity is invalid.")

        safe_subjects = (
            self.recurrent_transition_application_decision,
            application_receipt,
            (application_receipt.recurrent_transition_contract_decision),
            transition_contract,
        ...
        if not all(_has_safe_external_boundary(subject) for subject in safe_subjects):
            raise ValueError(
                "Recurrent progressed-state lineage violates the external-I/O or broker boundary."
            )
        ...
        if normalized_strings["state_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "state_digest does not match the canonical recurrent progressed session state."
            )
    ...
    def application_receipt(
        ...
        return self.recurrent_transition_application_decision.receipt_required

    @property
    def recurrent_transition_contract(
        self,
    ) -> StrategyPhase8OfflineReplayRecurrentTransitionContract:
    ...
    @property
    def is_recurrent_progressed_state(self) -> bool:
        return True
    ...
    @property
    def can_continue_to_future_recurrent_transition_contract(
        self,
    ) -> bool:
    ...
    @property
    def recurrent_progressed_state_id(self) -> str:
        return (
    ...
    def stable_id(self) -> str:
        return (
            f"{self.recurrent_transition_application_decision.stable_id}:"
            f"{self.recurrent_progressed_state_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayRecurrentProgressedSessionStateDecision:
    """Immutable recurrent progressed-state decision."""

    recurrent_transition_application_decision: Phase8OfflineReplayRecurrentTransitionApplicationDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayRecurrentProgressedSessionStateStatus
    reason: Phase8OfflineReplayRecurrentProgressedSessionStateReason
    blockers: tuple[
        Phase8OfflineReplayRecurrentProgressedSessionStateBlocker,
        ...,
    ]
    state: StrategyPhase8OfflineReplayRecurrentProgressedSessionState | None = field(repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        try:
            status = Phase8OfflineReplayRecurrentProgressedSessionStateStatus(self.status)
            reason = Phase8OfflineReplayRecurrentProgressedSessionStateReason(self.reason)
        except (TypeError, ValueError) as error:
            raise ValueError("Unsupported recurrent progressed-state status or reason.") from error

        blockers = tuple(
            Phase8OfflineReplayRecurrentProgressedSessionStateBlocker(blocker)
            for blocker in self.blockers
        )

        if len(set(blockers)) != len(blockers):
            raise ValueError("Recurrent progressed-state blockers cannot contain duplicates.")

        if self.recurrent_transition_application_decision.is_blocked:
            if (
                ...
                raise ValueError(
                    "Blocked recurrent progressed-state "
                    "result does not match its application "
                    "decision."
                )
        else:
            if (
                status != (Phase8OfflineReplayRecurrentProgressedSessionStateStatus.CREATED)
                or reason != (Phase8OfflineReplayRecurrentProgressedSessionStateReason.CREATED)
                or blockers
                or not isinstance(
                    self.state,
                ...
                or (
                    self.state.recurrent_transition_application_decision
                    is not self.recurrent_transition_application_decision
                )
            ):
                raise ValueError(
                    "Created recurrent progressed-state "
                    "result does not match its application "
                    "decision."
                )
    ...
    def broker_symbol(self) -> str:
        return self.recurrent_transition_application_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.recurrent_transition_application_decision.direction

    @property
    def is_created(self) -> bool:
        return self.status == (Phase8OfflineReplayRecurrentProgressedSessionStateStatus.CREATED)
    ...
    def state_required(
        ...
        if self.state is None:
            raise ValueError(
                "No Phase 8 recurrent progressed offline replay-session state was created."
            )
...
class StrategyPhase8OfflineReplayRecurrentProgressedSessionStateFactory:
    """Pure immutable recurrent progressed-state factory."""
    ...
    def generate(
        ...
        policy: (Phase8OfflineReplayRecurrentProgressedSessionStatePolicy | None) = None,
    ) -> Phase8OfflineReplayRecurrentProgressedSessionStateDecision:
        if not isinstance(
            recurrent_transition_application_decision,
            Phase8OfflineReplayRecurrentTransitionApplicationDecision,
        ):
            raise Phase8OfflineReplayRecurrentProgressedSessionStateError(
        ...
        selected_policy = policy or Phase8OfflineReplayRecurrentProgressedSessionStatePolicy()
        ...
        if recurrent_transition_application_decision.is_blocked:
            return Phase8OfflineReplayRecurrentProgressedSessionStateDecision(
                recurrent_transition_application_decision=(
                    recurrent_transition_application_decision
                ),
                status=(Phase8OfflineReplayRecurrentProgressedSessionStateStatus.BLOCKED),
        ...
        application_receipt = recurrent_transition_application_decision.receipt_required
        transition_contract = application_receipt.transition_contract
        prior_source_state = application_receipt.source_state
        event_batch = application_receipt.event_batch
        last_consumed_event = application_receipt.consumed_event
        ...
        canonical_payload = _canonical_state_payload(
            ...
            lifecycle=(Phase8OfflineReplayRecurrentProgressedSessionLifecycle.ACTIVE),
            state_version=4,
        ...
        state = StrategyPhase8OfflineReplayRecurrentProgressedSessionState(
            recurrent_transition_application_decision=(recurrent_transition_application_decision),
            policy=selected_policy,
            schema_version=(
                PHASE_8_OFFLINE_REPLAY_RECURRENT_PROGRESSED_SESSION_STATE_SCHEMA_VERSION
            ),
        ...
        return Phase8OfflineReplayRecurrentProgressedSessionStateDecision(
            recurrent_transition_application_decision=(recurrent_transition_application_decision),
            status=(Phase8OfflineReplayRecurrentProgressedSessionStateStatus.CREATED),
            reason=(Phase8OfflineReplayRecurrentProgressedSessionStateReason.CREATED),
            blockers=(),
            state=state,
        )
...
def generate_phase8_offline_replay_recurrent_progressed_session_state(
    recurrent_transition_application_decision: (
        Phase8OfflineReplayRecurrentTransitionApplicationDecision
    ),
    policy: (Phase8OfflineReplayRecurrentProgressedSessionStatePolicy | None) = None,
