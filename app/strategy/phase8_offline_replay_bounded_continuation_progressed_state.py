"""Immutable progressed state after bounded continuation application.

This module converts one allowed bounded continuation application
...
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_PROGRESSED_STATE_SCHEMA_VERSION = "1.0"
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_STATE_ACTIVE = "ACTIVE"
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_STATE_COMPLETED = "COMPLETED"
...
class Phase8OfflineReplayBoundedContinuationProgressedSessionState:
    """Reusable immutable replay state after continuation application."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.lifecycle == PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_STATE_COMPLETED:
        ...
        elif self.lifecycle == PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_STATE_ACTIVE:
    ...
    def state_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_PROGRESSED_STATE:"
            f"SHA256[{self.state_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayBoundedContinuationProgressedSessionStateDecision:
    """Allowed or blocked continuation progressed-state result."""

    is_allowed: bool
    state: Phase8OfflineReplayBoundedContinuationProgressedSessionState | None
    blockers: tuple[str, ...]
    ...
    def state_required(
        ...
        if self.state is None:
            raise RuntimeError("Bounded continuation progressed replay state is blocked.")
        return self.state


class StrategyPhase8OfflineReplayBoundedContinuationProgressedStateFactory:
    """Creates reusable state from one allowed application receipt."""
    ...
    def create(
        ...
        if application_decision is None:
            return Phase8OfflineReplayBoundedContinuationProgressedSessionStateDecision(
                is_allowed=False,
                state=None,
        ...
        lifecycle = (
            ...
            else PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_STATE_ACTIVE
        )

        try:
            state = Phase8OfflineReplayBoundedContinuationProgressedSessionState(
...
def create_phase8_offline_replay_bounded_continuation_progressed_state(
    application_decision: object,
) -> Phase8OfflineReplayBoundedContinuationProgressedSessionStateDecision:
    """Create one immutable reusable continuation progressed state."""

    return StrategyPhase8OfflineReplayBoundedContinuationProgressedStateFactory().create(
        application_decision
    )
