...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationMode(
    str,
    Enum,
):
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationStatus(
    str,
    Enum,
):
    APPLIED = "APPLIED"
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationReason(
    str,
    Enum,
):
    APPLIED = "APPLIED"
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationBlocker(
    str,
    Enum,
):
    ITERATIVE_CONTINUATION_TRANSITION_CONTRACT_BLOCKED = (
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationErrorReason(
    str,
    Enum,
):
    INVALID_ITERATIVE_CONTINUATION_TRANSITION_CONTRACT_DECISION = (
        "INVALID_ITERATIVE_CONTINUATION_TRANSITION_CONTRACT_DECISION"
    )


class Phase8OfflineReplayIterativeContinuationTransitionApplicationError(
    RuntimeError,
):
    """Structured iterative continuation transition-application failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayIterativeContinuationTransitionApplicationErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayIterativeContinuationTransitionApplicationErrorReason(
            reason
        )
...
class Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy:
...
def _canonical_application_payload(
    *,
    ...
    application_mode: (Phase8OfflineReplayIterativeContinuationTransitionApplicationMode),
    ...
    return "\n".join(
        (
            ...
            "ITERATIVE_CONTINUATION_TRANSITION_APPLIED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationReceipt:
    """
    Immutable receipt for consuming replay event sequence five.

    Exactly one closed-candle event is consumed in memory.
    The cursor-five source state remains immutable.
    """

    iterative_continuation_transition_contract_decision: Phase8OfflineReplayIterativeContinuationTransitionContractDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.iterative_continuation_transition_contract_decision,
            Phase8OfflineReplayIterativeContinuationTransitionContractDecision,
        ):
            raise ValueError(
                "iterative_continuation_transition_contract_decision "
                "must be a "
                "Phase8OfflineReplayIterativeContinuationTransitionContractDecision."
            )

        if not self.iterative_continuation_transition_contract_decision.is_created:
            raise ValueError(
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current iterative continuation transition-application schema."
            )
        ...
        if completion_reached:
            raise ValueError("The sixth replay transition cannot complete the 800-event session.")
        ...
        transition_contract = (
            self.iterative_continuation_transition_contract_decision.transition_contract_required
        )
    ...
    def transition_contract(
        ...
        return self.iterative_continuation_transition_contract_decision.transition_contract_required
    ...
    def stable_id(self) -> str:
        return f"{self.iterative_continuation_transition_contract_decision.stable_id}:{self.application_receipt_id}"


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision:
    """Immutable iterative continuation transition-application decision."""

    iterative_continuation_transition_contract_decision: Phase8OfflineReplayIterativeContinuationTransitionContractDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayIterativeContinuationTransitionApplicationStatus
    reason: Phase8OfflineReplayIterativeContinuationTransitionApplicationReason
    blockers: tuple[
        Phase8OfflineReplayIterativeContinuationTransitionApplicationBlocker,
        ...,
    ]
    receipt: StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationReceipt | None = (
        field(repr=False)
    )
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "Unsupported iterative continuation transition-application status or reason."
            ) from error

        blockers = tuple(
            Phase8OfflineReplayIterativeContinuationTransitionApplicationBlocker(blocker)
        ...
        if self.iterative_continuation_transition_contract_decision.is_blocked:
            if (
                status
            ...
            if (
                ...
                or (
                    self.receipt.iterative_continuation_transition_contract_decision
                    is not self.iterative_continuation_transition_contract_decision
                )
            ):
                raise ValueError(
    ...
    def broker_symbol(self) -> str:
        return self.iterative_continuation_transition_contract_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.iterative_continuation_transition_contract_decision.direction
    ...
    def stable_id(self) -> str:
        ...
        return (
            f"{self.iterative_continuation_transition_contract_decision.stable_id}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "CONTINUATION_TRANSITION_APPLICATION:"
            f"{self.status.value}:"
            f"{self.reason.value}:"
...
class StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationFactory:
    """Pure immutable iterative continuation transition-application factory."""
    ...
    def generate(
        ...
        policy: (Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy | None) = None,
    ) -> Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision:
        if not isinstance(
            iterative_continuation_transition_contract_decision,
            Phase8OfflineReplayIterativeContinuationTransitionContractDecision,
        ):
            raise Phase8OfflineReplayIterativeContinuationTransitionApplicationError(
        ...
        selected_policy = (
            policy or Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy()
        )
        ...
        if iterative_continuation_transition_contract_decision.is_blocked:
            return Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision(
                iterative_continuation_transition_contract_decision=(
                    iterative_continuation_transition_contract_decision
                ),
                status=(
        ...
        transition_contract = (
            iterative_continuation_transition_contract_decision.transition_contract_required
        )
        source_state = transition_contract.source_state
        prior_application_receipt = transition_contract.prior_application_receipt
        prior_transition_contract = transition_contract.prior_transition_contract
        ...
        canonical_payload = _canonical_application_payload(
            ...
            iterative_continuation_transition_contract_id=(transition_contract.stable_id),
            iterative_continuation_transition_contract_digest=(
                transition_contract.transition_digest
            ),
        ...
        receipt = StrategyPhase8OfflineReplayIterativeContinuationTransitionApplicationReceipt(
            iterative_continuation_transition_contract_decision=(
                iterative_continuation_transition_contract_decision
            ),
            policy=selected_policy,
        ...
        return Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision(
            ...
            status=(Phase8OfflineReplayIterativeContinuationTransitionApplicationStatus.APPLIED),
            reason=(Phase8OfflineReplayIterativeContinuationTransitionApplicationReason.APPLIED),
            blockers=(),
            receipt=receipt,
        )
...
def apply_phase8_offline_replay_iterative_continuation_transition(
    iterative_continuation_transition_contract_decision: (
        Phase8OfflineReplayIterativeContinuationTransitionContractDecision
    ),
    policy: (Phase8OfflineReplayIterativeContinuationTransitionApplicationPolicy | None) = None,
