"""Immutable subsequent iterative recurrent bounded replay plan.

This module plans one subsequent bounded replay window from the reusable
state produced by the iterative recurrent bounded cycle. It performs
planning only and does not apply transitions, consume events, execute
...
PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT = 32
...
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedPlan:
    """Immutable plan for the next iterative recurrent bounded window."""
    ...
    def plan_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_PLAN:"
            f"SHA256[{self.plan_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedPlanDecision:
    """Allowed or blocked subsequent iterative recurrent planning result."""

    is_allowed: bool
    plan: Phase8OfflineReplaySubsequentIterativeRecurrentBoundedPlan | None
    blockers: tuple[str, ...]
    ...
    def plan_required(
        ...
        if self.plan is None:
            raise RuntimeError("Subsequent iterative recurrent bounded replay plan is blocked.")
        return self.plan


class StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedPlanner:
    """Plans the next iterative recurrent bounded replay window."""
    ...
    def generate(
        ...
        *,
        ...
        if source_decision is None:
            return Phase8OfflineReplaySubsequentIterativeRecurrentBoundedPlanDecision(
                is_allowed=False,
                plan=None,
        ...
        try:
            plan = Phase8OfflineReplaySubsequentIterativeRecurrentBoundedPlan(
...
def generate_phase8_offline_replay_subsequent_iterative_recurrent_bounded_plan(
    source_decision: object,
    *,
    transition_limit: int = (
        PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT
    ...
    """Generate one immutable subsequent iterative recurrent plan."""

    return StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedPlanner().generate(
        source_decision,
        transition_limit=transition_limit,
    )
