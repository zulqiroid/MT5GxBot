"""Immutable successive iterative recurrent bounded offline replay plan.

This module plans one successive bounded replay window from the reusable
state produced by the subsequent iterative recurrent bounded cycle. It
...
PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT = 32
...
class Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlan:
    """Immutable plan for one successive recurrent bounded replay window."""
    ...
    def plan_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_PLAN:"
            f"SHA256[{self.plan_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanDecision:
    """Allowed or blocked successive recurrent bounded planning result."""

    is_allowed: bool
    plan: Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlan | None
    blockers: tuple[str, ...]
    ...
    def plan_required(
        ...
        if self.plan is None:
            raise RuntimeError("Successive iterative recurrent bounded replay plan is blocked.")
        return self.plan


class StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanner:
    """Plans one successive iterative recurrent bounded replay window."""
    ...
    def generate(
        ...
        *,
        ...
        if source_decision is None:
            return Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanDecision(
                is_allowed=False,
                plan=None,
        ...
        try:
            plan = Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlan(
            ...
            return Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanDecision(
                is_allowed=False,
                plan=None,
                blockers=(f"successive_iterative_recurrent_plan_invalid:{type(error).__name__}",),
            )
...
def generate_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
    source_decision: object,
    *,
    transition_limit: int = (
        PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT
    ...
    """Generate one immutable successive iterative recurrent plan."""

    return StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedPlanner().generate(
        source_decision,
        transition_limit=transition_limit,
    )
