"""Immutable iterative recurrent bounded offline replay plan.

This module plans one subsequent bounded replay window from the reusable
state produced by the recurrent bounded cycle. It performs planning only
and does not apply transitions, consume events, execute strategy logic,
...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_PLAN_SCHEMA_VERSION = "1.0"
PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT = 32
...
class Phase8OfflineReplayIterativeRecurrentBoundedPlan:
    """Immutable plan for one iterative recurrent bounded replay window."""
    ...
    def plan_id(self) -> str:
        return f"PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_PLAN:SHA256[{self.plan_digest}]"


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeRecurrentBoundedPlanDecision:
    """Allowed or blocked iterative recurrent bounded planning result."""

    is_allowed: bool
    plan: Phase8OfflineReplayIterativeRecurrentBoundedPlan | None
    blockers: tuple[str, ...]
    ...
    def plan_required(
        ...
        if self.plan is None:
            raise RuntimeError("Iterative recurrent bounded replay plan is blocked.")
        return self.plan


class StrategyPhase8OfflineReplayIterativeRecurrentBoundedPlanner:
    """Plans one iterative recurrent bounded replay window in memory."""

    def generate(
        self,
        source_decision: object,
        *,
        transition_limit: int = (PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT),
    ) -> Phase8OfflineReplayIterativeRecurrentBoundedPlanDecision:
        if source_decision is None:
            return Phase8OfflineReplayIterativeRecurrentBoundedPlanDecision(
                is_allowed=False,
                plan=None,
        ...
        try:
            plan = Phase8OfflineReplayIterativeRecurrentBoundedPlan(
...
def generate_phase8_offline_replay_iterative_recurrent_bounded_plan(
    source_decision: object,
    *,
    transition_limit: int = (PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT),
) -> Phase8OfflineReplayIterativeRecurrentBoundedPlanDecision:
    """Generate one immutable iterative recurrent bounded replay plan."""

    return StrategyPhase8OfflineReplayIterativeRecurrentBoundedPlanner().generate(
        source_decision,
        transition_limit=transition_limit,
    )
