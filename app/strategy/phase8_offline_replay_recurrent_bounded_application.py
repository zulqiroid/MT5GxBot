"""Pure in-memory recurrent bounded offline replay application.

This module applies one allowed recurrent bounded replay plan and
...
PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayRecurrentBoundedApplicationReceipt:
    """Immutable receipt for one recurrent bounded replay application."""
    ...
    def application_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_APPLICATION:"
            f"SHA256[{self.application_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayRecurrentBoundedApplicationDecision:
    """Allowed or blocked recurrent bounded replay application."""

    is_allowed: bool
    receipt: Phase8OfflineReplayRecurrentBoundedApplicationReceipt | None
    blockers: tuple[str, ...]
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise RuntimeError("Recurrent bounded replay application is blocked.")
        return self.receipt


class StrategyPhase8OfflineReplayRecurrentBoundedApplication:
    """Applies one recurrent bounded replay plan in memory."""
    ...
    def apply(
        ...
        if plan_decision is None:
            return Phase8OfflineReplayRecurrentBoundedApplicationDecision(
                is_allowed=False,
                receipt=None,
        ...
        try:
            receipt = Phase8OfflineReplayRecurrentBoundedApplicationReceipt(
            ...
            return Phase8OfflineReplayRecurrentBoundedApplicationDecision(
                is_allowed=False,
                receipt=None,
                blockers=(f"recurrent_application_invalid:{type(error).__name__}",),
            )
...
def apply_phase8_offline_replay_recurrent_bounded_plan(
    plan_decision: object,
) -> Phase8OfflineReplayRecurrentBoundedApplicationDecision:
    """Apply one immutable recurrent bounded replay plan."""

    return StrategyPhase8OfflineReplayRecurrentBoundedApplication().apply(plan_decision)
