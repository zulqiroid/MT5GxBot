...
PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_TRANSITION_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplaySubsequentTransitionApplicationMode(
    str,
    Enum,
):
...
class Phase8OfflineReplaySubsequentTransitionApplicationStatus(
    str,
    Enum,
):
    APPLIED = "APPLIED"
...
class Phase8OfflineReplaySubsequentTransitionApplicationReason(
    str,
    Enum,
):
    APPLIED = "APPLIED"
    SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED = "SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED"


class Phase8OfflineReplaySubsequentTransitionApplicationBlocker(
    str,
    Enum,
):
    SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED = "SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED"
...
class Phase8OfflineReplaySubsequentTransitionApplicationErrorReason(
    str,
    Enum,
):
    INVALID_SUBSEQUENT_TRANSITION_CONTRACT_DECISION = (
        "INVALID_SUBSEQUENT_TRANSITION_CONTRACT_DECISION"
    )


class Phase8OfflineReplaySubsequentTransitionApplicationError(
    RuntimeError,
):
    """Structured subsequent-transition application failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplaySubsequentTransitionApplicationErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplaySubsequentTransitionApplicationErrorReason(reason)
...
class Phase8OfflineReplaySubsequentTransitionApplicationPolicy:
    """Strict pure in-memory subsequent-transition policy."""
...
def _canonical_application_payload(
    *,
    ...
    application_mode: (Phase8OfflineReplaySubsequentTransitionApplicationMode),
    ...
    policy: (Phase8OfflineReplaySubsequentTransitionApplicationPolicy),
) -> str:
    return "\n".join(
        (
            ...
            "SUBSEQUENT_TRANSITION_APPLIED=true",
            "PROGRESSED_SOURCE_STATE_MUTATED=false",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplaySubsequentTransitionApplicationReceipt:
    """
    Immutable receipt for event sequence two consumption.

    It represents one pure in-memory transition and leaves
    the progressed source state unchanged.
    """

    subsequent_transition_contract_decision: Phase8OfflineReplaySubsequentTransitionContractDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplaySubsequentTransitionApplicationPolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.subsequent_transition_contract_decision,
            Phase8OfflineReplaySubsequentTransitionContractDecision,
        ):
            raise ValueError(
                "subsequent_transition_contract_decision must "
                "be a "
                "Phase8OfflineReplaySubsequentTransitionContractDecision."
            )

        if not (self.subsequent_transition_contract_decision.is_created):
            raise ValueError(
                "A subsequent-transition application requires a created transition contract."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplaySubsequentTransitionApplicationPolicy."
            )

        if not self.policy.is_strict:
            raise ValueError(
                "Subsequent-transition application policy must remain strict and no-lookahead."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current subsequent-transition application schema."
            )
        ...
        if self.application_mode != (
            Phase8OfflineReplaySubsequentTransitionApplicationMode.PURE_IMMUTABLE_IN_MEMORY
        ):
        ...
        if completion_reached:
            raise ValueError("The third replay transition cannot complete the 800-event session.")
        ...
        transition_contract = (
            self.subsequent_transition_contract_decision.transition_contract_required
        )
        progressed_state = transition_contract.progressed_state
        ...
        for field_name, supplied, expected in comparisons:
            if supplied != expected:
                raise ValueError(
                    f"{field_name} must match the immutable subsequent-transition lineage."
                )

        broker_symbol = normalized_strings["broker_symbol"]

        if not _is_gold_symbol(broker_symbol):
            raise ValueError("Subsequent-transition applications support Gold/XAUUSD only.")
        ...
        for field_name, supplied, expected in numeric_comparisons:
            if supplied != expected:
                raise ValueError(f"{field_name} must match the subsequent-transition contract.")

        if completion_reached != (transition_contract.completion_after_transition):
            raise ValueError("completion_reached must match the transition contract.")
        ...
        if progressed_state.cursor_index != 2:
            raise ValueError("The immutable progressed source cursor was altered.")

        if progressed_state.consumed_count != 2:
            raise ValueError("The immutable progressed source consumed count was altered.")

        if progressed_state.remaining_count != 798:
            raise ValueError("The immutable progressed source remaining count was altered.")
        ...
        safe_subjects = (
            ...
            (prior_application_receipt.next_transition_contract_decision),
        ...
        if not all(_has_safe_external_boundary(subject) for subject in safe_subjects):
            raise ValueError(
                "Subsequent-transition application lineage "
        ...
        if normalized_strings["application_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "application_digest does not match the canonical subsequent-transition application."
            )
    ...
    def transition_contract(
        ...
        return self.subsequent_transition_contract_decision.transition_contract_required

    @property
    def progressed_state(
        self,
    ) -> StrategyPhase8OfflineReplayProgressedSessionState:
        return self.transition_contract.progressed_state

    @property
    def prior_application_receipt(
        self,
    ) -> StrategyPhase8OfflineReplayNextTransitionApplicationReceipt:
        return self.transition_contract.application_receipt
    ...
    @property
    def progressed_source_state_preserved(self) -> bool:
        return True
    ...
    @property
    def can_continue_to_subsequent_progressed_state(
        self,
    ) -> bool:
        return True
    ...
    def stable_id(self) -> str:
        return (
            f"{self.subsequent_transition_contract_decision.stable_id}:"
            f"{self.application_receipt_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySubsequentTransitionApplicationDecision:
    """Immutable subsequent-transition application decision."""

    subsequent_transition_contract_decision: Phase8OfflineReplaySubsequentTransitionContractDecision = field(
        repr=False
    )
    status: Phase8OfflineReplaySubsequentTransitionApplicationStatus
    reason: Phase8OfflineReplaySubsequentTransitionApplicationReason
    blockers: tuple[
        Phase8OfflineReplaySubsequentTransitionApplicationBlocker,
        ...,
    ]
    receipt: StrategyPhase8OfflineReplaySubsequentTransitionApplicationReceipt | None = field(
        repr=False
    )
    ...
    def __post_init__(self) -> None:
        ...
        try:
            status = Phase8OfflineReplaySubsequentTransitionApplicationStatus(self.status)
            reason = Phase8OfflineReplaySubsequentTransitionApplicationReason(self.reason)
        except (TypeError, ValueError) as error:
            raise ValueError(
                "Unsupported subsequent-transition application status or reason."
            ) from error

        blockers = tuple(
            Phase8OfflineReplaySubsequentTransitionApplicationBlocker(blocker)
            for blocker in self.blockers
        )

        if len(set(blockers)) != len(blockers):
            raise ValueError(
                "Subsequent-transition application blockers cannot contain duplicates."
            )

        if self.subsequent_transition_contract_decision.is_blocked:
            if (
                status != (Phase8OfflineReplaySubsequentTransitionApplicationStatus.BLOCKED)
                or reason
                != (
                    Phase8OfflineReplaySubsequentTransitionApplicationReason.SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED
                )
                or blockers
                != (
                    Phase8OfflineReplaySubsequentTransitionApplicationBlocker.SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED,
                )
                or self.receipt is not None
            ):
                raise ValueError(
                    "Blocked subsequent-transition "
                    "application does not match its "
                    "contract decision."
                )
        else:
            if (
                status != (Phase8OfflineReplaySubsequentTransitionApplicationStatus.APPLIED)
                or reason != (Phase8OfflineReplaySubsequentTransitionApplicationReason.APPLIED)
                or blockers
                or not isinstance(
                    self.receipt,
                ...
                or (
                    self.receipt.subsequent_transition_contract_decision
                    is not self.subsequent_transition_contract_decision
                )
            ):
                raise ValueError(
                    "Applied subsequent-transition result does not match its contract decision."
                )
    ...
    def broker_symbol(self) -> str:
        return self.subsequent_transition_contract_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.subsequent_transition_contract_decision.direction

    @property
    def is_applied(self) -> bool:
        return self.status == (Phase8OfflineReplaySubsequentTransitionApplicationStatus.APPLIED)
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise ValueError(
                "No Phase 8 subsequent offline replay-transition application receipt was created."
            )
...
class StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory:
    """Pure immutable subsequent-transition application factory."""

    def generate(
        self,
        subsequent_transition_contract_decision: (
            Phase8OfflineReplaySubsequentTransitionContractDecision
        ),
        policy: (Phase8OfflineReplaySubsequentTransitionApplicationPolicy | None) = None,
        ...
        if not isinstance(
            ...
            raise (
                Phase8OfflineReplaySubsequentTransitionApplicationError(
                    Phase8OfflineReplaySubsequentTransitionApplicationErrorReason.INVALID_SUBSEQUENT_TRANSITION_CONTRACT_DECISION,
                    "subsequent_transition_contract_decision "
                    "must be a "
                    "Phase8OfflineReplaySubsequentTransitionContractDecision.",
                )
        ...
        selected_policy = policy or (Phase8OfflineReplaySubsequentTransitionApplicationPolicy())
        ...
        if subsequent_transition_contract_decision.is_blocked:
            return Phase8OfflineReplaySubsequentTransitionApplicationDecision(
                subsequent_transition_contract_decision=(subsequent_transition_contract_decision),
                status=(Phase8OfflineReplaySubsequentTransitionApplicationStatus.BLOCKED),
                reason=(
                    Phase8OfflineReplaySubsequentTransitionApplicationReason.SUBSEQUENT_TRANSITION_CONTRACT_BLOCKED
                ),
        ...
        transition_contract = subsequent_transition_contract_decision.transition_contract_required
        progressed_state = transition_contract.progressed_state
        prior_application_receipt = transition_contract.application_receipt
        event_batch = transition_contract.event_batch
        consumed_event = transition_contract.current_event
        ...
        receipt = StrategyPhase8OfflineReplaySubsequentTransitionApplicationReceipt(
            subsequent_transition_contract_decision=(subsequent_transition_contract_decision),
            policy=selected_policy,
            schema_version=(
                PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_TRANSITION_APPLICATION_SCHEMA_VERSION
        ...
        return Phase8OfflineReplaySubsequentTransitionApplicationDecision(
            subsequent_transition_contract_decision=(subsequent_transition_contract_decision),
            status=(Phase8OfflineReplaySubsequentTransitionApplicationStatus.APPLIED),
            reason=(Phase8OfflineReplaySubsequentTransitionApplicationReason.APPLIED),
            blockers=(),
            receipt=receipt,
        )
...
def apply_phase8_offline_replay_subsequent_transition(
    subsequent_transition_contract_decision: (
        Phase8OfflineReplaySubsequentTransitionContractDecision
    ),
    policy: (Phase8OfflineReplaySubsequentTransitionApplicationPolicy | None) = None,
) -> Phase8OfflineReplaySubsequentTransitionApplicationDecision:
    return StrategyPhase8OfflineReplaySubsequentTransitionApplicationFactory().generate(
        subsequent_transition_contract_decision,
        policy,
    )
