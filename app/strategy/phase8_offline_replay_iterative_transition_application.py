...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_TRANSITION_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayIterativeTransitionApplicationMode(
    str,
    Enum,
):
...
class Phase8OfflineReplayIterativeTransitionApplicationStatus(
    str,
    Enum,
):
    APPLIED = "APPLIED"
...
class Phase8OfflineReplayIterativeTransitionApplicationReason(
    str,
    Enum,
):
    APPLIED = "APPLIED"
    ITERATIVE_TRANSITION_CONTRACT_BLOCKED = "ITERATIVE_TRANSITION_CONTRACT_BLOCKED"


class Phase8OfflineReplayIterativeTransitionApplicationBlocker(
    str,
    Enum,
):
    ITERATIVE_TRANSITION_CONTRACT_BLOCKED = "ITERATIVE_TRANSITION_CONTRACT_BLOCKED"
...
class Phase8OfflineReplayIterativeTransitionApplicationErrorReason(
    str,
    Enum,
):
    INVALID_ITERATIVE_TRANSITION_CONTRACT_DECISION = (
        "INVALID_ITERATIVE_TRANSITION_CONTRACT_DECISION"
    )


class Phase8OfflineReplayIterativeTransitionApplicationError(
    RuntimeError,
):
    """Structured iterative transition-application failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayIterativeTransitionApplicationErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayIterativeTransitionApplicationErrorReason(reason)
...
class Phase8OfflineReplayIterativeTransitionApplicationPolicy:
    """Strict pure in-memory iterative-transition rules."""
...
def _canonical_application_payload(
    *,
    ...
    application_mode: (Phase8OfflineReplayIterativeTransitionApplicationMode),
    ...
    policy: (Phase8OfflineReplayIterativeTransitionApplicationPolicy),
) -> str:
    return "\n".join(
        (
            ...
            "ITERATIVE_TRANSITION_APPLIED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayIterativeTransitionApplicationReceipt:
    """
    Immutable receipt for consuming replay event sequence four.

    Exactly one closed-candle event is consumed in memory.
    The cursor-four source state remains immutable.
    """

    iterative_transition_contract_decision: Phase8OfflineReplayIterativeTransitionContractDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayIterativeTransitionApplicationPolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.iterative_transition_contract_decision,
            Phase8OfflineReplayIterativeTransitionContractDecision,
        ):
            raise ValueError(
                "iterative_transition_contract_decision "
                "must be a "
                "Phase8OfflineReplayIterativeTransitionContractDecision."
            )

        if not self.iterative_transition_contract_decision.is_created:
            raise ValueError(
                "An iterative transition application requires a created iterative contract."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayIterativeTransitionApplicationPolicy."
            )

        if not self.policy.is_strict:
            raise ValueError(
                "Iterative transition-application policy must remain strict and no-lookahead."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current iterative transition-application schema."
            )
        ...
        if self.application_mode != (
            Phase8OfflineReplayIterativeTransitionApplicationMode.PURE_IMMUTABLE_IN_MEMORY
        ):
        ...
        if completion_reached:
            raise ValueError("The fifth replay transition cannot complete the 800-event session.")
        ...
        transition_contract = (
            self.iterative_transition_contract_decision.transition_contract_required
        )
        ...
        if not _is_gold_symbol(broker_symbol):
            raise ValueError("Iterative transition applications support Gold/XAUUSD only.")

        if self.direction != transition_contract.direction:
            raise ValueError("direction must match the iterative transition contract.")

        if self.side != transition_contract.side:
            raise ValueError("side must match the iterative transition contract.")

        if captured_at != transition_contract.captured_at:
            raise ValueError("captured_at must match the iterative transition contract.")

        if self.timeframes != transition_contract.timeframes:
            raise ValueError("timeframes must match the iterative transition contract.")
        ...
        for field_name, supplied, expected in numeric_comparisons:
            if supplied != expected:
                raise ValueError(f"{field_name} must match the iterative transition contract.")

        if completion_reached != (transition_contract.completion_after_transition):
            raise ValueError("completion_reached must match the iterative transition contract.")
        ...
        if normalized_strings["consumed_event_id"] != (consumed_event.stable_id):
            raise ValueError("consumed_event_id must match the iterative contract current event.")

        if normalized_strings["consumed_event_digest"] != (consumed_event.event_digest):
            raise ValueError(
                "consumed_event_digest must match the iterative contract current event."
            )

        if consumed_event_time != consumed_event.event_time:
            raise ValueError("consumed_event_time must match the iterative contract current event.")

        if self.consumed_event_timeframe != (consumed_event.timeframe):
            raise ValueError(
                "consumed_event_timeframe must match the iterative contract current event."
            )
        ...
        if not all(_has_safe_external_boundary(subject) for subject in safe_subjects):
            raise ValueError(
                "Iterative transition-application lineage "
        ...
        if normalized_strings["application_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "application_digest does not match the canonical iterative transition application."
            )
    ...
    def transition_contract(
        ...
        return self.iterative_transition_contract_decision.transition_contract_required
    ...
    def prior_transition_contract(
        ...
        return self.transition_contract.prior_transition_contract
    ...
    @property
    def can_continue_to_iterative_progressed_state(
        self,
    ) -> bool:
        return True
    ...
    def stable_id(self) -> str:
        return (
            f"{self.iterative_transition_contract_decision.stable_id}:{self.application_receipt_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeTransitionApplicationDecision:
    """Immutable iterative transition-application decision."""

    iterative_transition_contract_decision: Phase8OfflineReplayIterativeTransitionContractDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayIterativeTransitionApplicationStatus
    reason: Phase8OfflineReplayIterativeTransitionApplicationReason
    blockers: tuple[
        Phase8OfflineReplayIterativeTransitionApplicationBlocker,
        ...,
    ]
    receipt: StrategyPhase8OfflineReplayIterativeTransitionApplicationReceipt | None = field(
        repr=False
    )
    ...
    def __post_init__(self) -> None:
        ...
        try:
            status = Phase8OfflineReplayIterativeTransitionApplicationStatus(self.status)
            reason = Phase8OfflineReplayIterativeTransitionApplicationReason(self.reason)
        except (TypeError, ValueError) as error:
            raise ValueError(
                "Unsupported iterative transition-application status or reason."
            ) from error

        blockers = tuple(
            Phase8OfflineReplayIterativeTransitionApplicationBlocker(blocker)
            for blocker in self.blockers
        )

        if len(set(blockers)) != len(blockers):
            raise ValueError("Iterative transition-application blockers cannot contain duplicates.")

        if self.iterative_transition_contract_decision.is_blocked:
            if (
                status != (Phase8OfflineReplayIterativeTransitionApplicationStatus.BLOCKED)
                or reason
                != (
                    Phase8OfflineReplayIterativeTransitionApplicationReason.ITERATIVE_TRANSITION_CONTRACT_BLOCKED
                )
                or blockers
                != (
                    Phase8OfflineReplayIterativeTransitionApplicationBlocker.ITERATIVE_TRANSITION_CONTRACT_BLOCKED,
                )
                or self.receipt is not None
            ):
                raise ValueError(
                    "Blocked iterative transition application does not match its contract decision."
                )
        else:
            if (
                status != (Phase8OfflineReplayIterativeTransitionApplicationStatus.APPLIED)
                or reason != (Phase8OfflineReplayIterativeTransitionApplicationReason.APPLIED)
                or blockers
                or not isinstance(
                    self.receipt,
                ...
                or (
                    self.receipt.iterative_transition_contract_decision
                    is not self.iterative_transition_contract_decision
                )
            ):
                raise ValueError(
                    "Applied iterative transition result does not match its contract decision."
                )
    ...
    def broker_symbol(self) -> str:
        return self.iterative_transition_contract_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.iterative_transition_contract_decision.direction

    @property
    def is_applied(self) -> bool:
        return self.status == (Phase8OfflineReplayIterativeTransitionApplicationStatus.APPLIED)
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise ValueError(
                "No Phase 8 iterative offline replay-transition application receipt was created."
            )
    ...
    def stable_id(self) -> str:
        ...
        return (
            f"{self.iterative_transition_contract_decision.stable_id}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "TRANSITION_APPLICATION:"
            f"{self.status.value}:"
            f"{self.reason.value}:"
...
class StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory:
    """Pure immutable iterative transition-application factory."""
    ...
    def generate(
        ...
        policy: (Phase8OfflineReplayIterativeTransitionApplicationPolicy | None) = None,
    ) -> Phase8OfflineReplayIterativeTransitionApplicationDecision:
        if not isinstance(
            iterative_transition_contract_decision,
            Phase8OfflineReplayIterativeTransitionContractDecision,
        ):
            raise Phase8OfflineReplayIterativeTransitionApplicationError(
                Phase8OfflineReplayIterativeTransitionApplicationErrorReason.INVALID_ITERATIVE_TRANSITION_CONTRACT_DECISION,
                "iterative_transition_contract_decision "
                "must be a "
                "Phase8OfflineReplayIterativeTransitionContractDecision.",
            )

        selected_policy = policy or Phase8OfflineReplayIterativeTransitionApplicationPolicy()
        ...
        if iterative_transition_contract_decision.is_blocked:
            return Phase8OfflineReplayIterativeTransitionApplicationDecision(
                iterative_transition_contract_decision=(iterative_transition_contract_decision),
                status=(Phase8OfflineReplayIterativeTransitionApplicationStatus.BLOCKED),
                reason=(
                    Phase8OfflineReplayIterativeTransitionApplicationReason.ITERATIVE_TRANSITION_CONTRACT_BLOCKED
                ),
        ...
        transition_contract = iterative_transition_contract_decision.transition_contract_required
        source_state = transition_contract.source_state
        prior_application_receipt = transition_contract.prior_application_receipt
        prior_transition_contract = transition_contract.prior_transition_contract
        event_batch = transition_contract.event_batch
        ...
        canonical_payload = _canonical_application_payload(
            schema_version=(PHASE_8_OFFLINE_REPLAY_ITERATIVE_TRANSITION_APPLICATION_SCHEMA_VERSION),
            iterative_transition_contract_id=(transition_contract.stable_id),
            iterative_transition_contract_digest=(transition_contract.transition_digest),
        ...
        receipt = StrategyPhase8OfflineReplayIterativeTransitionApplicationReceipt(
            iterative_transition_contract_decision=(iterative_transition_contract_decision),
            policy=selected_policy,
            schema_version=(PHASE_8_OFFLINE_REPLAY_ITERATIVE_TRANSITION_APPLICATION_SCHEMA_VERSION),
            iterative_transition_contract_id=(transition_contract.stable_id),
        ...
        return Phase8OfflineReplayIterativeTransitionApplicationDecision(
            iterative_transition_contract_decision=(iterative_transition_contract_decision),
            status=(Phase8OfflineReplayIterativeTransitionApplicationStatus.APPLIED),
            reason=(Phase8OfflineReplayIterativeTransitionApplicationReason.APPLIED),
            blockers=(),
            receipt=receipt,
        )
...
def apply_phase8_offline_replay_iterative_transition(
    iterative_transition_contract_decision: (
        Phase8OfflineReplayIterativeTransitionContractDecision
    ),
    policy: (Phase8OfflineReplayIterativeTransitionApplicationPolicy | None) = None,
) -> Phase8OfflineReplayIterativeTransitionApplicationDecision:
    return StrategyPhase8OfflineReplayIterativeTransitionApplicationFactory().generate(
        iterative_transition_contract_decision,
        policy,
    )
