...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_PROGRESSED_SESSION_STATE_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayIterativeProgressedSessionStateMode(
    str,
    Enum,
):
    IMMUTABLE_ITERATIVE_PROGRESSED_STATE = "IMMUTABLE_ITERATIVE_PROGRESSED_STATE"


class Phase8OfflineReplayIterativeProgressedSessionLifecycle(
    str,
    Enum,
):
    ACTIVE = "ACTIVE"
...
class Phase8OfflineReplayIterativeProgressedSessionStateStatus(
    str,
    Enum,
):
    CREATED = "CREATED"
...
class Phase8OfflineReplayIterativeProgressedSessionStateReason(
    str,
    Enum,
):
    CREATED = "CREATED"
    ITERATIVE_TRANSITION_APPLICATION_BLOCKED = "ITERATIVE_TRANSITION_APPLICATION_BLOCKED"


class Phase8OfflineReplayIterativeProgressedSessionStateBlocker(
    str,
    Enum,
):
    ITERATIVE_TRANSITION_APPLICATION_BLOCKED = "ITERATIVE_TRANSITION_APPLICATION_BLOCKED"
...
class Phase8OfflineReplayIterativeProgressedSessionStateErrorReason(
    str,
    Enum,
):
    INVALID_ITERATIVE_TRANSITION_APPLICATION_DECISION = (
        "INVALID_ITERATIVE_TRANSITION_APPLICATION_DECISION"
    )


class Phase8OfflineReplayIterativeProgressedSessionStateError(
    RuntimeError,
):
    """Structured iterative progressed-state failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayIterativeProgressedSessionStateErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayIterativeProgressedSessionStateErrorReason(reason)
...
class Phase8OfflineReplayIterativeProgressedSessionStatePolicy:
    """Strict reusable state requirements after five events."""
...
def _canonical_state_payload(
    *,
    ...
    iterative_transition_contract_id: str,
    iterative_transition_contract_digest: str,
    ...
    state_mode: (Phase8OfflineReplayIterativeProgressedSessionStateMode),
    lifecycle: (Phase8OfflineReplayIterativeProgressedSessionLifecycle),
    state_version: int,
    cursor_index: int,
    consumed_count: int,
    remaining_count: int,
    ...
    policy: (Phase8OfflineReplayIterativeProgressedSessionStatePolicy),
) -> str:
    return "\n".join(
        (
            ...
            (f"ITERATIVE_TRANSITION_CONTRACT_ID={iterative_transition_contract_id}"),
            (f"ITERATIVE_TRANSITION_CONTRACT_DIGEST={iterative_transition_contract_digest}"),
            ...
            "ITERATIVE_PROGRESSED_STATE_CREATED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayIterativeProgressedSessionState:
    """
    Immutable reusable session state after five events.

    It represents cursor five, five consumed events and
    795 remaining events without applying another transition.
    """

    iterative_transition_application_decision: Phase8OfflineReplayIterativeTransitionApplicationDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayIterativeProgressedSessionStatePolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.iterative_transition_application_decision,
            Phase8OfflineReplayIterativeTransitionApplicationDecision,
        ):
            raise ValueError(
                "iterative_transition_application_decision "
                "must be a "
                "Phase8OfflineReplayIterativeTransitionApplicationDecision."
            )

        if not (self.iterative_transition_application_decision.is_applied):
            raise ValueError(
                "An iterative progressed session state "
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayIterativeProgressedSessionStatePolicy."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current iterative progressed-state schema."
            )
        ...
        string_fields = (
            ...
            (
                "iterative_transition_contract_id",
                self.iterative_transition_contract_id,
            ),
            (
                "iterative_transition_contract_digest",
                self.iterative_transition_contract_digest,
            ),
            (
        ...
        if self.state_mode != (
            Phase8OfflineReplayIterativeProgressedSessionStateMode.IMMUTABLE_ITERATIVE_PROGRESSED_STATE
        ):
            raise ValueError("state_mode must remain IMMUTABLE_ITERATIVE_PROGRESSED_STATE.")
        ...
        if not isinstance(
            ...
            raise ValueError(
                "lifecycle must be a Phase8OfflineReplayIterativeProgressedSessionLifecycle member."
            )

        if self.lifecycle != (Phase8OfflineReplayIterativeProgressedSessionLifecycle.ACTIVE):
        ...
        if state_version != 5:
            raise ValueError("state_version must remain five after the fifth replay transition.")
        ...
        if completion_reached:
            raise ValueError("The state after five events cannot be complete.")

        application_receipt = self.iterative_transition_application_decision.receipt_required
        iterative_transition_contract = application_receipt.transition_contract
        ...
        comparisons = (
            ...
            (
                "iterative_transition_contract_id",
                normalized_strings["iterative_transition_contract_id"],
                iterative_transition_contract.stable_id,
            ),
            (
                "iterative_transition_contract_digest",
                normalized_strings["iterative_transition_contract_digest"],
                iterative_transition_contract.transition_digest,
            ),
            (
                "prior_source_state_id",
        ...
        if normalized_strings["last_consumed_event_id"] != last_consumed_event.stable_id:
            raise ValueError("last_consumed_event_id must match the fifth consumed event.")

        if normalized_strings["last_consumed_event_digest"] != last_consumed_event.event_digest:
            raise ValueError("last_consumed_event_digest must match the fifth consumed event.")

        if last_consumed_event_time != (last_consumed_event.event_time):
            raise ValueError("last_consumed_event_time must match the fifth consumed event.")

        if self.last_consumed_event_timeframe != (last_consumed_event.timeframe):
            raise ValueError("last_consumed_event_timeframe must match the fifth consumed event.")

        if normalized_strings["next_event_id"] != (next_event.stable_id):
            raise ValueError("next_event_id must match the event at cursor five.")

        if normalized_strings["next_event_digest"] != (next_event.event_digest):
            raise ValueError("next_event_digest must match the event at cursor five.")

        if next_event_time != next_event.event_time:
            raise ValueError("next_event_time must match the event at cursor five.")

        if self.next_event_timeframe != next_event.timeframe:
            raise ValueError("next_event_timeframe must match the event at cursor five.")
        ...
        safe_subjects = (
            self.iterative_transition_application_decision,
            application_receipt,
            (application_receipt.iterative_transition_contract_decision),
            iterative_transition_contract,
            (iterative_transition_contract.recurrent_progressed_state_decision),
            prior_source_state,
            (prior_source_state.recurrent_transition_application_decision),
        ...
        if normalized_strings["state_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "state_digest does not match the canonical iterative progressed session state."
            )
    ...
    def application_receipt(
        ...
        return self.iterative_transition_application_decision.receipt_required

    @property
    def iterative_transition_contract(
        self,
    ) -> StrategyPhase8OfflineReplayIterativeTransitionContract:
    ...
    def canonical_payload(self) -> str:
        return _canonical_state_payload(
            ...
            application_digest=self.application_digest,
            iterative_transition_contract_id=(self.iterative_transition_contract_id),
            iterative_transition_contract_digest=(self.iterative_transition_contract_digest),
    ...
    @property
    def is_iterative_progressed_state(self) -> bool:
        return True
    ...
    @property
    def can_continue_to_future_iterative_transition_contract(
        self,
    ) -> bool:
    ...
    @property
    def iterative_progressed_state_id(self) -> str:
        return (
    ...
    def stable_id(self) -> str:
        return (
            f"{self.iterative_transition_application_decision.stable_id}:"
            f"{self.iterative_progressed_state_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeProgressedSessionStateDecision:
    """Immutable iterative progressed-state decision."""

    iterative_transition_application_decision: Phase8OfflineReplayIterativeTransitionApplicationDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayIterativeProgressedSessionStateStatus
    reason: Phase8OfflineReplayIterativeProgressedSessionStateReason
    blockers: tuple[
        Phase8OfflineReplayIterativeProgressedSessionStateBlocker,
        ...,
    ]
    state: StrategyPhase8OfflineReplayIterativeProgressedSessionState | None = field(repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        try:
            status = Phase8OfflineReplayIterativeProgressedSessionStateStatus(self.status)
            reason = Phase8OfflineReplayIterativeProgressedSessionStateReason(self.reason)
        except (TypeError, ValueError) as error:
            raise ValueError("Unsupported iterative progressed-state status or reason.") from error

        blockers = tuple(
            Phase8OfflineReplayIterativeProgressedSessionStateBlocker(blocker)
        ...
        if self.iterative_transition_application_decision.is_blocked:
            if (
                status != (Phase8OfflineReplayIterativeProgressedSessionStateStatus.BLOCKED)
                or reason
                != (
                    Phase8OfflineReplayIterativeProgressedSessionStateReason.ITERATIVE_TRANSITION_APPLICATION_BLOCKED
                )
                or blockers
                != (
                    Phase8OfflineReplayIterativeProgressedSessionStateBlocker.ITERATIVE_TRANSITION_APPLICATION_BLOCKED,
                )
                or self.state is not None
            ):
                raise ValueError(
                    "Blocked iterative progressed-state "
                    "result does not match its application "
                    "decision."
                )
        else:
            if (
                status != (Phase8OfflineReplayIterativeProgressedSessionStateStatus.CREATED)
                or reason != (Phase8OfflineReplayIterativeProgressedSessionStateReason.CREATED)
                or blockers
                or not isinstance(
                    self.state,
                ...
                or (
                    self.state.iterative_transition_application_decision
                    is not self.iterative_transition_application_decision
                )
            ):
                raise ValueError(
                    "Created iterative progressed-state "
                    "result does not match its application "
                    "decision."
                )
    ...
    def broker_symbol(self) -> str:
        return self.iterative_transition_application_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.iterative_transition_application_decision.direction

    @property
    def is_created(self) -> bool:
        return self.status == (Phase8OfflineReplayIterativeProgressedSessionStateStatus.CREATED)
    ...
    def state_required(
        ...
        if self.state is None:
            raise ValueError(
                "No Phase 8 iterative progressed offline replay-session state was created."
            )
...
class StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory:
    """Pure immutable iterative progressed-state factory."""
    ...
    def generate(
        ...
        policy: (Phase8OfflineReplayIterativeProgressedSessionStatePolicy | None) = None,
    ) -> Phase8OfflineReplayIterativeProgressedSessionStateDecision:
        if not isinstance(
            iterative_transition_application_decision,
            Phase8OfflineReplayIterativeTransitionApplicationDecision,
        ):
            raise Phase8OfflineReplayIterativeProgressedSessionStateError(
                Phase8OfflineReplayIterativeProgressedSessionStateErrorReason.INVALID_ITERATIVE_TRANSITION_APPLICATION_DECISION,
                "iterative_transition_application_decision "
                "must be a "
                "Phase8OfflineReplayIterativeTransitionApplicationDecision.",
            )

        selected_policy = policy or Phase8OfflineReplayIterativeProgressedSessionStatePolicy()
        ...
        if iterative_transition_application_decision.is_blocked:
            return Phase8OfflineReplayIterativeProgressedSessionStateDecision(
                iterative_transition_application_decision=(
                    iterative_transition_application_decision
                ),
                status=(Phase8OfflineReplayIterativeProgressedSessionStateStatus.BLOCKED),
        ...
        application_receipt = iterative_transition_application_decision.receipt_required
        iterative_transition_contract = application_receipt.transition_contract
        prior_source_state = application_receipt.source_state
        prior_application_receipt = application_receipt.prior_application_receipt
        prior_transition_contract = application_receipt.prior_transition_contract
        ...
        canonical_payload = _canonical_state_payload(
            ...
            iterative_transition_contract_id=(iterative_transition_contract.stable_id),
            iterative_transition_contract_digest=(iterative_transition_contract.transition_digest),
            ...
            lifecycle=(Phase8OfflineReplayIterativeProgressedSessionLifecycle.ACTIVE),
            state_version=5,
        ...
        state = StrategyPhase8OfflineReplayIterativeProgressedSessionState(
            iterative_transition_application_decision=(iterative_transition_application_decision),
            policy=selected_policy,
            schema_version=(
                PHASE_8_OFFLINE_REPLAY_ITERATIVE_PROGRESSED_SESSION_STATE_SCHEMA_VERSION
            ),
        ...
        return Phase8OfflineReplayIterativeProgressedSessionStateDecision(
            iterative_transition_application_decision=(iterative_transition_application_decision),
            status=(Phase8OfflineReplayIterativeProgressedSessionStateStatus.CREATED),
            reason=(Phase8OfflineReplayIterativeProgressedSessionStateReason.CREATED),
            blockers=(),
            state=state,
        )
...
def generate_phase8_offline_replay_iterative_progressed_session_state(
    iterative_transition_application_decision: (
        Phase8OfflineReplayIterativeTransitionApplicationDecision
    ),
    policy: (Phase8OfflineReplayIterativeProgressedSessionStatePolicy | None) = None,
) -> Phase8OfflineReplayIterativeProgressedSessionStateDecision:
    return StrategyPhase8OfflineReplayIterativeProgressedSessionStateFactory().generate(
        iterative_transition_application_decision,
        policy,
    )
