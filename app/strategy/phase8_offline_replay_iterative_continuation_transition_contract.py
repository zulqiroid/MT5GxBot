...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_CONTINUATION_TRANSITION_CONTRACT_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayIterativeContinuationTransitionContractMode(
    str,
    Enum,
):
    IMMUTABLE_SINGLE_EVENT_CONTINUATION = "IMMUTABLE_SINGLE_EVENT_CONTINUATION"


class Phase8OfflineReplayIterativeContinuationTransitionAction(
    str,
    Enum,
):
...
class Phase8OfflineReplayIterativeContinuationTransitionContractStatus(
    str,
    Enum,
):
    CREATED = "CREATED"
...
class Phase8OfflineReplayIterativeContinuationTransitionContractReason(
    str,
    Enum,
):
    CREATED = "CREATED"
    ITERATIVE_PROGRESSED_STATE_BLOCKED = "ITERATIVE_PROGRESSED_STATE_BLOCKED"


class Phase8OfflineReplayIterativeContinuationTransitionContractBlocker(
    str,
    Enum,
):
    ITERATIVE_PROGRESSED_STATE_BLOCKED = "ITERATIVE_PROGRESSED_STATE_BLOCKED"
...
class Phase8OfflineReplayIterativeContinuationTransitionContractErrorReason(
    str,
    Enum,
):
    INVALID_ITERATIVE_PROGRESSED_STATE_DECISION = "INVALID_ITERATIVE_PROGRESSED_STATE_DECISION"


class Phase8OfflineReplayIterativeContinuationTransitionContractError(
    RuntimeError,
):
    """Structured iterative continuation-contract failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayIterativeContinuationTransitionContractErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayIterativeContinuationTransitionContractErrorReason(reason)
        self.message = str(message)

        super().__init__(
            "Phase 8 iterative continuation offline "
            f"replay-transition contract error [{self.reason.value}]: "
            f"{self.message}"
        )
...
class Phase8OfflineReplayIterativeContinuationTransitionContractPolicy:
    """Strict immutable single-event continuation rules."""

    source_state_immutable: bool = True
    source_state_active: bool = True
    current_event_bound: bool = True
    sequence_continuity_verified: bool = True
    consume_exactly_one_future_event: bool = True
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            "consume_exactly_one_future_event",
    ...
    def is_strict(self) -> bool:
        return all(
            (
                ...
                self.source_state_active,
                self.current_event_bound,
                self.sequence_continuity_verified,
                self.consume_exactly_one_future_event,
...
def _canonical_transition_payload(
    *,
    ...
    contract_mode: (Phase8OfflineReplayIterativeContinuationTransitionContractMode),
    action: (Phase8OfflineReplayIterativeContinuationTransitionAction),
    ...
    policy: (Phase8OfflineReplayIterativeContinuationTransitionContractPolicy),
) -> str:
    return "\n".join(
        (
            ...
            (f"SOURCE_STATE_ACTIVE={str(policy.source_state_active).lower()}"),
            ...
            (
                "CONSUME_EXACTLY_ONE_FUTURE_EVENT="
                f"{str(policy.consume_exactly_one_future_event).lower()}"
            ),
            ...
            "ITERATIVE_CONTINUATION_CONTRACT_CREATED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayIterativeContinuationTransitionContract:
    """
    Immutable continuation contract for event sequence five.

    It defines one future in-memory transition from cursor
    five to cursor six without applying that transition.
    """

    iterative_progressed_state_decision: Phase8OfflineReplayIterativeProgressedSessionStateDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayIterativeContinuationTransitionContractPolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.iterative_progressed_state_decision,
            Phase8OfflineReplayIterativeProgressedSessionStateDecision,
        ):
            raise ValueError(
                "iterative_progressed_state_decision must "
                "be a "
                "Phase8OfflineReplayIterativeProgressedSessionStateDecision."
            )

        if not self.iterative_progressed_state_decision.is_created:
            raise ValueError(
                "An iterative continuation contract requires a created iterative progressed state."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayIterativeContinuationTransitionContractPolicy."
            )

        if not self.policy.is_strict:
            raise ValueError("Iterative continuation policy must remain strict and no-lookahead.")
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current iterative continuation-contract schema."
            )
        ...
        if self.contract_mode != (
            Phase8OfflineReplayIterativeContinuationTransitionContractMode.IMMUTABLE_SINGLE_EVENT_CONTINUATION
        ):
            raise ValueError("contract_mode must remain IMMUTABLE_SINGLE_EVENT_CONTINUATION.")
        ...
        if not isinstance(
            ...
            raise ValueError(
                "action must be a Phase8OfflineReplayIterativeContinuationTransitionAction member."
            )

        if self.action != (
            Phase8OfflineReplayIterativeContinuationTransitionAction.CONSUME_CURRENT_EVENT
        ):
        ...
        if completion_after_transition:
            raise ValueError("The sixth transition cannot complete this 800-event replay session.")
        ...
        source_state = self.iterative_progressed_state_decision.state_required
        prior_application_receipt = source_state.application_receipt
        prior_transition_contract = source_state.iterative_transition_contract
        ...
        for field_name, supplied, expected in comparisons:
            if supplied != expected:
                raise ValueError(
                    f"{field_name} must match the immutable iterative continuation lineage."
                )

        broker_symbol = normalized_strings["broker_symbol"]

        if not _is_gold_symbol(broker_symbol):
            raise ValueError("Iterative continuation contracts support Gold/XAUUSD only.")
        ...
        for field_name, supplied, expected in state_comparisons:
            if supplied != expected:
                raise ValueError(f"{field_name} must match the iterative progressed source state.")

        if source_state.state_version != 5:
            raise ValueError(
                "The continuation source state must represent exactly five consumed events."
            )

        if source_state.cursor_index != 5:
            raise ValueError("The continuation source cursor must remain five.")

        if not source_state.session_active:
            raise ValueError("The continuation source state must remain active.")

        if source_state.completion_reached:
            raise ValueError("A completed source state cannot create a continuation contract.")

        if not source_state.has_next_event:
            raise ValueError("The continuation source state has no next event.")
        ...
        safe_subjects = (
            self.iterative_progressed_state_decision,
            source_state,
            source_state.iterative_transition_application_decision,
            prior_application_receipt,
            prior_application_receipt.iterative_transition_contract_decision,
            prior_transition_contract,
            prior_transition_contract.recurrent_progressed_state_decision,
        ...
        if not all(_has_safe_external_boundary(subject) for subject in safe_subjects):
            raise ValueError(
                "Iterative continuation lineage violates the external-I/O or broker boundary."
            )
        ...
        if normalized_strings["transition_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "transition_digest does not match the canonical iterative continuation contract."
            )
    ...
    def source_state(
        ...
        return self.iterative_progressed_state_decision.state_required
    ...
    def prior_transition_contract(
        ...
        return self.source_state.iterative_transition_contract
    ...
    @property
    def can_continue_to_iterative_continuation_application(
        self,
    ) -> bool:
        return True
    ...
    @property
    def continuation_transition_contract_id(self) -> str:
        return (
            f"{self.broker_symbol}:"
            f"{self.side.value}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "CONTINUATION_TRANSITION_CONTRACT:"
    ...
    def stable_id(self) -> str:
        return (
            f"{self.iterative_progressed_state_decision.stable_id}:"
            f"{self.continuation_transition_contract_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeContinuationTransitionContractDecision:
    """Immutable iterative continuation-contract decision."""

    iterative_progressed_state_decision: Phase8OfflineReplayIterativeProgressedSessionStateDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayIterativeContinuationTransitionContractStatus
    reason: Phase8OfflineReplayIterativeContinuationTransitionContractReason
    blockers: tuple[
        Phase8OfflineReplayIterativeContinuationTransitionContractBlocker,
        ...,
    ]
    transition_contract: (
        StrategyPhase8OfflineReplayIterativeContinuationTransitionContract | None
    ) = field(repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        try:
            status = Phase8OfflineReplayIterativeContinuationTransitionContractStatus(self.status)
            reason = Phase8OfflineReplayIterativeContinuationTransitionContractReason(self.reason)
        except (TypeError, ValueError) as error:
            raise ValueError(
                "Unsupported iterative continuation-contract status or reason."
            ) from error

        blockers = tuple(
            Phase8OfflineReplayIterativeContinuationTransitionContractBlocker(blocker)
            for blocker in self.blockers
        )

        if len(set(blockers)) != len(blockers):
            raise ValueError("Iterative continuation-contract blockers cannot contain duplicates.")

        if self.iterative_progressed_state_decision.is_blocked:
            if (
                status != (Phase8OfflineReplayIterativeContinuationTransitionContractStatus.BLOCKED)
                or reason
                != (
                    Phase8OfflineReplayIterativeContinuationTransitionContractReason.ITERATIVE_PROGRESSED_STATE_BLOCKED
                )
                or blockers
                != (
                    Phase8OfflineReplayIterativeContinuationTransitionContractBlocker.ITERATIVE_PROGRESSED_STATE_BLOCKED,
                )
                or self.transition_contract is not None
            ):
                raise ValueError(
                    "Blocked iterative continuation result does not match its source state."
                )
        else:
            if (
                status != (Phase8OfflineReplayIterativeContinuationTransitionContractStatus.CREATED)
                or reason
                != (Phase8OfflineReplayIterativeContinuationTransitionContractReason.CREATED)
                or blockers
                or not isinstance(
                ...
                or (
                    self.transition_contract.iterative_progressed_state_decision
                    is not self.iterative_progressed_state_decision
                )
            ):
                raise ValueError(
                    "Created iterative continuation result does not match its source state."
                )
    ...
    def broker_symbol(self) -> str:
        return self.iterative_progressed_state_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.iterative_progressed_state_decision.direction

    @property
    def is_created(self) -> bool:
        return self.status == (
            Phase8OfflineReplayIterativeContinuationTransitionContractStatus.CREATED
        )
    ...
    def transition_contract_required(
        ...
        if self.transition_contract is None:
            raise ValueError(
                "No Phase 8 iterative continuation offline replay-transition contract was created."
            )
    ...
    def stable_id(self) -> str:
        ...
        return (
            f"{self.iterative_progressed_state_decision.stable_id}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "CONTINUATION_CONTRACT_GENERATION:"
            f"{self.status.value}:"
            f"{self.reason.value}:"
            f"{blocker_fragment}"
        )
...
class StrategyPhase8OfflineReplayIterativeContinuationTransitionContractFactory:
    """Pure immutable iterative continuation-contract factory."""

    def generate(
        self,
        iterative_progressed_state_decision: (
            Phase8OfflineReplayIterativeProgressedSessionStateDecision
        ),
        policy: (Phase8OfflineReplayIterativeContinuationTransitionContractPolicy | None) = None,
    ) -> Phase8OfflineReplayIterativeContinuationTransitionContractDecision:
        if not isinstance(
            iterative_progressed_state_decision,
            Phase8OfflineReplayIterativeProgressedSessionStateDecision,
        ):
            raise Phase8OfflineReplayIterativeContinuationTransitionContractError(
                Phase8OfflineReplayIterativeContinuationTransitionContractErrorReason.INVALID_ITERATIVE_PROGRESSED_STATE_DECISION,
                "iterative_progressed_state_decision must "
                "be a "
                "Phase8OfflineReplayIterativeProgressedSessionStateDecision.",
            )
        ...
        selected_policy = (
            policy or Phase8OfflineReplayIterativeContinuationTransitionContractPolicy()
        )
        ...
        if iterative_progressed_state_decision.is_blocked:
            return Phase8OfflineReplayIterativeContinuationTransitionContractDecision(
                iterative_progressed_state_decision=(iterative_progressed_state_decision),
                status=(Phase8OfflineReplayIterativeContinuationTransitionContractStatus.BLOCKED),
                reason=(
                    Phase8OfflineReplayIterativeContinuationTransitionContractReason.ITERATIVE_PROGRESSED_STATE_BLOCKED
                ),
        ...
        source_state = iterative_progressed_state_decision.state_required
        prior_application_receipt = source_state.application_receipt
        prior_transition_contract = source_state.iterative_transition_contract
        event_batch = source_state.event_batch
        current_event = source_state.next_event
        ...
        canonical_payload = _canonical_transition_payload(
            ...
            action=(Phase8OfflineReplayIterativeContinuationTransitionAction.CONSUME_CURRENT_EVENT),
        ...
        transition_contract = StrategyPhase8OfflineReplayIterativeContinuationTransitionContract(
            iterative_progressed_state_decision=(iterative_progressed_state_decision),
            policy=selected_policy,
            schema_version=(
                PHASE_8_OFFLINE_REPLAY_ITERATIVE_CONTINUATION_TRANSITION_CONTRACT_SCHEMA_VERSION
        ...
        return Phase8OfflineReplayIterativeContinuationTransitionContractDecision(
            iterative_progressed_state_decision=(iterative_progressed_state_decision),
            status=(Phase8OfflineReplayIterativeContinuationTransitionContractStatus.CREATED),
            reason=(Phase8OfflineReplayIterativeContinuationTransitionContractReason.CREATED),
            blockers=(),
...
def generate_phase8_offline_replay_iterative_continuation_transition_contract(
    iterative_progressed_state_decision: (
        Phase8OfflineReplayIterativeProgressedSessionStateDecision
    ),
    policy: (Phase8OfflineReplayIterativeContinuationTransitionContractPolicy | None) = None,
