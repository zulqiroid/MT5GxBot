"""Immutable progressed state after iterative recurrent bounded application.

This module converts one allowed iterative recurrent bounded application
receipt into one reusable immutable replay session state. It does not
apply another transition, consume additional events, evaluate strategy
...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_STATE_ACTIVE = "ACTIVE"
PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_STATE_COMPLETED = "COMPLETED"
...
class Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionState:
    """Reusable immutable state after iterative recurrent application."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.lifecycle == PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_STATE_COMPLETED:
        ...
        elif self.lifecycle == PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_STATE_ACTIVE:
    ...
    def state_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_PROGRESSED_STATE:"
            f"SHA256[{self.state_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionStateDecision:
    """Allowed or blocked iterative recurrent progressed-state result."""

    is_allowed: bool
    state: Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionState | None
    blockers: tuple[str, ...]
    ...
    def state_required(
        ...
        if self.state is None:
            raise RuntimeError("Iterative recurrent bounded progressed replay state is blocked.")
        return self.state


class StrategyPhase8OfflineReplayIterativeRecurrentBoundedProgressedStateFactory:
    """Creates reusable state from one allowed iterative recurrent receipt."""
    ...
    def create(
        ...
        if application_decision is None:
            return Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionStateDecision(
                is_allowed=False,
                state=None,
        ...
        lifecycle = (
            ...
            else PHASE_8_OFFLINE_REPLAY_ITERATIVE_RECURRENT_BOUNDED_STATE_ACTIVE
        )

        try:
            state = Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionState(
...
def create_phase8_offline_replay_iterative_recurrent_bounded_progressed_state(
    application_decision: object,
) -> Phase8OfflineReplayIterativeRecurrentBoundedProgressedSessionStateDecision:
    """Create one immutable reusable iterative recurrent progressed state."""

    return StrategyPhase8OfflineReplayIterativeRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
