"""Immutable bounded continuation replay plan.

This module plans the next bounded replay window from one allowed
bounded progressed replay state. It performs planning only and does not
apply transitions, consume events, execute strategy logic, initialize
...
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_PLAN_SCHEMA_VERSION = "1.0"
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_DEFAULT_LIMIT = 32
...
class Phase8OfflineReplayBoundedContinuationPlan:
    """Immutable plan for one subsequent bounded replay window."""
    ...
    def plan_id(self) -> str:
        return f"PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_PLAN:SHA256[{self.plan_digest}]"


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayBoundedContinuationPlanDecision:
    """Allowed or blocked bounded continuation planning result."""

    is_allowed: bool
    plan: Phase8OfflineReplayBoundedContinuationPlan | None
    blockers: tuple[str, ...]
    ...
    @property
    def plan_required(self) -> Phase8OfflineReplayBoundedContinuationPlan:
        if self.plan is None:
            raise RuntimeError("Bounded continuation replay plan is blocked.")
        return self.plan


class StrategyPhase8OfflineReplayBoundedContinuationPlanner:
    """Plans one subsequent bounded replay window in memory."""

    def generate(
        self,
        source_decision: object,
        *,
        transition_limit: int = (PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_DEFAULT_LIMIT),
    ) -> Phase8OfflineReplayBoundedContinuationPlanDecision:
        if source_decision is None:
            return Phase8OfflineReplayBoundedContinuationPlanDecision(
                is_allowed=False,
                plan=None,
        ...
        try:
            plan = Phase8OfflineReplayBoundedContinuationPlan(
            ...
            return Phase8OfflineReplayBoundedContinuationPlanDecision(
                is_allowed=False,
                plan=None,
                blockers=(f"continuation_plan_invalid:{type(error).__name__}",),
            )
...
def generate_phase8_offline_replay_bounded_continuation_plan(
    source_decision: object,
    *,
    transition_limit: int = (PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_DEFAULT_LIMIT),
) -> Phase8OfflineReplayBoundedContinuationPlanDecision:
    """Generate one immutable bounded continuation replay plan."""

    return StrategyPhase8OfflineReplayBoundedContinuationPlanner().generate(
        source_decision,
        transition_limit=transition_limit,
    )
