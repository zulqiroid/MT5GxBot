"""Immutable continued successive iterative recurrent bounded replay plan.

This module plans one continued bounded replay window from the reusable
state produced by the successive iterative recurrent bounded cycle. It
performs planning only and does not apply transitions, consume events,
execute strategy logic, initialize MT5, contact a broker, write external
...
PHASE_8_OFFLINE_REPLAY_CONTINUED_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_PLAN_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlan:
    """Immutable plan for one continued recurrent bounded replay window."""
    ...
    def plan_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_CONTINUED_SUCCESSIVE_ITERATIVE_RECURRENT_"
            f"BOUNDED_PLAN:SHA256[{self.plan_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlanDecision:
    """Allowed or blocked continued recurrent bounded planning result."""

    is_allowed: bool
    plan: Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlan | None
    blockers: tuple[str, ...]
    ...
    def plan_required(
        ...
        if self.plan is None:
            raise RuntimeError(
                "Continued successive iterative recurrent bounded replay plan is blocked."
            )
        return self.plan


class StrategyPhase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlanner:
    """Plans one continued successive recurrent bounded replay window."""
    ...
    def generate(
        ...
        *,
        ...
        if source_decision is None:
            return Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlanDecision(
                is_allowed=False,
                plan=None,
        ...
        try:
            state = _required_attribute(source_decision, "state_required")
            ...
            total_event_count = _required_int(state, "total_event_count")
        ...
        try:
            plan = Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlan(
            ...
            return Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedPlanDecision(
                ...
                plan=None,
                blockers=(
                    f"continued_successive_iterative_recurrent_plan_invalid:{type(error).__name__}",
                ),
            )
...
def generate_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_plan(
    source_decision: object,
    *,
    transition_limit: int = (
        PHASE_8_OFFLINE_REPLAY_CONTINUED_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_DEFAULT_LIMIT
    ...
    """Generate one immutable continued successive recurrent plan."""
