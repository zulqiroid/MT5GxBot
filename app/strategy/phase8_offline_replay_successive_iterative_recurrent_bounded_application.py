"""Pure in-memory successive iterative recurrent bounded replay application.
...
PHASE_8_OFFLINE_REPLAY_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationReceipt:
    """Immutable receipt for one successive recurrent bounded application."""
...
@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationDecision:
    """Allowed or blocked successive recurrent bounded application."""

    is_allowed: bool
    receipt: Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationReceipt | None
    blockers: tuple[str, ...]
...
class StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplication:
    """Applies one successive iterative recurrent bounded replay plan."""
    ...
    def apply(
        ...
        if plan_decision is None:
            return Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationDecision(
                is_allowed=False,
                receipt=None,
        ...
        try:
            receipt = Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationReceipt(
            ...
            return Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationDecision(
                ...
                receipt=None,
                blockers=(
                    f"successive_iterative_recurrent_application_invalid:{type(error).__name__}",
                ),
            )
...
def apply_phase8_offline_replay_successive_iterative_recurrent_bounded_plan(
    plan_decision: object,
) -> Phase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplicationDecision:
    """Apply one immutable successive iterative recurrent bounded plan."""

    return StrategyPhase8OfflineReplaySuccessiveIterativeRecurrentBoundedApplication().apply(
        plan_decision
    )
