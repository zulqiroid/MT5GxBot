"""Pure in-memory subsequent iterative recurrent bounded replay application.

This module applies one allowed subsequent iterative recurrent bounded
replay plan and creates only an immutable application receipt. It does
not create a reusable replay state, evaluate strategy logic, initialize
MT5, contact a broker, write external state, or submit orders.
"""
...
PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationReceipt:
    """Immutable receipt for one subsequent iterative recurrent application."""
    ...
    def application_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_APPLICATION:"
            f"SHA256[{self.application_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationDecision:
    """Allowed or blocked subsequent iterative recurrent application."""

    is_allowed: bool
    receipt: Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationReceipt | None
    blockers: tuple[str, ...]
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise RuntimeError(
                "Subsequent iterative recurrent bounded replay application is blocked."
            )
        return self.receipt


class StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedApplication:
    """Applies one subsequent iterative recurrent bounded replay plan."""
    ...
    def apply(
        ...
        if plan_decision is None:
            return Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationDecision(
                is_allowed=False,
                receipt=None,
        ...
        try:
            ...
            return Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationDecision(
                is_allowed=False,
                receipt=None,
                blockers=(f"subsequent_iterative_recurrent_plan_invalid:{type(error).__name__}",),
            )
        ...
        try:
            receipt = Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationReceipt(
            ...
            return Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationDecision(
                ...
                receipt=None,
                blockers=(
                    f"subsequent_iterative_recurrent_application_invalid:{type(error).__name__}",
                ),
            )
...
def apply_phase8_offline_replay_subsequent_iterative_recurrent_bounded_plan(
    plan_decision: object,
) -> Phase8OfflineReplaySubsequentIterativeRecurrentBoundedApplicationDecision:
    """Apply one immutable subsequent iterative recurrent bounded plan."""

    return StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedApplication().apply(
        plan_decision
    )
