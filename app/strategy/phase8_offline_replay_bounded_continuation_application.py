"""Pure in-memory bounded continuation replay application.

This module applies one allowed bounded continuation replay plan and
creates only an immutable application receipt. It does not create a
...
PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayBoundedContinuationApplicationReceipt:
    """Immutable receipt for one bounded continuation application."""
    ...
    def application_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_BOUNDED_CONTINUATION_APPLICATION:"
            f"SHA256[{self.application_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayBoundedContinuationApplicationDecision:
    """Allowed or blocked bounded continuation application result."""

    is_allowed: bool
    receipt: Phase8OfflineReplayBoundedContinuationApplicationReceipt | None
    blockers: tuple[str, ...]
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise RuntimeError("Bounded continuation replay application is blocked.")
        return self.receipt


class StrategyPhase8OfflineReplayBoundedContinuationApplication:
    """Applies one bounded continuation plan in memory."""
    ...
    def apply(
        ...
        if plan_decision is None:
            return Phase8OfflineReplayBoundedContinuationApplicationDecision(
                is_allowed=False,
                receipt=None,
        ...
        try:
            receipt = Phase8OfflineReplayBoundedContinuationApplicationReceipt(
            ...
            return Phase8OfflineReplayBoundedContinuationApplicationDecision(
                is_allowed=False,
                receipt=None,
                blockers=(f"continuation_application_invalid:{type(error).__name__}",),
            )
...
def apply_phase8_offline_replay_bounded_continuation_plan(
    plan_decision: object,
) -> Phase8OfflineReplayBoundedContinuationApplicationDecision:
    """Apply one immutable bounded continuation replay plan."""

    return StrategyPhase8OfflineReplayBoundedContinuationApplication().apply(plan_decision)
