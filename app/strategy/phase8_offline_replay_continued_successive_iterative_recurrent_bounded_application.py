"""Pure in-memory continued successive iterative recurrent bounded replay application.

This module applies one allowed successive iterative recurrent bounded
...
PHASE_8_OFFLINE_REPLAY_CONTINUED_SUCCESSIVE_ITERATIVE_RECURRENT_BOUNDED_APPLICATION_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationReceipt:
    """Immutable receipt for one continued successive recurrent bounded application."""
    ...
    def application_id(self) -> str:
        return (
            ...
            f"APPLICATION:SHA256[{self.application_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationDecision:
    """Allowed or blocked continued successive recurrent bounded application."""

    is_allowed: bool
    receipt: (
        Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationReceipt | None
    )
    blockers: tuple[str, ...]
    ...
    def receipt_required(
        ...
        if self.receipt is None:
            raise RuntimeError(
                "Successive iterative recurrent bounded replay application is blocked."
            )
        return self.receipt


class StrategyPhase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplication:
    """Applies one continued successive iterative recurrent bounded replay plan."""
        ...
        try:
            ...
            return (
                Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationDecision(
                    ...
                    receipt=None,
                    blockers=(
                        f"successive_iterative_recurrent_plan_invalid:{type(error).__name__}",
                    ),
                )
            )
        ...
        try:
            ...
            return (
                Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationDecision(
                    ...
                    receipt=None,
                    blockers=(
                        "successive_iterative_recurrent_application_invalid:"
                        f"{type(error).__name__}",
                    ),
                )
            )

        return Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationDecision(
            is_allowed=True,
            receipt=receipt,
            blockers=(),
        )
...
def apply_phase8_offline_replay_continued_successive_iterative_recurrent_bounded_plan(
    plan_decision: object,
) -> Phase8OfflineReplayContinuedSuccessiveIterativeRecurrentBoundedApplicationDecision:
    """Apply one immutable successive iterative recurrent bounded plan."""
