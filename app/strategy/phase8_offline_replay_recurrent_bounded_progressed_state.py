"""Immutable progressed state after recurrent bounded application.

This module converts one allowed recurrent bounded application receipt
into one reusable immutable replay session state. It does not apply
another transition, consume additional events, evaluate strategy logic,
...
PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_PROGRESSED_STATE_SCHEMA_VERSION = "1.0"
PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_ACTIVE = "ACTIVE"
PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_COMPLETED = "COMPLETED"
...
class Phase8OfflineReplayRecurrentBoundedProgressedSessionState:
    """Reusable immutable replay state after recurrent application."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.lifecycle == PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_COMPLETED:
        ...
        elif self.lifecycle == PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_ACTIVE:
    ...
    def state_id(self) -> str:
        return (
            f"PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_PROGRESSED_STATE:SHA256[{self.state_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayRecurrentBoundedProgressedSessionStateDecision:
    """Allowed or blocked recurrent bounded progressed-state result."""

    is_allowed: bool
    state: Phase8OfflineReplayRecurrentBoundedProgressedSessionState | None
    blockers: tuple[str, ...]
    ...
    def state_required(
        ...
        if self.state is None:
            raise RuntimeError("Recurrent bounded progressed replay state is blocked.")
        return self.state


class StrategyPhase8OfflineReplayRecurrentBoundedProgressedStateFactory:
    """Creates reusable state from one allowed recurrent receipt."""
    ...
    def create(
        ...
        if application_decision is None:
            return Phase8OfflineReplayRecurrentBoundedProgressedSessionStateDecision(
                is_allowed=False,
                state=None,
        ...
        lifecycle = (
            ...
            else PHASE_8_OFFLINE_REPLAY_RECURRENT_BOUNDED_STATE_ACTIVE
        )

        try:
            state = Phase8OfflineReplayRecurrentBoundedProgressedSessionState(
...
def create_phase8_offline_replay_recurrent_bounded_progressed_state(
    application_decision: object,
) -> Phase8OfflineReplayRecurrentBoundedProgressedSessionStateDecision:
    """Create one immutable reusable recurrent progressed state."""

    return StrategyPhase8OfflineReplayRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
