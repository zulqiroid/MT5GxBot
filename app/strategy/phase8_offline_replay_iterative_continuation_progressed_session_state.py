...
PHASE_8_OFFLINE_REPLAY_ITERATIVE_CONTINUATION_PROGRESSED_SESSION_STATE_SCHEMA_VERSION = "1.0"
...
class Phase8OfflineReplayIterativeContinuationProgressedSessionStateMode(
    str,
    Enum,
):
    IMMUTABLE_ITERATIVE_CONTINUATION_PROGRESSED_STATE = (
        "IMMUTABLE_ITERATIVE_CONTINUATION_PROGRESSED_STATE"
    )


class Phase8OfflineReplayIterativeContinuationProgressedSessionLifecycle(
    str,
    Enum,
):
    ACTIVE = "ACTIVE"
...
class Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus(
    str,
    Enum,
):
    CREATED = "CREATED"
...
class Phase8OfflineReplayIterativeContinuationProgressedSessionStateReason(
    str,
    Enum,
):
    CREATED = "CREATED"
    ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_BLOCKED = (
        "ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_BLOCKED"
    )


class Phase8OfflineReplayIterativeContinuationProgressedSessionStateBlocker(
    str,
    Enum,
):
    ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_BLOCKED = (
...
class Phase8OfflineReplayIterativeContinuationProgressedSessionStateErrorReason(
    str,
    Enum,
):
    INVALID_ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_DECISION = (
        "INVALID_ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_DECISION"
    )


class Phase8OfflineReplayIterativeContinuationProgressedSessionStateError(
    RuntimeError,
):
    """Structured iterative continuation progressed-state failure."""

    def __init__(
        self,
        reason: (Phase8OfflineReplayIterativeContinuationProgressedSessionStateErrorReason),
        message: str,
    ) -> None:
        self.reason = Phase8OfflineReplayIterativeContinuationProgressedSessionStateErrorReason(
            reason
        )
        self.message = str(message)

        super().__init__(
            "Phase 8 iterative progressed offline "
...
class Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy:
    """Strict reusable state requirements after six events."""

    application_receipt_verified: bool = True
    prior_source_state_immutable: bool = True
    ...
    def is_strict(self) -> bool:
        return all(
            (
                ...
                self.prior_source_state_immutable,
...
def _canonical_state_payload(
    *,
    ...
    iterative_continuation_transition_contract_id: str,
    iterative_continuation_transition_contract_digest: str,
    prior_source_state_id: str,
    prior_source_state_digest: str,
    ...
    state_mode: (Phase8OfflineReplayIterativeContinuationProgressedSessionStateMode),
    lifecycle: (Phase8OfflineReplayIterativeContinuationProgressedSessionLifecycle),
    state_version: int,
    cursor_index: int,
    consumed_count: int,
    remaining_count: int,
    ...
    policy: (Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy),
) -> str:
    return "\n".join(
        (
            ...
            (f"ITERATIVE_TRANSITION_CONTRACT_ID={iterative_continuation_transition_contract_id}"),
            (
                f"ITERATIVE_TRANSITION_CONTRACT_DIGEST={iterative_continuation_transition_contract_digest}"
            ),
            f"PRIOR_SOURCE_STATE_ID={prior_source_state_id}",
            (f"PRIOR_SOURCE_STATE_DIGEST={prior_source_state_digest}"),
            ...
            (f"PRIOR_SOURCE_STATE_IMMUTABLE={str(policy.prior_source_state_immutable).lower()}"),
            ...
            "ITERATIVE_CONTINUATION_PROGRESSED_STATE_CREATED=true",
...
@dataclass(frozen=True, slots=True)
class StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionState:
    """
    Immutable reusable session state after six events.

    It represents cursor six, six consumed events and
    794 remaining events without applying another transition.
    """

    iterative_continuation_transition_application_decision: Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision = field(
        repr=False
    )
    policy: Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy
    schema_version: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.iterative_continuation_transition_application_decision,
            Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision,
        ):
            raise ValueError(
                "iterative_continuation_transition_application_decision "
                "must be a "
                "Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision."
            )

        if not (self.iterative_continuation_transition_application_decision.is_applied):
            raise ValueError(
                "An iterative continuation progressed session state "
                "requires an applied iterative-transition "
                "receipt."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "policy must be a Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy."
            )

        if not self.policy.is_strict:
            raise ValueError(
                "Iterative progressed-state policy must remain strict and no-lookahead."
            )
        ...
        if schema_version != (
            ...
            raise ValueError(
                "schema_version must match the current iterative continuation progressed-state schema."
            )
        ...
        string_fields = (
            ...
            (
                ...
                self.iterative_continuation_transition_contract_id,
            ),
            (
                "iterative_continuation_transition_contract_digest",
                self.iterative_continuation_transition_contract_digest,
            ),
            (
                "prior_source_state_id",
                self.prior_source_state_id,
            ),
            (
                "prior_source_state_digest",
                self.prior_source_state_digest,
            ),
            (
        ...
        if self.state_mode != (
            Phase8OfflineReplayIterativeContinuationProgressedSessionStateMode.IMMUTABLE_ITERATIVE_CONTINUATION_PROGRESSED_STATE
        ):
            raise ValueError(
                "state_mode must remain IMMUTABLE_ITERATIVE_CONTINUATION_PROGRESSED_STATE."
            )
        ...
        if not isinstance(
            ...
            raise ValueError(
                "lifecycle must be a Phase8OfflineReplayIterativeContinuationProgressedSessionLifecycle member."
            )

        if self.lifecycle != (
            Phase8OfflineReplayIterativeContinuationProgressedSessionLifecycle.ACTIVE
        ):
        ...
        if state_version != 6:
            raise ValueError("state_version must remain five after the sixth replay transition.")
        ...
        if completion_reached:
            raise ValueError("The state after six events cannot be complete.")

        application_receipt = (
            self.iterative_continuation_transition_application_decision.receipt_required
        )
        iterative_continuation_transition_contract = application_receipt.transition_contract
        prior_source_state = application_receipt.source_state
        ...
        comparisons = (
            ...
            (
                "iterative_continuation_transition_contract_id",
                normalized_strings["iterative_continuation_transition_contract_id"],
                iterative_continuation_transition_contract.stable_id,
            ),
            (
                "iterative_continuation_transition_contract_digest",
                normalized_strings["iterative_continuation_transition_contract_digest"],
                iterative_continuation_transition_contract.transition_digest,
            ),
            (
                "prior_source_state_id",
                normalized_strings["prior_source_state_id"],
                prior_source_state.stable_id,
            ),
            (
                "prior_source_state_digest",
                normalized_strings["prior_source_state_digest"],
                prior_source_state.state_digest,
            ),
            (
                "prior_application_receipt_id",
        ...
        for field_name, supplied, expected in comparisons:
            if supplied != expected:
                raise ValueError(
                    ...
                    "iterative-transition application "
                    "lineage."
                )
        ...
        if not _is_gold_symbol(broker_symbol):
            raise ValueError("Iterative progressed states support Gold/XAUUSD only.")
        ...
        for field_name, supplied, expected in numeric_comparisons:
            if supplied != expected:
                raise ValueError(
                    f"{field_name} must match the iterative transition-application receipt."
                )
        ...
        if normalized_strings["last_consumed_event_id"] != last_consumed_event.stable_id:
            raise ValueError("last_consumed_event_id must match the sixth consumed event.")

        if normalized_strings["last_consumed_event_digest"] != last_consumed_event.event_digest:
            raise ValueError("last_consumed_event_digest must match the sixth consumed event.")

        if last_consumed_event_time != (last_consumed_event.event_time):
            raise ValueError("last_consumed_event_time must match the sixth consumed event.")

        if self.last_consumed_event_timeframe != (last_consumed_event.timeframe):
            raise ValueError("last_consumed_event_timeframe must match the sixth consumed event.")

        if normalized_strings["next_event_id"] != (next_event.stable_id):
            raise ValueError("next_event_id must match the event at cursor six.")

        if normalized_strings["next_event_digest"] != (next_event.event_digest):
            raise ValueError("next_event_digest must match the event at cursor six.")

        if next_event_time != next_event.event_time:
            raise ValueError("next_event_time must match the event at cursor six.")

        if self.next_event_timeframe != next_event.timeframe:
            raise ValueError("next_event_timeframe must match the event at cursor six.")
        ...
        if prior_source_state.state_version != 5:
            raise ValueError("The immutable prior source state version was altered.")

        if prior_source_state.cursor_index != 5:
            raise ValueError("The immutable prior source cursor was altered.")

        if prior_source_state.consumed_count != 5:
            raise ValueError("The immutable prior source consumed count was altered.")

        if prior_source_state.remaining_count != 795:
            raise ValueError("The immutable prior source remaining count was altered.")

        if last_consumed_sequence_index != (prior_source_state.last_consumed_sequence_index + 1):
            raise ValueError("Iterative progressed-state sequence continuity is invalid.")

        safe_subjects = (
            self.iterative_continuation_transition_application_decision,
            application_receipt,
            (application_receipt.iterative_continuation_transition_contract_decision),
            iterative_continuation_transition_contract,
            (iterative_continuation_transition_contract.iterative_progressed_state_decision),
            prior_source_state,
            (prior_source_state.iterative_transition_application_decision),
            prior_application_receipt,
            (prior_application_receipt.iterative_transition_contract_decision),
        ...
        if not all(_has_safe_external_boundary(subject) for subject in safe_subjects):
            raise ValueError(
                "Iterative progressed-state lineage violates the external-I/O or broker boundary."
            )
        ...
        canonical_payload = _canonical_state_payload(
            ...
            prior_source_state_id=normalized_strings["prior_source_state_id"],
            prior_source_state_digest=normalized_strings["prior_source_state_digest"],
        ...
        if normalized_strings["state_digest"] != (_sha256_digest(canonical_payload)):
            raise ValueError(
                "state_digest does not match the canonical iterative continuation progressed session state."
            )
    ...
    def application_receipt(
        ...
        return self.iterative_continuation_transition_application_decision.receipt_required

    @property
    def iterative_continuation_transition_contract(
        self,
    ) -> StrategyPhase8OfflineReplayIterativeContinuationTransitionContract:
        return self.application_receipt.transition_contract

    @property
    def prior_source_state(
        self,
    ) -> StrategyPhase8OfflineReplayIterativeProgressedSessionState:
    ...
    def canonical_payload(self) -> str:
        return _canonical_state_payload(
            ...
            prior_source_state_id=(self.prior_source_state_id),
            prior_source_state_digest=(self.prior_source_state_digest),
    ...
    @property
    def is_iterative_continuation_progressed_state(self) -> bool:
        return True
    ...
    @property
    def can_continue_to_future_iterative_continuation_transition_contract(
        self,
    ) -> bool:
    ...
    @property
    def iterative_continuation_progressed_state_id(self) -> str:
        return (
            f"{self.broker_symbol}:"
            f"{self.side.value}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "CONTINUATION_PROGRESSED_SESSION_STATE:"
    ...
    def stable_id(self) -> str:
        return (
            f"{self.iterative_continuation_transition_application_decision.stable_id}:"
            f"{self.iterative_continuation_progressed_state_id}"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplayIterativeContinuationProgressedSessionStateDecision:
    """Immutable iterative continuation progressed-state decision."""

    iterative_continuation_transition_application_decision: Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision = field(
        repr=False
    )
    status: Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus
    reason: Phase8OfflineReplayIterativeContinuationProgressedSessionStateReason
    blockers: tuple[
        Phase8OfflineReplayIterativeContinuationProgressedSessionStateBlocker,
        ...,
    ]
    state: StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionState | None = field(
        repr=False
    )
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "Unsupported iterative continuation progressed-state status or reason."
            ) from error

        blockers = tuple(
            Phase8OfflineReplayIterativeContinuationProgressedSessionStateBlocker(blocker)
            for blocker in self.blockers
        )

        if len(set(blockers)) != len(blockers):
            raise ValueError("Iterative progressed-state blockers cannot contain duplicates.")

        if self.iterative_continuation_transition_application_decision.is_blocked:
            if (
                status
                != (Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus.BLOCKED)
                or reason
                != (
                    Phase8OfflineReplayIterativeContinuationProgressedSessionStateReason.ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_BLOCKED
                )
                or blockers
                != (
                    Phase8OfflineReplayIterativeContinuationProgressedSessionStateBlocker.ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_BLOCKED,
                )
                or self.state is not None
            ):
                raise ValueError(
                    "Blocked iterative continuation progressed-state "
            ...
            if (
                status
                != (Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus.CREATED)
                or reason
                != (Phase8OfflineReplayIterativeContinuationProgressedSessionStateReason.CREATED)
                or blockers
                or not isinstance(
                    self.state,
                ...
                or (
                    self.state.iterative_continuation_transition_application_decision
                    is not self.iterative_continuation_transition_application_decision
                )
            ):
                raise ValueError(
                    "Created iterative continuation progressed-state "
                    "result does not match its application "
                    "decision."
                )
    ...
    def broker_symbol(self) -> str:
        return self.iterative_continuation_transition_application_decision.broker_symbol

    @property
    def direction(
        self,
    ) -> DirectionalPermissionDirection:
        return self.iterative_continuation_transition_application_decision.direction
    ...
    def state_required(
        ...
        if self.state is None:
            raise ValueError(
                "No Phase 8 iterative continuation progressed offline replay-session state was created."
            )
    ...
    def stable_id(self) -> str:
        ...
        return (
            f"{self.iterative_continuation_transition_application_decision.stable_id}:"
            "PHASE_8_OFFLINE_REPLAY_ITERATIVE_"
            "CONTINUATION_PROGRESSED_SESSION_STATE_GENERATION:"
            f"{self.status.value}:"
            f"{self.reason.value}:"
            f"{blocker_fragment}"
        )
...
class StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory:
    """Pure immutable iterative continuation progressed-state factory."""
    ...
    def generate(
        ...
        policy: (
            Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy | None
        ) = None,
        ...
        if not isinstance(
            ...
            raise Phase8OfflineReplayIterativeContinuationProgressedSessionStateError(
                Phase8OfflineReplayIterativeContinuationProgressedSessionStateErrorReason.INVALID_ITERATIVE_CONTINUATION_TRANSITION_APPLICATION_DECISION,
                "iterative_continuation_transition_application_decision "
                "must be a "
                "Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision.",
            )
        ...
        selected_policy = (
            policy or Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy()
        )
        ...
        if iterative_continuation_transition_application_decision.is_blocked:
            return Phase8OfflineReplayIterativeContinuationProgressedSessionStateDecision(
                iterative_continuation_transition_application_decision=(
                    iterative_continuation_transition_application_decision
                ),
                status=(
        ...
        application_receipt = (
            iterative_continuation_transition_application_decision.receipt_required
        )
        iterative_continuation_transition_contract = application_receipt.transition_contract
        prior_source_state = application_receipt.source_state
        prior_application_receipt = application_receipt.prior_application_receipt
        ...
        canonical_payload = _canonical_state_payload(
            ...
            prior_source_state_id=(prior_source_state.stable_id),
            prior_source_state_digest=(prior_source_state.state_digest),
            ...
            lifecycle=(Phase8OfflineReplayIterativeContinuationProgressedSessionLifecycle.ACTIVE),
            state_version=6,
        ...
        state = StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionState(
            iterative_continuation_transition_application_decision=(
                iterative_continuation_transition_application_decision
            ),
            policy=selected_policy,
        ...
        return Phase8OfflineReplayIterativeContinuationProgressedSessionStateDecision(
            ...
            status=(Phase8OfflineReplayIterativeContinuationProgressedSessionStateStatus.CREATED),
            reason=(Phase8OfflineReplayIterativeContinuationProgressedSessionStateReason.CREATED),
            blockers=(),
            state=state,
        )
...
def generate_phase8_offline_replay_iterative_continuation_progressed_session_state(
    iterative_continuation_transition_application_decision: (
        Phase8OfflineReplayIterativeContinuationTransitionApplicationDecision
    ),
    policy: (Phase8OfflineReplayIterativeContinuationProgressedSessionStatePolicy | None) = None,
) -> Phase8OfflineReplayIterativeContinuationProgressedSessionStateDecision:
    return StrategyPhase8OfflineReplayIterativeContinuationProgressedSessionStateFactory().generate(
        iterative_continuation_transition_application_decision,
        policy,
    )
