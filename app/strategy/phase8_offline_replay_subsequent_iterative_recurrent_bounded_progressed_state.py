"""Immutable progressed state after subsequent iterative recurrent application.

This module converts one allowed subsequent iterative recurrent bounded
...
PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_STATE_ACTIVE = "ACTIVE"
PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_STATE_COMPLETED = "COMPLETED"
...
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionState:
    """Reusable immutable state after subsequent recurrent application."""
    ...
    def state_id(self) -> str:
        return (
            "PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_"
            f"PROGRESSED_STATE:SHA256[{self.state_digest}]"
        )


@dataclass(frozen=True, slots=True)
class Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionStateDecision:
    """Allowed or blocked subsequent recurrent progressed-state result."""

    is_allowed: bool
    state: Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionState | None
    blockers: tuple[str, ...]
    ...
    def state_required(
        ...
        if self.state is None:
            raise RuntimeError(
                "Subsequent iterative recurrent bounded progressed replay state is blocked."
            )
        return self.state


class StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedStateFactory:
    """Creates reusable state from one allowed subsequent receipt."""
    ...
    def create(
        ...
        if application_decision is None:
            return Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionStateDecision(
                is_allowed=False,
                state=None,
        ...
        lifecycle = (
            ...
            else PHASE_8_OFFLINE_REPLAY_SUBSEQUENT_ITERATIVE_RECURRENT_BOUNDED_STATE_ACTIVE
        )

        try:
            state = Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionState(
...
def create_phase8_offline_replay_subsequent_iterative_recurrent_bounded_progressed_state(
    application_decision: object,
) -> Phase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedSessionStateDecision:
    """Create one immutable reusable subsequent progressed state."""

    return StrategyPhase8OfflineReplaySubsequentIterativeRecurrentBoundedProgressedStateFactory().create(
        application_decision
    )
